...
class FormulaireTabView {
	...
	public function renderHTML($mod) {
		...
		$aff .= '	<a href="../../../../?menu=5">Convention</a> > ';
		$aff .= '	<a href="../Convention/?id=' . $this->myFormulaire->getConventionID () . '">Conventions</a> > ';
		$aff .= '	<a href="?id=' . $this->myFormulaire->getConventionID () . '">Formulaires</a> > ';
		switch ($mod) {
			case 'new' :
				$aff .= 'Création';
				break;
		...
		switch ($mod) {
			case 'new' :
				$aff .= '<form action="?action=new&id=' . $this->myFormulaire->getConventionID () . '" method="POST" onsubmit="return FormulaireValidation()">';
				break;
			case 'edit' :
	...
	private function renderLCA_HTML($mod) {
		...
		$aTabALL = $aFormulaireLCA->SQL_SELECT_ALL_BY_CONVENTION_TYPE ( $this->myFormulaire->getConventionID (), $this->myFormulaire->getTypeFormulaire () );
