<?php
class ConventionFormulaireComposantDAO {
	public function create($conventionFormulaireComposant) {
		$sql = "INSERT INTO conv_formulaire_composant(id, nom, valeur, type) VALUES('%s', '%s', '%s', '%s')";
	...
	public function update($conventionFormulaireComposant) {
		$sql = "UPDATE conv_formulaire_composant SET nom='%s', valeur='%s' WHERE id='%s'";
	...
	public function delete($id) {
		$sql = "DELETE FROM conv_formulaire_composant  WHERE id='%s'";
		$query = sprintf ( $sql, mysqli_real_escape_string ($_SESSION['LINK'], $id ) );

		mysqli_query ($_SESSION['LINK'], $query ) or die ( mysqli_error ($_SESSION['LINK']) );
	}
	public function find($id) {
		$sql = "SELECT id, nom, valeur,type FROM conv_formulaire_composant WHERE id = '%s'";
		$query = sprintf ( $sql, mysqli_real_escape_string ($_SESSION['LINK'], $id ) );
		$result = mysqli_query ($_SESSION['LINK'], $query ) or die ( mysqli_error ($_SESSION['LINK']) );

		$instance = new ConventionFormulaireComposant ();
