<?php
class ConventionFormulaireFieldDAO {
	public function create($conventionFormulaireField) {
		$sql = "INSERT INTO conv_formulaire_field(id, question, type, choix1, choix2, choix3, choix4, choix5, choix6, choix7, choix8, choix9, choix10) VALUES('%s', '%s', '%s','%s', '%s', '%s','%s', '%s', '%s','%s', '%s', '%s', '%s')";
	...
	public function update($conventionFormulaireField) {
		$sql = "UPDATE conv_formulaire_field SET question='%s', type='%s', choix1='%s', choix2='%s', choix3='%s', choix4='%s', choix5='%s', choix6='%s', choix7='%s', choix8='%s', choix9='%s', choix10='%s' WHERE id='%s'";
	...
	public function delete($id) {
		$sql = "DELETE FROM conv_formulaire_field WHERE id = '%s'";
		$query = sprintf ( $sql, mysqli_real_escape_string ($_SESSION['LINK'], $id ) );

		mysqli_query ($_SESSION['LINK'], $query ) or die ( mysqli_error ($_SESSION['LINK']) );
	}
	public function find($id) {
		$sql = "SELECT id, question, type, choix1, choix2, choix3, choix4, choix5, choix6, choix7, choix8, choix9, choix10 FROM conv_formulaire_field WHERE id = '%s'";
		$query = sprintf ( $sql, mysqli_real_escape_string ($_SESSION['LINK'], $id ) );
		$result = mysqli_query ($_SESSION['LINK'], $query ) or die ( mysqli_error ($_SESSION['LINK']) );

		$instance = new ConventionFormulaireField ();

		while ( $line = mysqli_fetch_array  ( $result ) ) {
			$instance->setId ( $line [0] );
			$instance->setQuestion ( str_replace ( '\\', '', $line [1] ) );
