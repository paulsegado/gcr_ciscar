...
if ($_SESSION ['ADMIN'] ['USER'] ['CONNECTED']) {
	...
	if (isset ( $_GET ['action'] )) {
		$aFonctionBNControler = new FonctionBNControler ();
		$aFonctionBNControler->run ();
	}
	...
	if (! isset ( $_GET ['action'] )) {
		...
		$aFonctionBNListe->select_all_fonctionbn_tri ( isset ( $_GET ['column'] ) ? $_GET ['column'] : '1', isset ( $_GET ['order'] ) ? $_GET ['order'] : 'a' );

		$aFonctionBNListeView = new FonctionBNListeView ( $aFonctionBNListe );
		$aFonctionBNListeView->render ();
	}
