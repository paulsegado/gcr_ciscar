...
class BiblioMediaDAO {
	...
	public function findAll() {
		$sql = "SELECT id, titre, description, create_on, file_name, file_mime, file_size FROM biblio_media ORDER BY id DESC, create_on DESC";
	...
	// Specific Search //
	public function findAllByWord($word) {
		$sql = "SELECT id, titre, description, create_on, file_name, file_mime, file_size FROM biblio_media ";
		$sql .= "WHERE titre LIKE '%s' OR description LIKE '%s' OR file_name LIKE '%s' ORDER BY create_on DESC";
		$query = sprintf ( $sql, mysqli_real_escape_string ($_SESSION['LINK'], '%' . $word . '%' ), mysqli_real_escape_string ($_SESSION['LINK'], '%' . $word . '%' ), mysqli_real_escape_string ($_SESSION['LINK'], '%' . $word . '%' ) );
		$result = mysqli_query ($_SESSION['LINK'], $query ) or die ( mysqli_error ($_SESSION['LINK']) );
