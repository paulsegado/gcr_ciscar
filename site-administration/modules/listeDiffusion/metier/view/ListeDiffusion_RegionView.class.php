<?php
class ListeDiffusion_RegionView {
	private $myList;
	...
	public function renderHTML() {
		...
		$aff .= '&nbsp;>&nbsp;Crit&eacute;re R&eacute;gion';
		$aff .= '</div><br/><br/>' . "\n";
		...
		foreach ( $this->myList as $aRegion ) {
			$aff .= '<tr' . ($i == 0 ? $row1 : $row2) . '>';
			$aff .= '<td>' . $aRegion->getName () . '</td>';
			$aff .= '<td width="50" align="center"><a href="#" onclick="javascript:addRule(\'' . $aRegion->getID () . '\',\'' . addslashes ( $aRegion->getName () ) . '\')"><img src="../../include/images/add.png" border="0"/></a></td>';
			$aff .= '</tr>';
			$i = ($i == 0 ? 1 : 0);
		}
