...
<meta http-equiv="Content-Type" content="text/html; charset=iso-8859-15" />
	...
	echo HelperHead::includeJS ( '../../../include/js/DocPartenaire.js' );
...
if ($_SESSION ['ADMIN'] ['USER'] ['CONNECTED']) {
	...
	if (isset ( $_GET ['action'] )) {
		$aControler = new DocPartenaireControler ();
		$aControler->run ();
	} else {
		$aList = new DocPartenaireList ();
		$aList->SQL_SELECT_ALL_TRI ( isset ( $_GET ['column'] ) ? $_GET ['column'] : '2', isset ( $_GET ['order'] ) ? $_GET ['order'] : 'a' );

		$aView = new DocPartenaireListView ( $aList->getList () );
		$aView->render ();
	}
	// DB Deconnection
