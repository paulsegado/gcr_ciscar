...
if ($_SESSION ['ADMIN'] ['USER'] ['CONNECTED']) {
	...
	if (isset ( $_GET ['action'] )) {
		$aControler = new DocZoomControler ();
		$aControler->run ();
	} else {
		$aList = new DocZoomList ();
		$aList->SQL_SELECT_ALL_TRI ( isset ( $_GET ['column'] ) ? $_GET ['column'] : '1', isset ( $_GET ['order'] ) ? $_GET ['order'] : 'a' );

		$aView = new DocZoomListView ( $aList->getList () );
		$aView->render ();
	}
