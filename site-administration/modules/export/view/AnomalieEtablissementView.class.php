 ...
 * @version 1.0.4
 */
class AnomalieEtablissementView implements DefaultListView {
	private $myList;
	private $nbrEntre;
	...
	public function renderHTML() {
		...
		$aff .= '	<a href="../../?menu=1">Général</a>&nbsp;>&nbsp;<a href="?">Export</a>&nbsp;>&nbsp;Anomalie Etablissement sans Individu';
		$aff .= '</div><br/><br/>';
		...
		$aff .= '<tr class="title">';
		$aff .= '<td align="center" width="50"><a href="?action=anomalie_etablissement&tri=1&ordre=' . $sens . $rech . '" ><b>#</b></a></td>';
		$aff .= '<td align="center"><a href="?action=anomalie_etablissement&tri=2&ordre=' . $sens . $rech . '" ><b>Concession de</b></a></td>';
		$aff .= '<td align="center"><a href="?action=anomalie_etablissement&tri=3&ordre=' . $sens . $rech . '" ><b>Nom</b></a></td>';
		$aff .= '<td align="center"><a href="?action=anomalie_etablissement&tri=4&ordre=' . $sens . $rech . '" ><b>CP / Bureau</b></a></td>';
		$aff .= '<td align="center"><a href="?action=anomalie_etablissement&tri=5&ordre=' . $sens . $rech . '" ><b>Ville</b></a></td>';
		...
		foreach ( $this->myList->getList () as $aRow ) {
			...
			$aff .= '<td align="center" class="' . ($row == 1 ? 'row1' : 'row2') . '">' . $aRow [5] . '</td>';
			$aff .= '<td align="center" class="' . ($row == 1 ? 'row1' : 'row2') . '">' . $aRow [1] . '</td>';
			$aff .= '<td align="center" class="' . ($row == 1 ? 'row1' : 'row2') . '">' . $aRow [4] . ' / ' . $aRow [3] . '</td>';
			$aff .= '<td align="center" class="' . ($row == 1 ? 'row1' : 'row2') . '">' . $aRow [5] . '</td>';
			$aff .= '<td align="center" class="' . ($row == 1 ? 'row1' : 'row2') . '"><b><a href="../etablissement/?action=m&id=' . $aRow [0] . '" target="_BLANK"><img src="../../include/images/voir.jpg" width="16" border="0"/></a></td>';
			$aff .= '</tr>';
		...
		$aff .= '<script>
                ...
                $("#Pagination").pagination(num_entries, {
					...
					link_to:\'?action=anomalie_etablissement&page=__id__' . $tri . $ordre . $rech . '\',
