 ...
 * @version 1.0.4
 */
class AnomalieSageIndividu implements DefaultModeleList {
	private $myList;
	...
	public function SQL_SELECT_BY_SITE($aID) {
		...
		$sql = "SELECT IndividuID, Nom, Prenom, Mail FROM annuaire_individu WHERE LoginSage='' AND AnnuaireID='%s'";
	...
	public function SQL_SELECT_ALL() {
		...
		$sql = "SELECT IndividuID, Nom, Prenom,Mail FROM annuaire_individu WHERE LoginSage=''";
	...
	public function SQL_COUNT($aID) {
		$sql = "SELECT COUNT(IndividuID) FROM annuaire_individu WHERE LoginSage='' AND AnnuaireID='%s'";
	...
	public function SQL_SEARCH_COUNT($aID, $Recherche) {
		...
		$query = sprintf ( $sql, mysqli_real_escape_string ($_SESSION['LINK'], $aID ), mysqli_real_escape_string ($_SESSION['LINK'], '%' . $Recherche . '%' ), mysqli_real_escape_string ($_SESSION['LINK'], '%' . $Recherche . '%' ), mysqli_real_escape_string ($_SESSION['LINK'], '%' . $Recherche . '%' ), mysqli_real_escape_string ($_SESSION['LINK'], '%' . $Recherche . '%' ) );
