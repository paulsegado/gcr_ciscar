<?php
/**
 * Export du Top 20 des consultations
	...
	if (isset ( $_GET ['export'] )) {
		...
		switch ($_GET ['export']) {
			...
			case 1 :
				...
				$aStat->SQL_CONSULT_GRAPH ( isset ( $_GET ['m'] ) ? $_GET ['m'] : date ( 'm', $time ), isset ( $_GET ['a'] ) ? $_GET ['a'] : date ( 'Y', $time ), $_GET ['site'], (isset ( $_GET ['type'] ) ? $_GET ['type'] : '3'), 'top' );
				header ( 'Content-Type:application/csv-tab-delimited-table' );
				header ( 'Content-disposition: filename="Top 20 des consultations ' . $site . ' ' . FunctionDate::getMois ( $_GET ['m'] ) . ' ' . $_GET ['a'] . '.csv"' );
			...
			case 2 :
				...
				header ( 'Content-Type:application/csv-tab-delimited-table' );
				header ( 'Content-disposition: filename="Top 20 des consultations ' . $site . '.csv"' );
