...
class PHPGraphLib {
	...
	function initialize() {
		...
		$this->image = @imagecreate ( $this->width, $this->height ) or die ( "Cannot Initialize new GD image stream - Check your PHP setup" );
	...
	function generateBars() {
		...
		if ($this->bool_multi_offset) {
			...
			switch ($this->data_set_count) {
				case 2 :
					$dataset_offset = $this->bar_width * ($this->multi_offset_two / 100);
					break;
				case 3 :
					$dataset_offset = $this->bar_width * ($this->multi_offset_three / 100);
					break;
				case 4 :
					$dataset_offset = $this->bar_width * ($this->multi_offset_four / 100);
					break;
				case 5 :
					$dataset_offset = $this->bar_width * ($this->multi_offset_five / 100);
					break;
			}
		}
	...
	function finalizeColors() {
		if ($this->bool_gradient) {
			...
			while ( $num_set < $this->data_set_count ) {
				...
				$color_darken_decimal = (100 - $this->color_darken_factor) / 100;
				$this->multi_bar_colors [$num_set] = imagecolorallocate ( $this->image, round ( $color_parts ['red'] * $color_darken_decimal ), round ( $color_parts ['green'] * $color_darken_decimal ), round ( $color_parts ['blue'] * $color_darken_decimal ) );
				$num_set ++;
			}
		...
		if ($this->bool_line) {
			if (! $this->bool_bars) {
				...
				while ( $num_set < $this->data_set_count ) {
					...
					$color_parts = imagecolorsforindex ( $this->image, $color_ref );
					$color_darken_decimal = (100 - $this->color_darken_factor) / 100;
					$this->line_color [$num_set] = imagecolorallocate ( $this->image, round ( $color_parts ['red'] * $color_darken_decimal ), round ( $color_parts ['green'] * $color_darken_decimal ), round ( $color_parts ['blue'] * $color_darken_decimal ) );
					$num_set ++;
				}
			} else {
				$num_set = count ( $this->line_color );
	...
	function addData($data, $data2 = '', $data3 = '', $data4 = '', $data5 = '') {
		...
		if (is_array ( $data4 )) {
			$this->data_array [] = $data4;
		}
		if (is_array ( $data5 )) {
			$this->data_array [] = $data5;
		}
		// assess data
	...
	function generateLegend() {
		...
		$this->legend_width = $totalChars * $this->legend_text_width + ($this->legend_padding * 2.2) + ($this->data_set_count * ($swatchSize + ($this->legend_padding * 2)));
		$this->legend_x = $this->x_axis_x2 - $this->legend_width;
		$highestElement = ($this->top_margin < $this->y_axis_y2) ? $this->top_margin : $this->y_axis_y2;
		$this->legend_y = ($highestElement / 2) - ($this->legend_height / 2); // centered
		                                                                      // background
		imagefilledrectangle ( $this->image, $this->legend_x, $this->legend_y, $this->legend_x + $this->legend_width, $this->legend_y + $this->legend_height, $this->legend_color );
		// border
		imagerectangle ( $this->image, $this->legend_x, $this->legend_y, $this->legend_x + $this->legend_width, $this->legend_y + $this->legend_height, $this->legend_outline_color );
		$length_covered = 0;
		for($i = 0; $i < $this->data_set_count; $i ++) {
			$data_label = '';
