...
<!-- CISCAR  on supprimes les borudres de la table          
...
<!-- CISCAR renseigne la ligne livraison suivant que le port est en SUS ou gratuit -->     
                            {if isset($option_list_delivery)}                          
								{foreach $option_list_delivery as $id_address => $option_list}
									{foreach $option_list as $key => $option}
										{if !$option.unique_carrier}
											<td colspan="2" class="price" id="total_shipping">{l s='In addition'}</td>
										{else}								
											{foreach from=$option_list item='carrier'}
											
												{foreach $carrier.carrier_list as $key => $instance}
													{foreach $instance.instance as $key => $detail}
														{if $key=='is_free'}
															{if $detail=='0'}
																<td colspan="2" class="price" id="total_shipping">{l s='In addition'}</td>
															{else}
                    ...
                    {if $use_taxes}
                        ...
                        {/if}
                        <tr class="cart_total_price">
                            <td colspan="4" class="total_price_container text-right"><span>{l s='Total ex VAT'}</span></td>
...
<!-- CISCAR on supprime le lien qui est repris dans payment_execution_auto.tpl 
