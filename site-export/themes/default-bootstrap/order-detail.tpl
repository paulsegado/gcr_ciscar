...
<div class="info-order box">
	{if $carrier->id}<p><strong class="dark">{l s='Shipping'}</strong> 
	{if $carrier->name == "0"}{$shop_name|escape:'html':'UTF-8'}{else}{$carrier->name|escape:'html':'UTF-8'}{/if}</p>{/if}
...
<!-- CISCAR On masque la ligne Articles (hors taxes)-->				
			...
			{/if}
<!-- CISCAR On masque la ligne Produits TTC-->	
			<tr class="item" style="display:none;">
				<td colspan="{if $return_allowed}2{else}1{/if}">
			...
			{/if}
<!-- CISCAR On masque la ligne des frais de manutention -->			
			<tr class="item" style="display:none;">
				<td colspan="{if $return_allowed}2{else}1{/if}">
...
<!-- CISCAR on ajoute HT sur le total des produits -->			
				<td colspan="{if $return_allowed}2{else}1{/if}">
					<strong>{l s='Total'} {l s='Tax excluded'}</strong>
				</td>
				<td colspan="{if $order->hasProductReturned()}5{else}4{/if}">
...
<!-- CISCAR on supprime le tableau concernant le transporteur -->
	<table class="table table-bordered footab" style="display:none;">
		<thead>
			<tr>
				<th class="first_item">{l s='Date'}</th>
