...
<!-- CISCAR , On rend cette div invisible si le produit n'est pas disponible à la vente dans le back office 	-->					
					<div class="box-info-product" {if !$product->available_for_order} style="display:none;" {/if}>
...
<!-- CISCAR 3, ajouter le prix unitaire si renseigne 	-->									
										{if  $product->unit_price_ratio > 0.000000}
											{math equation="pprice / punit_price" pprice=$productPrice  punit_price=$product->unit_price_ratio assign=unit_price}
											<span class="price" itemprop="price" id="unit_price_display">{$unit_price}</span>
...
<!-- FIN ajouter le prix unitaire -->									
										...
										{/if}
<!-- CISCAR, ajouter l'expression "le lot" si le prix unitaire est renseigne 	-->		
										{if  $product->unit_price_ratio > 0.000000}
											{l s='The unit'}
										{/if}
...
<!-- CISCAR, ajouter le prix du lot si renseigne prix unitaire renseigne 	-->	
							{if $product->unit_price_ratio > 0.000000}
								<p class="unit-price"><span id="our_unit_price_display">{convertPrice price=$productPrice|floatval}</span> {l s='package'} </p>
								{hook h="displayProductPriceBlock" product=$product type="unit_price"}
							{/if}
<!-- CISCAR, on supprime l'affichage du lot  							
								{math equation="pprice / punit_price" pprice=$productPrice  punit_price=$product->unit_price_ratio assign=unit_price}
								<p class="unit-price"><span id="unit_price_display">{convertPrice price=$productPrice|floatval}</span> {l s='per'} {$product->unity|escape:'html':'UTF-8'}</p>
								{hook h="displayProductPriceBlock" product=$product type="unit_price"}
-->	
