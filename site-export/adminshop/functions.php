 ...
 *        	Base URI
 * @param string $theme
 *        	Theme name (eg. default)
 * @param array $array_db
 *        	Parameters in order to connect to database
 *        	
 */
 ...
 *        	Date in SQL format (YYYY-MM-DD HH:mm:ss)
 ...
 *        	String to highlight (in XHTML/CSS)
 * @param string $type
 *        	Category type (products/cms)
 *        	
 */
	...
	if ($category_type == 'catalog') {
		...
		if (isset ( $category ['id_category'] )) {
			...
			foreach ( $categories as $category ) {
				...
				$full_path .= $edit . ($n < $n_categories ? '<a href="' . Tools::safeOutput ( $url_base . '&id_category=' . ( int ) $category ['id_category'] . '&viewcategory&token=' . Tools::getAdminToken ( 'AdminCategories' . ( int ) Tab::getIdFromClassName ( 'AdminCategories' ) . ( int ) $context->employee->id ) ) . '" title="' . htmlentities ( $category ['name'], ENT_NOQUOTES, 'UTF-8' ) . '">' : '') . (! empty ( $highlight ) ? str_ireplace ( $highlight, '<span class="highlight">' . htmlentities ( $highlight, ENT_NOQUOTES, 'UTF-8' ) . '</span>', $category ['name'] ) : $category ['name']) . ($n < $n_categories ? '</a>' : '') . (($n ++ != $n_categories || ! empty ( $path )) ? ' > ' : '');
			}
...
function checkingTab($tab) {
	...
	if (! $admin_obj->viewAccess () && ($admin_obj->table != 'employee' || Context::getContext ()->employee->id != Tools::getValue ( 'id_employee' ) || ! Tools::isSubmit ( 'updateemployee' ))) {
		$admin_obj->_errors = array (
				Tools::displayError ( 'Access denied.' ) 
		);
 ...
 *        	the XML to convert
 * @param bool $flatten_values
 *        	Choose wether to flatten values
 *        	or to set them under a particular index.
 *        	defaults to true;
 * @param bool $flatten_attributes
 *        	Choose wether to flatten attributes
 *        	or to set them under a particular index.
 *        	Defaults to true;
 * @param bool $flatten_children
 *        	Choose wether to flatten children
 *        	or to set them under a particular index.
 *        	Defaults to true;
 * @param string $value_key
 *        	index for values, in case $flatten_values was set to false. Defaults to "@value"
 * @param string $attributes_key
 *        	index for attributes, in case $flatten_attributes was set to false. Defaults to "@attributes"
 * @param string $children_key
 *        	index for children, in case $flatten_children was set to false. Defaults to "@children"
