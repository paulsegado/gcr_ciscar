...
if (isset ( $_GET ['action'] )) {
	switch ($_GET ['action']) {
		...
		case 'save_img' :
			...
			if (strpos ( $_POST ['path'], '/' ) === 0 || strpos ( $_POST ['path'], '../' ) !== false || strpos ( $_POST ['path'], './' ) === 0 || strpos ( $_POST ['url'], 'http://featherfiles.aviary.com/' ) !== 0 || $_POST ['name'] != fix_filename ( $_POST ['name'], $transliteration ) || ! in_array ( strtolower ( $info ['extension'] ), array (
					'jpg',
					'jpeg',
					'png' 
			) )) {
	...
	<div class="jp-type-single">
		...
		<div class="jp-no-solution">
			...
			either update your browser to a recent version or update your <a
				href="http://get.adobe.com/flashplayer/" class="_blank">Flash plugin</a>.
		</div>
	</div>
</div>
<?php
...
Tools::safeOutput ( $_GET ['title'] );
				?>",
									mp3: "<?php
				
echo Tools::safeOutput ( $preview_file );
				?>",
									m4a: "<?php
