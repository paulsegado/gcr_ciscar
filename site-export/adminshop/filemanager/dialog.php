...
echo $loading_bar ? "true" : "false";
	?>;
			var image_editor =<?php
	
echo $aviary_active ? "true" : "false";
	?>;
			//dropzone config
			Dropzone.options.myAwesomeDropzone = {
				dictInvalidFileType: "<?php
	
echo lang_Error_extension;
	?>",
				dictFileTooBig: "<?php
	
echo lang_Error_Upload;
	?>",
...
echo Tools::safeOutput ( $popup );
	?>" />
	<input type="hidden" id="view"
		value="<?php
	
echo Tools::safeOutput ( $view );
	?>" />
	<input type="hidden" id="cur_dir"
		value="<?php
	
echo Tools::safeOutput ( $cur_dir );
	?>" />
	<input type="hidden" id="cur_dir_thumb"
		value="<?php
	
echo Tools::safeOutput ( $subdir );
	?>" />
	<input type="hidden" id="insert_folder_name"
		value="<?php
	
echo Tools::safeOutput ( lang_Insert_Folder_Name );
	?>" />
	<input type="hidden" id="new_folder"
		value="<?php
	
echo Tools::safeOutput ( lang_New_Folder );
	?>" />
	<input type="hidden" id="ok"
		value="<?php
	
echo Tools::safeOutput ( lang_OK );
	?>" />
	<input type="hidden" id="cancel"
		value="<?php
	
echo Tools::safeOutput ( lang_Cancel );
	?>" />
	<input type="hidden" id="rename"
		value="<?php
	
echo Tools::safeOutput ( lang_Rename );
	?>" />
	<input type="hidden" id="lang_duplicate"
		value="<?php
	
echo Tools::safeOutput ( lang_Duplicate );
	?>" />
...
echo Tools::safeOutput ( $subfolder );
	?>" />
	<input type="hidden" id="file_number_limit_js"
		value="<?php
	
echo Tools::safeOutput ( $file_number_limit_js );
	?>" />
	<input type="hidden" id="descending"
		value="<?php
	
echo $descending ? "true" : "false";
	?>" />
	<?php
...
echo str_replace ( array (
			...
			'' 
	), $protocol . "://" . $_SERVER ['HTTP_HOST'] . Tools::safeOutput ( $_SERVER ['REQUEST_URI'] ) );
	?>" />
	<input type="hidden" id="lang_show_url"
		value="<?php
	
echo Tools::safeOutput ( lang_Show_url );
	?>" />
	<input type="hidden" id="lang_extract"
		value="<?php
	
echo Tools::safeOutput ( lang_Extract );
	?>" />
...
echo Tools::safeOutput ( lang_Edit_image );
	?>" />
...
echo Tools::safeOutput ( lang_Upload_base );
			?></a></li>
				<li><a href="#tab2" id="uploader-btn" data-toggle="tab"><?php
			
echo Tools::safeOutput ( lang_Upload_java );
			?></a></li>
			</ul>
...
echo Tools::safeOutput ( $subfolder . $subdir );
		?>" /> <input type="hidden" name="path_thumb"
							value="<?php
...
echo Tools::safeOutput ( $subfolder . $subdir );
		?>" />
						...
						<div class="fallback">
								<?php echo lang_Upload_file ?>:<br /> <input name="file"
								type="file" /> <input type="hidden" name="fldr"
								value="<?php
...
echo Tools::safeOutput ( $_GET ['type'] );
		?>" /> <input type="hidden" name="field_id"
								value="<?php
		
echo ( int ) $_GET ['field_id'];
...
echo Tools::safeOutput ( $lang );
		?>" /> <input type="hidden" name="filter"
								value="<?php
		
echo Tools::safeOutput ( $filter );
		?>" /> <input type="submit" name="submit"
								value="<?php echo lang_OK ?>" />
						</div>
					</form>
					<div class="upload-help"><?php
		
echo Tools::safeOutput ( lang_Upload_base_help );
		?></div>
						<?php
...
echo Tools::safeOutput ( lang_Upload_java_help );
			?></div>
				</div>
					<?php
		}
...
echo Tools::safeOutput ( lang_Toolbar );
	?> -></div>
...
echo Tools::safeOutput ( lang_Actions );
	?>:</span>
								<?php
...
echo Tools::safeOutput ( lang_Upload_file );
		?>">
...
echo Tools::safeOutput ( lang_View_boxes );
	?>">
										<i
											class="icon-th <?php
...
echo Tools::safeOutput ( lang_View_list );
	?>">
										<i
											class="icon-align-justify <?php
...
echo Tools::safeOutput ( lang_View_columns_list );
	?>">
										<i
											class="icon-fire <?php
...
echo Tools::safeOutput ( lang_Filters );
	?>:</span>
								<?php
...
echo Tools::safeOutput ( lang_Files );
		?>"
...
echo Tools::safeOutput ( lang_Images );
		?>"
...
echo Tools::safeOutput ( lang_Archives );
		?>"
...
echo Tools::safeOutput ( lang_Videos );
		?>"
...
echo Tools::safeOutput ( lang_Music );
		?>"
...
echo fix_strtolower ( lang_Text_filter );
	?>..."
										value="<?php
...
if ($n_files > $file_number_limit_js) {
...
echo Tools::safeOutput ( lang_All );
	?>"
										<?php
	
if (Tools::getValue ( 'type' ) == 1 || Tools::getValue ( 'type' ) == 3) {
		?>
	...
	?>
		<ul class="breadcrumb">
				<li class="pull-left"><a
					href="<?php echo Tools::safeOutput($link) ?>/"><i class="icon-home"></i></a></li>
...
echo "/";
	...
	?>
			<li class="pull-right"><a class="btn-small"
					href="javascript:void('')" id="info"><i class="icon-question-sign"></i></a></li>
...
echo Tools::safeOutput ( lang_Filename );
	?></a></li>
							<li><a
								class="sorter sort-date <?php
...
echo Tools::safeOutput ( lang_Date );
	?></a></li>
							<li><a
								class="sorter sort-size <?php
...
echo Tools::safeOutput ( lang_Size );
	?></a></li>
							<li><a
								class="sorter sort-extension <?php
...
echo Tools::safeOutput ( lang_Type );
	?></a></li>
						</ul>
					</div>
				</li>
...
if (@opendir ( $current_path . $subfolder . $subdir ) === false) {
		?>
		<br />
...
echo Tools::safeOutput ( lang_Swipe_help );
		?></h4>
	<?php
...
echo Tools::safeOutput ( $folder_message );
			?></div>
	<?php
		}
		?>
...
if ($show_sorting_bar) {
		...
		<!-- sorter -->
				<div
					class="sorter-container <?php
			
echo "list-view" . Tools::safeOutput ( $view );
			?>">
					<div class="file-name">
						<a
							class="sorter sort-name <?php
...
echo Tools::safeOutput ( lang_Dimension );
			?></div>
...
echo Tools::safeOutput ( lang_Operations );
			?></div>
				</div>
	<?php
		}
...
echo Tools::safeOutput ( $n_files );
		?>" />
				<!--ul class="thumbnails ff-items"-->
				<ul
					class="grid cs-style-2 <?php
...
echo Tools::safeOutput ( $icon_theme );
			?>/folder<?php
...
if ($ellipsis_title_after_first_row) {
				...
				?>">
									<a class="folder-link"
										...
										href="dialog.php?<?php echo Tools::safeOutput($get_params.rawurlencode($src)."&".uniqid()) ?>"><?php
				
echo Tools::safeOutput ( $file );
				?></a>
								</h4>
							</div>
...
echo Tools::safeOutput ( $file_array ['date'] );
				?>" />
							<input type="hidden" class="size"
								value="<?php
				
echo Tools::safeOutput ( $file_array ['size'] );
				?>" />
				...
				?></div>
							<figcaption>
								<a href="javascript:void('')"
									class="tip-left edit-button <?php
...
echo Tools::safeOutput ( $subfolder . $subdir . $file );
				?>"
									data-thumb="<?php
				
echo Tools::safeOutput ( $subdir . $file );
				?>"> <i
	...
	<?php
		...
		foreach ( $files as $nu => $file_array ) {
			...
			if ((! (Tools::getValue ( 'type' ) == 1 && ! $is_img) && ! ((Tools::getValue ( 'type' ) == 3 && ! $is_video) && (Tools::getValue ( 'type' ) == 3 && ! $is_audio))) && $class_ext > 0) {
				?>
	<li
						class="ff-item-type-<?php
				
echo Tools::safeOutput ( $class_ext );
				?> file"
						data-name="<?php
...
echo Tools::safeOutput ( $filename . " thumbnails" );
				?>"
											class="<?php echo $show_original ? "original" : "" ?> <?php echo $is_icon_thumb ? "icon" : "" ?>"
											src="<?php
				
echo Tools::safeOutput ( $src_thumb );
				?>">
									</div>
								</div>
								<div
									class="img-precontainer-mini <?php
...
if ($is_img) {
				...
				?>">
									<div
										class="filetype <?php echo $extension_lower ?> <?php
...
echo Tools::safeOutput ( $mini_src );
					?>">
						<?php
				}
				?>
...
echo Tools::safeOutput ( $filename );
				?></a>
								</h4>
							</div>
							<input type="hidden" class="date"
...
echo Tools::safeOutput ( Context::getContext ()->shop->physical_uri . 'img/cms/' . $subfolder . $subdir . $file );
					?>"
