...
class AdminCarrierWizardControllerCore extends AdminController {
	...
	public function renderView() {
		...
		if (Shop::isFeatureActive ()) {
			array_splice ( $this->tpl_view_vars ['wizard_contents'] ['contents'], 1, 0, array (
					0 => $this->renderStepTwo ( $carrier ) 
			) );
		}
	...
	public function renderStepOne($carrier) {
		...
		return $this->renderGenericForm ( array (
	...
	public function renderStepFive($carrier) {
		...
		$active_form = $this->renderGenericForm ( array (
				'form' => $this->fields_form 
		), $fields_value );
	...
	public function getStepTwoFieldsValues($carrier) {
		return array (
				'shop' => $this->getFieldValue ( $carrier, 'shop' ) 
		);
	}
	...
	public function getStepFiveFieldsValues($carrier) {
		return array (
				'active' => $this->getFieldValue ( $carrier, 'active' ) 
		);
	}
	public function ajaxProcessChangeRanges() {
		...
		if ((! ( int ) $shipping_method = Tools::getValue ( 'shipping_method' )) || ! in_array ( $shipping_method, array (
	...
	public function getValidationRules() {
		...
		if (Shop::isFeatureActive ()) {
			$tmp = $step_fields;
			$step_fields = array_slice ( $tmp, 0, 1, true ) + array (
					2 => array (
							'shop' 
					) 
			);
