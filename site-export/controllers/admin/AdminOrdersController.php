...
class AdminOrdersControllerCore extends AdminController {
	...
	public function __construct() {
		...
		parent::__construct ();
//<!-- CISCAR pour pouvoir dupprimer une commande à partir du back-office
					...
					if (! count ( $this->errors )) {
						...
						if (! $customer_message->add ()) {
							...
							$varsTpl = array (
									...
									'{order_name}' => $order->getUniqReference (),
									'{message}' => $message,
//<!-- CISCAR on ajoute la reference de la commande									
	...
	public function renderView() {
		...
		if ($carrier->is_module) {
			...
			if (method_exists ( $module, 'displayInfoByCart' )) {
				$carrier_module_call = call_user_func ( array (
						$module,
						'displayInfoByCart' 
				), $order->id_cart );
			}
		...
		if ($this->tabAccess ['edit'] === '1') {
			...
			if (Validate::isLoadedObject ( $cart )) {
				...
				if (Validate::isLoadedObject ( $customer )) {
					...
					if (Mail::Send ( ( int ) $cart->id_lang, 'backoffice_order', Mail::l ( 'Process the payment of your order', ( int ) $cart->id_lang ), $mailVars, $customer->email, $customer->firstname . ' ' . $customer->lastname, null, null, null, null, _PS_MAIL_DIR_, true, $cart->id_shop )) {
						die ( Tools::jsonEncode ( array (
								'errors' => false,
								'result' => $this->l ( 'The email was sent to your customer.' ) 
						) ) );
					}
				}
			}
			$this->content = Tools::jsonEncode ( array (
					'errors' => true,
					'result' => $this->l ( 'Error in sending the email to your customer.' ) 
			) );
		}
	}
	public function ajaxProcessAddProductOnOrder() {
		...
		if ($update_quantity < 0) {
			...
			die ( Tools::jsonEncode ( array (
					'error' => sprintf ( Tools::displayError ( 'You must add %d minimum quantity', false ), $minimal_quantity ) 
			) ) );
		} elseif (! $update_quantity) {
			die ( Tools::jsonEncode ( array (
					'error' => Tools::displayError ( 'You already have the maximum quantity available for this product.', false ) 
			) ) );
		}
	...
	public function sendChangedNotification(Order $order = null) {
		...
		Hook::exec ( 'actionOrderEdited', array (
				'order' => $order 
		) );
	}
