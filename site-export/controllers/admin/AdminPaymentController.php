	...
	public function renderView() {
		...
		foreach ( $lists as $key_list => $list ) {
			...
			foreach ( $list ['items'] as $key_item => $item ) {
				...
				if ($name_id === 'currency' && Tools::strpos ( $list ['items'] [$key_item] ['name'], '(' . $list ['items'] [$key_item] ['iso_code'] . ')' ) === false) {
					$list ['items'] [$key_item] ['name'] = sprintf ( $this->l ( '%1$s (%2$s)' ), $list ['items'] [$key_item] ['name'], $list ['items'] [$key_item] ['iso_code'] );
				}
