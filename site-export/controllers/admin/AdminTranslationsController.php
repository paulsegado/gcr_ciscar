...
class AdminTranslationsControllerCore extends AdminController {
	...
	public function initForm($method_name) {
		...
		$title = sprintf ( $this->l ( '%1$s (Language: %2$s, Theme: %3$s)' ), $this->translations_informations [$this->type_selected] ['name'], $this->lang_selected->name, $this->theme_selected ? $this->theme_selected : $this->l ( 'none' ) );
	...
	public function initMain() {
		...
		$array_stream_context = @stream_context_create ( array (
				'http' => array (
						'method' => 'GET',
						'timeout' => 8 
				) 
	...
	public function submitImportLang() {
		if (! isset ( $_FILES ['file'] ['tmp_name'] ) || ! $_FILES ['file'] ['tmp_name']) {
			...
			$iso_code = str_replace ( array (
					'.tar.gz',
					'.gzip' 
			), '', $filename );
			if (Validate::isLangIsoCode ( $iso_code )) {
				$themes_selected = Tools::getValue ( 'theme', array (
	...
	/**
	 * Filter the translation files contained in a .
	 * gzip pack
	 ...
	 *
	 * @param array $list
	 *        	Is the output of Archive_Tar::listContent()
	 *        	
	 * @return array
	 */
	...
	protected function findAndWriteTranslationsIntoFile($file_name, $files, $theme_name, $module_name, $dir = false) {
		...
		if (! isset ( $cache_file [$theme_name . '-' . $file_name] )) {
			...
			if (! is_writable ( $file_name )) {
				throw new PrestaShopException ( sprintf ( Tools::displayError ( 'Cannot write to the theme\'s language file (%s). Please check writing permissions.' ), $file_name ) );
			}
			...
			case 'back' :
				$directories = array (
						'php' => array (
								...
								_PS_CLASS_DIR_ . 'controller/' => array (
										'AdminController.php' 
								),
								_PS_CLASS_DIR_ => array (
										'PaymentModule.php' 
								) 
						),
	 ...
	 * @param
	 *        	$content
	 * @param $type_translation :
	 *        	front, back, errors, modules...
	 * @param string|bool $type_file
	 *        	: (tpl|php)
	 * @param string $module_name
	 *        	: name of the module
	 * @return array
	...
	public function getTranslationsInformations() {
		...
		if (defined ( '_PS_THEME_SELECTED_DIR_' )) {
			$this->translations_informations ['modules'] ['override'] = array (
					'dir' => _PS_THEME_SELECTED_DIR_ . 'modules/',
					'file' => '' 
			);
			$this->translations_informations ['pdf'] ['override'] = array (
					'dir' => _PS_THEME_SELECTED_DIR_ . 'pdf/lang/',
					'file' => $this->lang_selected->iso_code . '.php' 
			);
			$this->translations_informations ['mails'] ['override'] = array (
...
<body>
	...
	protected function submitTranslationsMails() {
		...
		if (($subjects = Tools::getValue ( 'subject' )) && is_array ( $subjects )) {
			$array_subjects ['core_and_modules'] = array (
					'translations' => array (),
					'path' => $arr_mail_path ['core_mail'] . 'lang.php' 
			);
	...
	public function initFormFields() {
		...
		foreach ( $files_by_directory ['php'] as $dir => $files ) {
			foreach ( $files as $file ) {
				...
				$class_array [$class_name] = call_user_func ( array (
						$class_name,
						'getValidationRules' 
				), $class_name );
			}
	...
	public function getMailFiles($dir, $group_name = 'mail') {
		...
		if (Tools::file_exists_cache ( $dir_en )) {
			...
			$this->warnings [] = sprintf ( Tools::displayError ( 'A mail directory exists for the "%1$s" language, but not for the default language (%3$s) in %2$s' ), $this->lang_selected->iso_code, str_replace ( _PS_ROOT_DIR_, '', dirname ( $dir ) ), $default_language );
		}
		return $arr_return;
	}
	 ...
	 * @param string|bool $name_for_module
	 *        	Is not false define add a name for distinguish mails module
	 *        	
	 * @return string
	 */
			...
			foreach ( $mails ['files'] as $mail_name => $mail_files ) {
				...
				if (array_key_exists ( 'html', $mail_files ) || array_key_exists ( 'txt', $mail_files )) {
					if (array_key_exists ( $mail_name, $all_subject_mail )) {
						...
						$str_return .= '
                            <hr><div class="alert alert-info">' . sprintf ( $this->l ( 'No Subject was found for %s in the database.' ), $mail_name ) . '</div>';
					}
					// tab menu
	 ...
	 * @param string $lang
	 *        	ISO code of chosen language to translate
	 * @param bool $is_default
	 *        	Set it if modules are located in root/prestashop/modules folder
	 ...
	 *        	Contains expression in the chosen language
	 * @param string $tab
