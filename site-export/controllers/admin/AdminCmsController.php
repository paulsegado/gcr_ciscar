...
//<!-- CISCAR FIN affectations groupes a CMS -->
		...
		$this->tpl_form_vars = array (
				...
				( int ) Configuration::get ( 'PS_ALLOW_ACCENTED_CHARS_URL' ) 
		);

//<!-- CISCAR affectations groupes a CMS -->
		// Added values of object Group
		$cms = $this->loadObject ( true );

		if (! Validate::isUnsignedId ( $cms->id )) {
			$cms_groups = array ();
		} else {
			$cms_groups = $cms->getGroups ();
		}

		$cms_groups_ids = array ();
		if (is_array ( $cms_groups )) {
			foreach ( $cms_groups as $cms_group ) {
				$cms_groups_ids [] = $cms_group;
			}
		}
		
		
		foreach ( $groups as $group ) {
			$this->fields_value ['groupBox_' . $group ['id_group']] = Tools::getValue ( 'groupBox_' . $group ['id_group'], in_array ( $group ['id_group'], $cms_groups_ids ) );
		}
//<!-- CISCAR FIN affectations groupes a CMS -->		
