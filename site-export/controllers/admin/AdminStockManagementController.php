	...
	public function getList($id_lang, $order_by = null, $order_way = null, $start = 0, $limit = null, $id_lang_shop = false) {
		...
		for($i = 0; $i < $nb_items; $i ++) {
			...
			if (Tools::isSubmit ( 'id_product' )) {
				...
				$this->addRowActionSkipList ( 'details', array (
						$item ['id'] 
				) );
				...
				if (($item ['physical_quantity'] <= 0 && $item ['usable_quantity'] <= 0) || (empty ( $item ['physical_quantity'] ) && empty ( $item ['usable_quantity'] ))) {
					$this->addRowActionSkipList ( 'prepareRemovestock', array (
							$item ['id'] 
					) );
					$this->addRowActionSkipList ( 'prepareTransferstock', array (
							$item ['id'] 
					) );
				}
			...
			elseif (array_key_exists ( 'variations', $item ) && ( int ) $item ['variations'] > 0) {
				...
				$this->addRowActionSkipList ( 'addstock', array (
						$item ['id'] 
				) );
				$this->addRowActionSkipList ( 'prepareRemovestock', array (
						$item ['id'] 
			...
			if (! ($this->tabAccess ['delete'] === '1')) {
				$this->addRowActionSkipList ( 'removestock', array (
						$item ['id'] 
				) );
			}
			if (! ($this->tabAccess ['edit'] === '1')) {
				$this->addRowActionSkipList ( 'transferstock', array (
						$item ['id'] 
				) );
			}
		}
		...
		$this->fields_form [] ['form'] = array (
				...
				'input' => array (
						...
						array (
								...
								'required' => true,
								'options' => array (
										'query' => StockMvtReason::getStockMvtReasonsWithFilter ( $this->context->language->id, array (
	...
	public function postProcess() {
		...
		if (Tools::isSubmit ( 'removestock' ) && Tools::isSubmit ( 'is_post' )) {
			...
			if (count ( $this->errors ) == 0) {
				...
				// remove stock
				$stock_manager = StockManagerFactory::getManager ();
				$removed_products = $stock_manager->removeProduct ( $stockAttributes ['product_id'], $stockAttributes ['product_attribute_id'], $stockAttributes ['warehouse'], $quantity, $id_stock_mvt_reason, $usable, $id_order = null, $ignore_pack = 0, $employee = null, $stockAttributes ['stock'] );
				...
				if (count ( $removed_products ) > 0) {
					...
					if ($testedQuantity < $quantity) {
						$this->errors [] = sprintf ( $errorMessage, ( int ) $quantity, ( int ) $testedQuantity );
					}
				}
			}
		}
		...
		if (Tools::isSubmit ( 'transferstock' ) && Tools::isSubmit ( 'is_post' )) {
			...
			if (count ( $this->errors ) == 0) {
				...
				$is_transfer = $stock_manager->transferBetweenWarehouses ( $stockAttributes ['product_id'], $stockAttributes ['product_attribute_id'], $quantity, $stockAttributes ['warehouse_id'], $id_warehouse_to, $usable_from, $usable_to );
				StockAvailable::synchronize ( $stockAttributes ['product_id'] );
				if ($is_transfer) {
