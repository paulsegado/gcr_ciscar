...
class AdminCategoriesControllerCore extends AdminController {
	...
	public function renderForm() {
		...
		$selected_categories = array (
				(isset ( $obj->id_parent ) && $obj->isParentCategoryAvailable ( $id_shop )) ? ( int ) $obj->id_parent : ( int ) Tools::getValue ( 'id_parent', Category::getRootCategory ()->id ) 
		);
				...
				'input' => array (
						...
						array (
								...
								'tree' => array (
										...
										'disabled_categories' => (! Tools::isSubmit ( 'add' . $this->table ) && ! Tools::isSubmit ( 'submitAdd' . $this->table )) ? array (
												$this->_category->id 
										) : null,
	...
	public function postProcess() {
		if (! in_array ( $this->display, array (
				'edit',
				'add' 
		) )) {
	...
	public function processForceDeleteThumb() {
		...
		if (Validate::isLoadedObject ( $category )) {
			if (file_exists ( _PS_TMP_IMG_DIR_ . $this->table . '_' . $category->id . '-thumb.' . $this->imageType ) && ! unlink ( _PS_TMP_IMG_DIR_ . $this->table . '_' . $category->id . '-thumb.' . $this->imageType )) {
				return false;
			}
			if (file_exists ( _PS_CAT_IMG_DIR_ . $category->id . '_thumb.' . $this->imageType ) && ! unlink ( _PS_CAT_IMG_DIR_ . $category->id . '_thumb.' . $this->imageType )) {
				return false;
			}
			...
			$formated_medium = ImageType::getFormatedName ( 'medium' );
			foreach ( $images_types as $k => $image_type ) {
				if ($formated_medium == $image_type ['name'] && file_exists ( _PS_CAT_IMG_DIR_ . $category->id . '-' . $image_type ['name'] . '.' . $this->imageType ) && ! unlink ( _PS_CAT_IMG_DIR_ . $category->id . '-' . $image_type ['name'] . '.' . $this->imageType )) {
					return false;
				}
			}
		}
	...
	public function ajaxProcessStatusCategory() {
		if (! $id_category = ( int ) Tools::getValue ( 'id_category' )) {
			...
			if (Validate::isLoadedObject ( $category )) {
				...
				$category->save () ? die ( Tools::jsonEncode ( array (
						'success' => true,
