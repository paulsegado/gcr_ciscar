...
class AdminSearchControllerCore extends AdminController {
	 ...
	 *
	 * @param
	 *        	s string $query String to find in the catalog
	 */
	...
	public function searchFeatures() {
		...
		WHERE active = 1
		' . ($this->context->employee->id_profile != 1 ? 'AND view = 1' : '') . (defined ( '_PS_HOST_MODE_' ) ? ' AND t.`hide_host_mode` = 0' : '') );
		...
		foreach ( $_LANGADM as $key => $value ) {
			if (stripos ( $value, $this->query ) !== false) {
				...
				$this->_list ['features'] [$tabs [$key]] [] = array (
						'link' => Context::getContext ()->link->getAdminLink ( $key_match [$key] ),
