...
class AdminThemesControllerCore extends AdminController {
	...
	/**
	 * This value is used in isThemeCompatible method.
	 * only version node with an
	...
	public function init() {
				...
				'appearance' => array (
						...
						'buttons' => array (
								'storeLink' => array (
										...
										'href' => 'http://addons.prestashop.com/en/3-templates-prestashop' . '?utm_source=back-office&utm_medium=theme-button' . '&utm_campaign=back-office-' . $iso_lang_uc . '&utm_content=' . (defined ( '_PS_HOST_MODE_' ) ? 'cloud' : 'download'),
		...
		$installed_theme = Theme::getAllThemes ( array (
				$this->context->shop->id_theme 
		) );
		...
		if ($this->object) {
			if (( int ) $this->object->id > 0) {
				...
				$theme_metas = Db::getInstance ()->executeS ( 'SELECT ml.`title`, m.`page`, tm.`left_column` as `left`, tm.`right_column` as `right`, m.`id_meta`, tm.`id_theme_meta`
					...
					LEFT JOIN ' . _DB_PREFIX_ . 'meta_lang ml ON(ml.id_meta = m.id_meta AND ml.id_lang = ' . ( int ) $this->context->language->id . (( int ) Context::getContext ()->shop->id ? ' AND id_shop = ' . ( int ) Context::getContext ()->shop->id : '') . ')
	 ...
	 *
	 * @param string $base_theme_dir
	 *        	relative path to base dir
	 * @param string $target_theme_dir
	...
	public function downloadAddonsThemes() {
		if (! $this->logged_on_addons || ! in_array ( $this->authorizationLevel (), array (
	...
	public function processUpdate() {
		if (! in_array ( $this->authorizationLevel (), array (
	...
	private function checkVersionsAndCompatibility() {
		...
		if (! preg_match ( '#^[0-9][.][0-9]$#', Tools::getValue ( 'theme_version' ) ) || ! preg_match ( $exp, Tools::getValue ( 'compa_from' ) ) || ! preg_match ( $exp, Tools::getValue ( 'compa_to' ) ) || version_compare ( Tools::getValue ( 'compa_from' ), Tools::getValue ( 'compa_to' ) ) == 1) {
	...
	private function generateArchive() {
		...
		if ($zip->open ( _PS_CACHE_DIR_ . $zip_file_name, ZipArchive::OVERWRITE | ZipArchive::CREATE ) === true) {
			...
			if (isset ( $_FILES ['documentation'] )) {
				if (! empty ( $_FILES ['documentation'] ['tmp_name'] ) && ! empty ( $_FILES ['documentation'] ['name'] ) && ! $zip->addFile ( $_FILES ['documentation'] ['tmp_name'], 'doc/' . $_FILES ['documentation'] ['name'] )) {
	...
	protected function importThemeXmlConfig(SimpleXMLElement $xml, $theme_dir = false) {
		...
		if ($this->isThemeInstalled ( $th_name )) {
			return array (
					sprintf ( $this->l ( 'Theme %s already installed.' ), $th_name ) 
			);
		}
	 ...
	 * and noticed it on the $theme_dir/config.xml file.
	 * If not, some new functionnalities has
	 * to be desactivated
	 *
	 * @since 1.4
	 *       
	 ...
	 *
	 * @param array $arrFeatures
	 *        	array of feature code to check
	...
	public function postProcess() {
		if (Tools::isSubmit ( 'submitOptionstheme' ) && Tools::isSubmit ( 'id_theme' ) && ! Tools::isSubmit ( 'deletetheme' ) && Tools::getValue ( 'action' ) != 'ThemeInstall' && $this->context->shop->id_theme != Tools::getValue ( 'id_theme' )) {
			...
			if (Tools::getValue ( 'updatetheme' ) && (! in_array ( $this->authorizationLevel (), array (
					AdminController::LEVEL_EDIT,
					AdminController::LEVEL_ADD,
					AdminController::LEVEL_DELETE 
			) ) || _PS_MODE_DEMO_)) {
	...
	public function ajaxProcessLeftMeta() {
		$theme_meta = Db::getInstance ()->getRow ( 'SELECT * FROM ' . _DB_PREFIX_ . 'theme_meta WHERE id_theme_meta = ' . ( int ) Tools::getValue ( 'id_theme_meta' ) );
