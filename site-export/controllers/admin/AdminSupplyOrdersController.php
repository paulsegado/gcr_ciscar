...
class AdminSupplyOrdersControllerCore extends AdminController {
	...
	public function __construct() {
		...
		array_unshift ( $this->warehouses, array (
				'id_warehouse' => - 1,
				'name' => $this->l ( 'All Warehouses' ) 
		) );
	...
	public function renderList() {
		...
		// unsets some fields
		unset ( $this->fields_list ['state'], $this->fields_list ['date_upd'], $this->fields_list ['id_pdf'], $this->fields_list ['date_delivery_expected'], $this->fields_list ['id_export'] );
			...
			if (Validate::isLoadedObject ( $supply_order )) {
				...
				if (($new_supplier_id != $supply_order->id_supplier) || ($new_currency_id != $supply_order->id_currency)) {
					...
					foreach ( $product_ids as $id ) {
						...
						$entry->unit_price_te = ( float ) str_replace ( array (
								' ',
								',' 
						), array (
								'',
						...
						$entry->quantity_expected = ( int ) str_replace ( array (
								' ',
								',' 
						), array (
								'',
								'.' 
						), Tools::getValue ( 'input_quantity_expected_' . $id, 0 ) );
						$entry->discount_rate = ( float ) str_replace ( array (
								' ',
								',' 
						), array (
								'',
								'.' 
						), Tools::getValue ( 'input_discount_rate_' . $id, 0 ) );
						$entry->tax_rate = ( float ) str_replace ( array (
								' ',
								',' 
						), array (
								'',
								'.' 
						), Tools::getValue ( 'input_tax_rate_' . $id, 0 ) );
	...
	public function postProcess() {
		...
		if (Tools::isSubmit ( 'submitAddsupply_order' ) || Tools::isSubmit ( 'submitAddsupply_orderAndStay' )) {
			...
			if (! count ( $this->errors )) {
				...
				$_POST ['discount_rate'] = ( float ) str_replace ( array (
						' ',
						',' 
				), array (
						'',
								...
								if ($supply_order->save ()) {
									...
									if ($new_state->receipt_state && $new_state->enclosed) {
										...
										foreach ( $supply_order_details as $detail ) {
											...
											if ($stock_manager->addProduct ( $detail ['id_product'], $detail ['id_product_attribute'], $warehouse, ( int ) ($detail ['quantity_expected'] - $detail ['quantity_received']), Configuration::get ( 'PS_STOCK_MVT_SUPPLY_ORDER' ), $detail ['unit_price_te'], true, $supply_order->id )) {
									...
									if ((($old_state->receipt_state || $old_state->pending_receipt) && $new_state->enclosed && ! $new_state->receipt_state) || ($new_state->receipt_state && $new_state->enclosed)) {
										$supply_order_details = $supply_order->getEntries ();
										$products_done = array ();
										foreach ( $supply_order_details as $supply_order_detail ) {
	...
	protected function renderCSV() {
		...
		elseif (Tools::isSubmit ( 'csv_orders_details' )) {
			...
			$keys = array (
					...
					'sod.reference',
					'sod.supplier_reference',
					'sod.ean13',
					'sod.upc',
					'sod.name',
					'FORMAT(sod.unit_price_te, 2)',
					'sod.quantity_expected',
					'sod.quantity_received',
					'FORMAT(sod.price_te, 2)',
					'FORMAT(sod.discount_rate, 2)',
					'FORMAT(sod.discount_value_te, 2)',
					'FORMAT(sod.price_with_discount_te, 2)',
					'FORMAT(sod.tax_rate, 2)',
					'FORMAT(sod.tax_value, 2)',
					'FORMAT(sod.price_ti, 2)',
					'FORMAT(sod.tax_value_with_order_discount, 2)',
					'FORMAT(sod.price_with_order_discount_te, 2)',
					'sod.id_supply_order' 
			);
			foreach ( $ids as $id ) {
				$query = new DbQuery ();
