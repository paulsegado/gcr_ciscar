...
class AdminLanguagesControllerCore extends AdminController {
				...
				'input' => array (
                    ...
                    /*'desc' => sprintf($this->l('Full date format (e.g., %s)'), '<a href="http://php.net/date" target="_blank">Y-m-d H:i:s</a>')*/
						...
						array (
								...
								'hint' => array (
										$this->l ( 'Enable if this language is read from right to left.' ) . ' ' . $this->l ( '(Experimental: your theme must be compliant with RTL languages).' ) 
								) 
						),
						array (
								'type' => 'switch',
	 ...
	 *        	Language iso_code for no_picture image filename
	 *        	
	 * @return void|false
	 */
	...
	protected function afterImageUpload() {
		...
		if (($id_lang = ( int ) Tools::getValue ( 'id_lang' )) && isset ( $_FILES ) && count ( $_FILES ) && file_exists ( _PS_LANG_IMG_DIR_ . $id_lang . '.jpg' )) {
