...
class AdminModulesControllerCore extends AdminController {
	...
	public function __construct() {
		...
		uasort ( $this->list_modules_categories, array (
				$this,
	...
	public function ajaxProcessLogOnAddonsWebservices() {
		$content = Tools::addonsRequest ( 'check_customer', array (
				'username_addons' => pSQL ( trim ( Tools::getValue ( 'username_addons' ) ) ),
				'password_addons' => pSQL ( trim ( Tools::getValue ( 'password_addons' ) ) ) 
		) );
	...
	public function ajaxProcessSaveFavoritePreferences() {
		...
		if ($id_module_preference > 0) {
			if ($action == 'i') {
				$update = array (
						'interest' => ($value == '' ? null : ( int ) $value) 
				);
			}
			if ($action == 'f') {
				$update = array (
						'favorite' => ($value == '' ? null : ( int ) $value) 
				);
			}
	...
	public function postProcessDownload() {
		...
		if ($this->tabAccess ['add'] === '1') {
			...
			} elseif (substr ( $_FILES ['file'] ['name'], - 4 ) != '.tar' && substr ( $_FILES ['file'] ['name'], - 4 ) != '.zip' && substr ( $_FILES ['file'] ['name'], - 4 ) != '.tgz' && substr ( $_FILES ['file'] ['name'], - 7 ) != '.tar.gz') {
		...
		if ($this->tabAccess ['delete'] === '1') {
			if (Tools::getValue ( 'module_name' ) != '') {
				...
				if (Validate::isLoadedObject ( $module ) && ! $module->getPermission ( 'configure' )) {
					...
					$moduleDir = _PS_MODULE_DIR_ . str_replace ( array (
							'.',
							'/',
							'\\' 
					), array (
				...
				foreach ( $modules as $name ) {
					...
					if (! file_exists ( _PS_MODULE_DIR_ . $name . '/' . $name . '.php' ) || $key == 'update' || $key == 'updateAll') {
						$files_list = array (
								array (
										'type' => 'addonsNative',
										'file' => Module::CACHE_FILE_DEFAULT_COUNTRY_MODULES_LIST,
										'loggedOnAddons' => 0 
								),
								array (
										'type' => 'addonsBought',
										'file' => Module::CACHE_FILE_CUSTOMER_MODULES_LIST,
										'loggedOnAddons' => 1 
								),
								array (
										'type' => 'addonsMustHave',
										'file' => Module::CACHE_FILE_MUST_HAVE_MODULES_LIST,
										'loggedOnAddons' => 1 
								) 
						);
						...
						foreach ( $module_to_update as $name => $attr ) {
							if ((is_null ( $attr ) && $this->logged_on_addons == 0) || ($attr ['need_loggedOnAddons'] == 1 && $this->logged_on_addons == 0)) {
								...
								$download_ok = false;
								if ($attr ['need_loggedOnAddons'] == 0 && file_put_contents ( _PS_MODULE_DIR_ . $name . '.zip', Tools::addonsRequest ( 'module', array (
										'id_module' => pSQL ( $attr ['id'] ) 
								) ) )) {
									$download_ok = true;
								} elseif ($attr ['need_loggedOnAddons'] == 1 && $this->logged_on_addons && file_put_contents ( _PS_MODULE_DIR_ . $name . '.zip', Tools::addonsRequest ( 'module', array (
										'id_module' => pSQL ( $attr ['id'] ),
							...
							if (isset ( $this->_modules_ad [$module->name] )) {
								...
								foreach ( $ad_modules ['not_installed'] as &$module ) {
									if (isset ( $module->addons_buy_url )) {
										$module->addons_buy_url = str_replace ( 'utm_source=v1trunk_api', 'utm_source=back-office', $module->addons_buy_url ) . '&utm_medium=related-modules&utm_campaign=back-office-' . strtoupper ( $this->context->language->iso_code ) . '&utm_content=' . (($this->context->mode >= Context::MODE_HOST_CONTRIB) ? 'cloud' : 'download');
									}
	...
	public function isModuleFiltered($module) {
		...
		if ($show_type_modules == 'nativeModules' && ! in_array ( $module->name, $this->list_natives_modules )) {
			...
			$author_selected = substr ( str_replace ( array (
					'authorModules[',
					"\'" 
			), array (
					'',
					"'" 
			), $show_type_modules ), 0, - 1 );
		...
		if ($show_country_modules && (isset ( $module->limited_countries ) && ! empty ( $module->limited_countries ) && ((is_array ( $module->limited_countries ) && count ( $module->limited_countries ) && ! in_array ( strtolower ( $this->iso_default_country ), $module->limited_countries )) || (! is_array ( $module->limited_countries ) && strtolower ( $this->iso_default_country ) != strval ( $module->limited_countries ))))) {
			return true;
		}
