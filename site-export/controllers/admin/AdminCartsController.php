...
class AdminCartsControllerCore extends AdminController {
	...
	public function ajaxProcessUpdateQty() {
		if ($this->tabAccess ['edit'] === '1') {
			...
			echo Tools::jsonEncode ( array_merge ( $this->ajaxReturnVars (), array (
					'errors' => $errors 
			) ) );
		}
	}
		...
		foreach ( current ( $delivery_option_list ) as $key => $delivery_option ) {
			...
			foreach ( $delivery_option ['carrier_list'] as $carrier ) {
				...
				if (! $this->context->cart->id_carrier) {
					$this->context->cart->setDeliveryOption ( array (
							$this->context->cart->id_address_delivery => ( int ) $carrier ['instance']->id . ',' 
					) );
					$this->context->cart->save ();
				}
			}
	...
	public function displayAjaxSearchCarts() {
		...
		if ($orders || $carts) {
			$to_return = array_merge ( $this->ajaxReturnVars (), array (
					'carts' => $carts,
					'orders' => $orders,
					'found' => true 
			) );
