	...
	public function __construct() {
		...
		$this->fields_list = array_merge ( $this->fields_list, array (
				...
				'active' => array (
						...
						'align' => 'text-center',
						'active' => 'status',
//<!-- CISCAR on remplace 'type' => 'bool', par 'callback' => 'printSatusIcon' pour envoi mail suite � changement de statut
//						'type' => 'bool',
		...
		$this->fields_form = array (
				...
				'input' => array (
						...
						array (
								...
								'col' => '4',
								'hint' => ($obj->id ? $this->l ( 'Leave this field blank if there\'s no change.' ) : sprintf ( $this->l ( 'Password should be at least %s characters long.' ), Validate::PASSWORD_LENGTH )) 
						),
//<!-- CISCAR pour conserver le mot de passe en clair -->
						array (
								'type' => 'text',
...
//<!-- CISCAR la societe est obligatoire -->				
			...
			$this->fields_form ['input'] [] = array (
					'type' => 'text',
//<!-- CISCAR le SIRET est remplace par la TVA Intra communautaire -->				
					'label' => $this->l ( 'TVA intra communautaire' ),
					'name' => 'siret' 
			);
			$this->fields_form ['input'] [] = array (
					'type' => 'text',
//<!-- CISCAR le code APE est remplace par la Fonction -->				
					'label' => $this->l ( 'Fonction' ),
					'name' => 'ape' 
			);
			$this->fields_form ['input'] [] = array (
					'type' => 'text',
//<!-- CISCAR le site Web est remplace par Marques distribuees  -->				
	...
	public function printNewsIcon($value, $customer) {
		return '<a class="list-action-enable ' . ($value ? 'action-enabled' : 'action-disabled') . '" href="index.php?' . htmlspecialchars ( 'tab=AdminCustomers&id_customer=' . ( int ) $customer ['id_customer'] . '&changeNewsletterVal&token=' . Tools::getAdminTokenLite ( 'AdminCustomers' ) ) . '">
				' . ($value ? '<i class="icon-check"></i>' : '<i class="icon-remove"></i>') . '</a>';
	}
	public function printOptinIcon($value, $customer) {
		return '<a class="list-action-enable ' . ($value ? 'action-enabled' : 'action-disabled') . '" href="index.php?' . htmlspecialchars ( 'tab=AdminCustomers&id_customer=' . ( int ) $customer ['id_customer'] . '&changeOptinVal&token=' . Tools::getAdminTokenLite ( 'AdminCustomers' ) ) . '">
				' . ($value ? '<i class="icon-check"></i>' : '<i class="icon-remove"></i>') . '</a>';
	}
