...
class AdminCustomerThreadsControllerCore extends AdminController {
			...
			if (Tools::isSubmit ( 'submitReply' )) {
				...
				if (($error = $cm->validateField ( 'message', $cm->message, null, array (), true )) !== true) {
					...
					$params = array (
							...
							'{link}' => Tools::url ( $this->context->link->getPageLink ( 'contact', true, null, null, false, $ct->id_shop ), 'id_customer_thread=' . ( int ) $ct->id . '&token=' . $ct->token ),
					...
					if (Mail::Send ( ( int ) $ct->id_lang, 'reply_msg', sprintf ( Mail::l ( 'An answer to your message is available #ct%1$s #tc%2$s', $ct->id_lang ), $ct->id, $ct->token ), $params, Tools::getValue ( 'msg_email' ), null, $from_email, $from_name, $file_attachment, null, _PS_MAIL_DIR_, true, $ct->id_shop )) {
						$ct->status = 'closed';
						$ct->update ();
					}
	...
	protected function displayMessage($message, $email = false, $id_employee = null) {
		...
		$contacts = Contact::getContacts ( $this->context->language->id );
		foreach ( $contacts as $contact ) {
			$contact_array [$contact ['id_contact']] = array (
	...
	public function syncImap() {
		...
		if (! $mbox) {
			return array (
					'hasError' => true,
					'errors' => array (
							'Cannot connect to the mailbox :<br />' . ($str_errors) 
					) 
			);
		}
		...
		if (! $check) {
			return array (
					'hasError' => true,
					'errors' => array (
							'Fail to get information about the current mailbox' 
					) 
			);
		}
