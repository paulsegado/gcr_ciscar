...
class AdminStoresControllerCore extends AdminController {
	...
	public function postProcess() {
		if (isset ( $_POST ['submitAdd' . $this->table] )) {
			...
			foreach ( $_POST as $kp => $vp ) {
				if (! in_array ( $kp, array (
						'checkBoxShopGroupAsso_store',
						'checkBoxShopAsso_store' 
				) ) && ! is_array ( $_POST [$kp] )) {
					$_POST [$kp] = trim ( $vp );
				}
			}
	...
	protected function _getDefaultFieldsContent() {
		...
		$countryList [] = array (
				'id' => '0',
				'name' => $this->l ( 'Choose your country' ) 
		);
		...
		$stateList [] = array (
				'id' => '0',
				'name' => $this->l ( 'Choose your state (if applicable)' ) 
		);
