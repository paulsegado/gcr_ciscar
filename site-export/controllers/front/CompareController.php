...
class CompareControllerCore extends FrontController {
	...
	public function initContent() {
		...
		if (($product_list = urldecode ( Tools::getValue ( 'compare_product_list' ) )) && ($postProducts = (isset ( $product_list ) ? rtrim ( $product_list, '|' ) : ''))) {
			...
			if (count ( $ids )) {
				Tools::redirect ( $this->context->link->getPageLink ( 'products-comparison', null, $this->context->language->id, array (
						'compare_product_list' => implode ( '|', $ids ) 
				) ) );
			}
		}
		...
		if ($ids) {
			if (count ( $ids ) > 0) {
				...
				foreach ( $ids as $k => &$id ) {
					...
					$curProduct->id_image = Tools::htmlentitiesUTF8 ( Product::defineProductImage ( array (
				...
				if (count ( $listProducts ) > 0) {
					...
					$this->context->smarty->assign ( array (
							...
							'width' => $width,
							'HOOK_COMPARE_EXTRA_INFORMATION' => Hook::exec ( 'displayCompareExtraInformation', array (
									'list_ids_product' => $ids 
							) ),
							'HOOK_EXTRA_PRODUCT_COMPARISON' => Hook::exec ( 'displayProductComparison', array (
									'list_ids_product' => $ids 
							) ),
