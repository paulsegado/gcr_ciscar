			...
			if (! count ( $this->errors )) {
				...
				if (Validate::isLoadedObject ( $order ) && $order->id_customer == $this->context->customer->id) {
					...
					if (Validate::isLoadedObject ( $customer )) {
						Mail::Send ( $this->context->language->id, 'order_customer_comment', Mail::l ( 'Message from a customer' ), array (
...
//<!-- CISCAR on ajoute la reference de la commande								
						...
						), $to, $toName, strval ( Configuration::get ( 'PS_SHOP_EMAIL' ) ), $customer->firstname . ' ' . $customer->lastname, null, null, _PS_MAIL_DIR_, false, null, null, $customer->email );
					}
	...
	public function initContent() {
		...
		if (! ($id_order = ( int ) Tools::getValue ( 'id_order' )) || ! Validate::isUnsignedId ( $id_order )) {
			...
			if (Validate::isLoadedObject ( $order ) && $order->id_customer == $this->context->customer->id) {
				...
				$this->context->smarty->assign ( 'HOOK_ORDERDETAILDISPLAYED', Hook::exec ( 'displayOrderDetail', array (
						'order' => $order 
				) ) );
				Hook::exec ( 'actionOrderDetail', array (
						'carrier' => $carrier,
						'order' => $order 
				) );
