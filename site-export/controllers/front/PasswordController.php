...
//<!-- CISCAR on ajoute le mot de passe en clair							
							'{pwdclear}' => $customer->pwdclear,
			...
			if ($email) {
				...
				if (! Validate::isLoadedObject ( $customer )) {
					...
					if ($customer->update ()) {
						Hook::exec ( 'actionPasswordRenew', array (
								'customer' => $customer,
								'password' => $password 
						) );
						$mail_params = array (
...
//<!-- CISCAR On supprime l'envoi du mail qui indique que le demande de nenouvellement du mot de passe a bien étét prise en compte						
