...
class ParentOrderControllerCore extends FrontController {
	...
	public function setMedia() {
		...
		if (in_array ( ( int ) Tools::getValue ( 'step' ), array (
				0,
				2,
				3 
	...
	protected function _assignSummaryInformations() {
		...
		foreach ( $summary ['products'] as $key => &$product ) {
			...
			$product ['price_without_specific_price'] = Product::getPriceStatic ( $product ['id_product'], ! Product::getTaxCalculationMethod (), $product ['id_product_attribute'], 6, null, false, false, 1, false, null, null, null, $null, true, true, $cart_product_context );
		...
		if (Validate::isLoadedObject ( $customer )) {
			...
			if (! count ( $customerAddresses ) && ! Tools::isSubmit ( 'ajax' )) {
				...
				if (($bad_delivery = ( bool ) ! Address::isCountryActiveById ( ( int ) $this->context->cart->id_address_delivery )) || ! Address::isCountryActiveById ( ( int ) $this->context->cart->id_address_invoice )) {
					...
					$params = array (
							'back' => $back_url,
							'id_address' => ($bad_delivery ? ( int ) $this->context->cart->id_address_delivery : ( int ) $this->context->cart->id_address_invoice) 
					);
					if ($multi) {
						$params ['multi-shipping'] = $multi;
					}
