...
// <!-- CISCAR si produit passe en parametre
		if (isset($_GET['id_product']))
			$this->context->smarty->assign ( 'id_product',$_GET['id_product']);
		else
			$this->context->smarty->assign ( 'id_product','');

// <!-- CISCAR si catégorie passe en parametre	
		if (isset($_GET['id_category']))
			$this->context->smarty->assign ( 'id_category',$_GET['id_category']);
		else
			$this->context->smarty->assign ( 'id_category','');
		
// <!-- CISCAR si cms passe en parametre		
		if (isset($_GET['id_cms']))
			$this->context->smarty->assign ( 'id_cms',$_GET['id_cms']);
		else
			$this->context->smarty->assign ( 'id_cms','');

// <!-- CISCAR si cms_category passe en parametre
		if (isset($_GET['id_cms_category']))
			$this->context->smarty->assign ( 'id_cms_category',$_GET['id_cms_category']);
		else
			$this->context->smarty->assign ( 'id_cms_category','');
		
// <!-- CISCAR si langue passee en parametre
		if (isset($_GET['id_lang']))
			$this->context->cookie->id_lang = $_GET['id_lang'];
		
		// <!-- CISCAR on verifie si auto Login passee en parametre
		$param = array();
		$param = explode("&", base64_decode(Tools::getValue('autologin')));
		if(count($param) == 2)
		{
			$this->context->smarty->assign ( 'login_mail', $param[0]);
			$this->context->smarty->assign ( 'login_pwd', $param[1]);
			$this->context->smarty->assign ( 'autologin','O');
		}
		else 
		{
			$this->context->smarty->assign ( 'login_mail','');
			$this->context->smarty->assign ( 'login_pwd', '');				
			$this->context->smarty->assign ( 'autologin','N');
		}
...
//<!-- CISCAR initialisation du Cookie utiliser pour le retour sur siye appelant -->	
		if (Tools::isSubmit ( 'SubmitLogin' )) {
			setcookie("from_url", '',0);
			$this->processSubmitLogin ();
		}
		
//<!-- CISCAR soumission du formulaire en cas d'autoLogin -->		
		if (Tools::isSubmit ( 'SubmitAutoLogin' )) {
			$this->processSubmitLogin ();
		}
	}
...
//<!-- CISCAR produit passé en parametre dans l'autologin
		$product =  trim ( Tools::getValue ( 'product' ) );
//<!-- CISCAR catégorie passé en parametre dans l'autologin
		$category =  trim ( Tools::getValue ( 'category' ) );
//<!-- CISCAR cms passé en parametre dans l'autologin
		$cms =  trim ( Tools::getValue ( 'cms' ) );
//<!-- CISCAR cms_category passé en parametre dans l'autologin
		$cms_category =  trim ( Tools::getValue ( 'cms_category' ) );
...
//<!-- CISCAR on redirige sur la bonne page en fonction des paramètres de l'url -->
				if ($product != '')
					Tools::redirect ( 'index.php?id_product='.$product.'&controller=product&id_lang='.$this->context->cookie->id_lang);
				else
					if ($category != '')
						Tools::redirect ( 'index.php?id_category='.$category.'&controller=category&id_lang='.$this->context->cookie->id_lang);
					else
						if ($cms != '')
							Tools::redirect ( 'index.php?id_cms='.$cms.'&controller=cms&id_lang='.$this->context->cookie->id_lang);
						else
							if ($cms_category != '')
								Tools::redirect ( 'index.php?id_cms_category='.$cms_category.'&controller=cms&id_lang='.$this->context->cookie->id_lang);
							else
								Tools::redirect ( 'index.php?id_lang='.$this->context->cookie->id_lang);
				} else {
...
//<!-- CISCAR Ajout du login utilisateur dans le contexte customer -->				
				$this->context->cookie->customer_login_ciscar = $customer->login_ciscar;
				...
				if (! $this->ajax) {
					...
					$back = Tools::getValue ( 'my-account' );
...
//<!-- CISCAR on redirige sur la bonne page en fonction des paramètres de l'url -->								
	 ...
	 *        	Reference on the customer Object.
	 *        	
	...
	protected function processCustomerNewsletter(&$customer) {
		...
		if ($blocknewsletter && $module_newsletter->active && ! Tools::getValue ( 'newsletter' )) {
			...
			) ) && $module_newsletter->isNewsletterRegistered ( Tools::getValue ( 'email' ) ) == Blocknewsletter::GUEST_REGISTERED) {
...
//<!-- CISCAR le nouveau client n'est pas actif par défaut -->				
				$customer->active = 0;
