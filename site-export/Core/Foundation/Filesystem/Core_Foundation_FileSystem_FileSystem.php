...
class Core_Foundation_FileSystem_FileSystem {
	...
	public function joinPaths() {
		if (func_num_args () < 2) {
			...
			return $this->joinPaths ( $arg_0, call_user_func_array ( array (
					$this,
					'joinPaths' 
	...
	public function listEntriesRecursively($path) {
		if (! file_exists ( $path )) {
			throw new Core_Foundation_FileSystem_Exception ( sprintf ( 'No such file or directory: %s', $path ) );
		}
		
		if (! is_dir ( $path )) {
			throw new Core_Foundation_FileSystem_Exception ( sprintf ( '%s is not a directory', $path ) );
		}
		...
		foreach ( scandir ( $path ) as $entry ) {
			...
			if ($info->isDir ()) {
				$entries = array_merge ( $entries, $this->listEntriesRecursively ( $newPath ) );
			}
		}
	...
	public function listFilesRecursively($path) {
		return array_filter ( $this->listEntriesRecursively ( $path ), array (
				$this,
				'matchOnlyFiles' 
		) );
	}
