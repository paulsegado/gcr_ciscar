...
class Core_Business_Stock_StockManager {
	 ...
	 *        	A product pack object to update its quantity
	 * @param StockAvailable $stock_available
	 *        	the stock of the product to fix with correct quantity
	 * @param integer $delta_quantity
	 *        	The movement of the stock (negative for a decrease)
	 * @param integer|null $id_shop
	 *        	Opional shop ID
	 */
	 ...
	 *
	 * @param Product $product
	 *        	A product object to update its quantity
	 * @param integer $id_product_attribute
	 *        	The product attribute to update
	 * @param StockAvailable $stock_available
	 *        	the stock of the product to fix with correct quantity
	 * @param integer|null $id_shop
	 *        	Opional shop ID
	...
	public function updatePacksQuantityContainingProduct($product, $id_product_attribute, $stock_available, $id_shop = null) {
		...
		foreach ( $packs as $pack ) {
			...
			if (! (( int ) $pack->pack_stock_type == 2) && ! (( int ) $pack->pack_stock_type == 3 && $configuration->get ( 'PS_PACK_STOCK_TYPE' ) == 2)) {
				continue;
			}
			...
			$max_pack_quantity = max ( array (
					0,
					floor ( $stock_available->quantity / $quantity_by_pack ) 
			) );
	...
	/**
	 * Will update Product available stock int he given declinaison.
	 * If product is a Pack, could decrease the sub products.
	 * If Product is contained in a Pack, Pack could be decreased or not (only if sub product stocks become not sufficient).
	 *
	 * @param Product $product
	 *        	The product to update its stockAvailable
	 * @param integer $id_product_attribute
	 *        	The declinaison to update (null if not)
	 * @param integer $delta_quantity
