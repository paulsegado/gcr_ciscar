...
class XML_Feed_Parser_RSS11 extends XML_Feed_Parser_Type {
	 ...
	 * to the constructs they are.
	 * The optional second parameter in the array
...
;
	...
	/**
	 * Here we map some elements to their atom equivalents.
	 * This is going to be
	 ...
	 * keep them together.
	 * We will retain support for some common RSS1.0 modules
	 * 
	 * @var array
	 */
	 ...
	 * @param
	 *        	bool (optional) $string Whether or not to validate this feed
	 */
	...
	protected function getTextInput() {
		...
		if ($inputs->length > 0) {
			$input = $inputs->item ( 0 );
			$results = array ();
			$results ['title'] = isset ( $input->getElementsByTagName ( 'title' )->item ( 0 )->value ) ? $input->getElementsByTagName ( 'title' )->item ( 0 )->value : null;
			$results ['description'] = isset ( $input->getElementsByTagName ( 'description' )->item ( 0 )->value ) ? $input->getElementsByTagName ( 'description' )->item ( 0 )->value : null;
			$results ['name'] = isset ( $input->getElementsByTagName ( 'name' )->item ( 0 )->value ) ? $input->getElementsByTagName ( 'name' )->item ( 0 )->value : null;
			$results ['link'] = isset ( $input->getElementsByTagName ( 'link' )->item ( 0 )->value ) ? $input->getElementsByTagName ( 'link' )->item ( 0 )->value : null;
			if (empty ( $results ['link'] ) and $input->attributes->getNamedItem ( 'resource' )) {
