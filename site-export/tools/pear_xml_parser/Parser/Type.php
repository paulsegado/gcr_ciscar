...
abstract class XML_Feed_Parser_Type {
	...
	function getEntryByOffset($offset) {
		if (! isset ( $this->entries [$offset] )) {
			...
			if ($entries->length > $offset) {
				...
				$this->entries [$offset] = new $this->itemClass ( $entries->item ( $offset ), $this, $xmlBase );
	 ...
	 * @param string $method
	 *        	The name of the date construct we want
	 * @param array $arguments
	 *        	Included for compatibility with our __call usage
	 ...
	 *
	 * @param string $method
	 *        	The name of the text construct we want
	 * @param array $arguments
	 *        	Included for compatibility with our __call usage
	 * @return string
	 */
	 ...
	 *
	 * @param string $call
	 *        	for compatibility with our overloading
	 * @param array $arguments
	 *        	- arg 0 is the offset, arg 1 is whether to return as array
	 ...
	 *
	 * @param string $type
	 *        	the element we want to get a count of
	 * @return int
	 */
	 ...
	 *        	The DOM node we are iterating over
	 * @return string
	 */
	 ...
	 *
	 * @param DOMElement $node
	 *        	The DOM node we are processing
	 * @return string
	 */
	function traverseNode($node) {
		...
		if ($node instanceof DOMElement) {
			$content .= '<' . $node->tagName . $this->processXHTMLAttributes ( $node ) . '>';
		}
	...
	/**
	 * Return an XML serialization of the feed, should it be required.
	 * Most
	...
	/**
	 * Get directory holding RNG schemas.
