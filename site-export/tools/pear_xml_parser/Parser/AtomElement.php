...
/**
 * This class provides support for atom entries.
 * It will usually be called by
...
class XML_Feed_Parser_AtomElement extends XML_Feed_Parser_Atom {
	 ...
	 * compatibilities (eg.
	 * with the Univeral Feed Parser). Key is the other version's
	 ...
	 *        	- this item as a DOM element
	 * @param XML_Feed_Parser_Atom $parent
	 *        	- the feed of which this is a member
	 */
	 ...
	 *
	 * @param string $method
	 *        	- for compatibility with our __call usage
	...
	function getEnclosure($method, $arguments = array()) {
		...
		$query = "//atom:entry[atom:id='" . $this->getText ( 'id', false ) . "']/atom:link[@rel='enclosure']";
		...
		if ($encs->length > $offset) {
			try {
				...
				$length = $encs->item ( $offset )->hasAttribute ( 'length' ) ? $encs->item ( $offset )->getAttribute ( 'length' ) : false;
				return array (
