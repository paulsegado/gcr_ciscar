 ...
 * feed types.
 * It makes sense to load it here to keep the other files
 * clean.
 */
...
/**
 * This is the core of the XML_Feed_Parser package.
 * It identifies feed types
...
class XML_Feed_Parser implements Iterator {
	 ...
	 * @param bool $strict
	 *        	Whether or not to validate the feed
	 * @param bool $suppressWarnings
	 *        	Trigger errors for deprecated feed types?
	 * @param bool $tidy
	 *        	Whether or not to try and use the tidy library on input
	 */
	function __construct($feed, $strict = false, $suppressWarnings = false, $tidy = false) {
		...
		switch (true) {
			...
			case ($doc_element->namespaceURI == 'http://purl.org/atom/ns#') :
				...
				require_once (_PS_PEAR_XML_PARSER_PATH_ . 'Parser/AtomElement.php');
				$class = 'XML_Feed_Parser_Atom';
				$error = 'Atom 0.3 deprecated, using 1.0 parser which won\'t provide ' . 'all options';
				break;
			case ($doc_element->namespaceURI == 'http://purl.org/rss/1.0/' || ($doc_element->hasChildNodes () && $doc_element->childNodes->length > 1 && $doc_element->childNodes->item ( 1 )->namespaceURI == 'http://purl.org/rss/1.0/')) :
				...
				break;
			case ($doc_element->namespaceURI == 'http://purl.org/rss/1.1/' || ($doc_element->hasChildNodes () && $doc_element->childNodes->length > 1 && $doc_element->childNodes->item ( 1 )->namespaceURI == 'http://purl.org/rss/1.1/')) :
				...
				break;
			case (($doc_element->hasChildNodes () && $doc_element->childNodes->length > 1 && $doc_element->childNodes->item ( 1 )->namespaceURI == 'http://my.netscape.com/rdf/simple/0.9/') || $doc_element->namespaceURI == 'http://my.netscape.com/rdf/simple/0.9/') :
				...
				break;
			case ($doc_element->tagName == 'rss' and $doc_element->hasAttribute ( 'version' ) && $doc_element->getAttribute ( 'version' ) == 0.91) :
				...
				break;
			case ($doc_element->tagName == 'rss' and $doc_element->hasAttribute ( 'version' ) && $doc_element->getAttribute ( 'version' ) == 0.92) :
				$error = 'RSS 0.92 has been superceded by RSS2.0. Using RSS2.0 parser.';
				require_once (_PS_PEAR_XML_PARSER_PATH_ . 'Parser/RSS2.php');
				require_once (_PS_PEAR_XML_PARSER_PATH_ . 'Parser/RSS2Element.php');
				$class = 'XML_Feed_Parser_RSS2';
				break;
			case (in_array ( $doc_element->namespaceURI, $this->feedNamespaces ['rss2'] ) || $doc_element->tagName == 'rss') :
	 ...
	 */
	function next() {
		if (isset ( $this->current_item ) && $this->current_item <= $this->feed->numberEntries - 1) {
			++ $this->current_item;
	 ...
	 *
	 * @param string $id
	 *        	Valid ID for the given feed format
	 ...
	 *
	 * @param int $offset
	 *        	The position of the entry within the feed, starting from 0
	 * @return XML_Feed_Parser_Type|false
	 */
