...
class HTMLPurifier_Printer_ConfigForm extends HTMLPurifier_Printer {
	 ...
	 * False to disable or integer letters
	 * before clipping.
	 * @type bool
	 */
	 ...
	 * 
	 * @param $cols Integer
	 *        	columns of textarea, null to use default
	 * @param $rows Integer
	 *        	rows of textarea, null to use default
	 */
	...
	public function render($config, $allowed = true, $render_controls = true) {
		...
		$ret = '';
		$ret .= $this->start ( 'table', array (
				'class' => 'hp-config' 
		) );
		$ret .= $this->start ( 'thead' );
		$ret .= $this->start ( 'tr' );
		$ret .= $this->element ( 'th', 'Directive', array (
				'class' => 'hp-directive' 
		) );
		$ret .= $this->element ( 'th', 'Value', array (
				'class' => 'hp-value' 
		) );
		$ret .= $this->end ( 'tr' );
		$ret .= $this->end ( 'thead' );
		...
		if ($render_controls) {
			...
			$ret .= $this->start ( 'tr' );
			$ret .= $this->start ( 'td', array (
					'colspan' => 2,
					'class' => 'controls' 
			) );
			$ret .= $this->elementEmpty ( 'input', array (
					'type' => 'submit',
					'value' => 'Submit' 
			) );
			$ret .= '[<a href="?">Reset</a>]';
	...
	protected function renderNamespace($ns, $directives) {
		$ret = '';
		$ret .= $this->start ( 'tbody', array (
				'class' => 'namespace' 
		) );
		$ret .= $this->start ( 'tr' );
		$ret .= $this->element ( 'th', $ns, array (
				'colspan' => 2 
		) );
		$ret .= $this->end ( 'tr' );
		$ret .= $this->end ( 'tbody' );
		...
		foreach ( $directives as $directive => $value ) {
			...
			if ($this->docURL) {
				...
				$ret .= $this->start ( 'a', array (
						'href' => $url 
				) );
			}
			$attr = array (
					'for' => "{$this->name}:$ns.$directive" 
			);
			...
			$ret .= $this->element ( 'label', $directive_disp, 
			...
			$ret .= $type_obj->render ( $ns, $directive, $value, $this->name, array (
					$this->genConfig,
					$this->config 
			) );
			$ret .= $this->end ( 'td' );
...
class HTMLPurifier_Printer_ConfigForm_NullDecorator extends HTMLPurifier_Printer {
	...
	public function render($ns, $directive, $value, $name, $config) {
		...
		$ret = '';
		$ret .= $this->start ( 'label', array (
				'for' => "$name:Null_$ns.$directive" 
		) );
		$ret .= $this->element ( 'span', "$ns.$directive:", array (
				'class' => 'verbose' 
		) );
...
;
		...
		$ret .= $this->obj->render ( $ns, $directive, $value, $name, array (
				$gen_config,
				$config 
		) );
		return $ret;
