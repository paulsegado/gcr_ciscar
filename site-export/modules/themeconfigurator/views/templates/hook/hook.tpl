	...
	<ul class="htmlcontent-home clearfix row">
		{foreach name=items from=$htmlitems item=hItem}
			...
			{/if}
					{if $hItem.url}
						{if $id_client == 0}
							<a href="index.php?controller=authentication" class="item-link" title="{$hItem.title|escape:'htmlall':'UTF-8'}">
						{else}
							<a href="{$hItem.url|escape:'htmlall':'UTF-8'}{$id_client|escape:'htmlall':'UTF-8'}&idLang={$iso_language|escape:'htmlall':'UTF-8'}" class="item-link"{if $hItem.target == 1} onclick="return !window.open(this.href);"{/if} title="{$hItem.title|escape:'htmlall':'UTF-8'}">
						{/if}
					{/if}
						{if $hItem.image}
