					...
					<li id="item-{$hItem.id_item|escape:'htmlall':'UTF-8'}" class="item well">
						<form method="post" action="{$htmlitems.postAction|escape:'htmlall':'UTF-8'}" enctype="multipart/form-data" class="item-form defaultForm  form-horizontal">
							...
							<div class="item-container clearfix">
								...
								<!-- CISCAR, On affiche la liste des groupes -->
								<div class="col-lg-3 control-label"><label>Group access</label></div>
								<div class="col-lg-5">
									<table class="table table-bordered">
										...
										<tbody>
										{foreach from=$htmlitems.listgroups  item=group key=keyGroup}
											<tr>
												<td>
													<input type="checkbox" name="groupBox[]" class="groupBox" id="groupBox_{$group.id_group}" value="{$group.id_group}"
													{foreach from=$htmlitems.itemsgroups  item=itemGroup key=keyItemGroup}	
													{if $itemGroup.id_item ==  $hItem.id_item && $itemGroup.id_group == $group.id_group }
														checked="checked"
													{/if}
													{/foreach}							
								...
								<!-- FIN CISCAR, On affiche la liste des groupes -->
