...
class ThemeConfigurator extends Module
{
	...
	public function hookDisplayHome()
	{
		$this->context->smarty->assign(array(
			...
			'iso_language' => $this->context->language->language_code			
		));
	...
	protected function getItemsFromHook($hook)
	{
		...
		//<!-- CISCAR on recupère les groupes de l'utilisateur -->
		//<!-- CISCAR on compare les groupes de l'utilisateur à ceux de l'item -->
		...
		SELECT *
		FROM `'._DB_PREFIX_.'themeconfigurator` tc
		WHERE id_shop = '.(int)$this->context->shop->id.' AND id_lang = '.(int)$this->context->language->id.'
		AND hook = \''.pSQL($hook).'\' AND active = 1
		AND tc.id_item in (SELECT id_item from '. _DB_PREFIX_ .'themeconfigurator_group where id_group in ('.implode(',',$customer_groups). ') and id_item = tc.id_item)
	...
	protected function removeItem()
	{
		...
		//<!-- CISCAR suppression groupes en BDD rattachés à un ITEM -->
		$this->deleteGroups($id_item);
		//<!-- FIN CISCAR suppression groupes en BDD rattachés à un ITEM -->
	...
	protected function updateItem()
	{
		...
		//<!-- CISCAR affectations groupes en BDD à un ITEM -->
		$this->updateGroup(Tools::getValue ( 'groupBox'));
		//<!-- FIN CISCAR affectations groupes en BDD à un ITEM -->
	...
	protected function addItem()
	{
		...
		))
		{
			...
			$this->addNewGroups($newId, Tools::getValue ( 'groupBox'));
			//<!-- FIN CISCAR affectations groupes en BDD à un ITEM -->
	...
	//<!-- CISCAR affectations groupes à un ITEM -->
	...
	public function cleanGroups() {
		return Db::getInstance ()->delete ( 'themeconfigurator_group', 'id_item = ' . ( int ) Tools::getValue ( 'item_id' ) );
	}
	
	public function deleteGroups($id_item) {
		return Db::getInstance ()->delete ( 'themeconfigurator_group', 'id_item = ' . ( int ) $id_item );
	}
	...
	public function addGroups($groups) {
		foreach ( $groups as $group ) {
			...
			Db::getInstance ()->insert ( 'themeconfigurator_group', $row, false, true, Db::INSERT_IGNORE );
		}
	}
	public function addNewGroups($id_item,$groups) {
		foreach ( $groups as $group ) {
			$row = array (
					'id_item' => ( int ) $id_item,
					'id_group' => ( int ) $group
	...
	public function getGroupsStatic($id_item) {
	
		$item_groups = array ();
		$result = Db::getInstance ()->executeS ( '
			SELECT hsg.`id_group`
			FROM ' . _DB_PREFIX_ . 'themeconfigurator_group
			WHERE `id_item` = ' . ( int ) $id_item );
		foreach ( $result as $group ) {
			$item_groups [] = ( int ) $group ['id_group'];
		}
	
		return $item_groups;
	}
	
	//<!-- FIN CISCAR affectations groupes à un ITEM -->
	...
	protected function renderThemeConfiguratorForm()
	{
		...
		//<!-- CISCAR, On recherche les groupes disponibles -->
		$groups = Group::getGroups ( $this->context->language->id, true );
		
		//<!-- CISCAR, On recherche les groupes des items -->
		$items_groups = Db::getInstance ()->ExecuteS ( '
					SELECT * FROM `' . _DB_PREFIX_ . 'themeconfigurator_group`
					ORDER BY id_item,id_group ASC' );
		//<!-- FIN CISCAR, On recherche les groupes des items -->
		...
		$this->context->smarty->assign('htmlitems', array(
			...
			'id_shop' => $id_shop,
			//on passe de nouveaux paramètres au template
			'listgroups' => $groups,
			'itemsgroups' => $items_groups
