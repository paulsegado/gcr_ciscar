...
class StatsData extends Module {
	...
	public function install() {
		...
		return (parent::install () && $this->registerHook ( $hookFooter ) && $this->registerHook ( 'authentication' ) && $this->registerHook ( 'createAccount' ));
	}
	...
	private function getScriptPlugins($params) {
		if (! isset ( $params ['cookie']->id_guest )) {
			...
			if (Configuration::get ( 'PS_STATSDATA_PLUGINS' )) {
				if (_PS_VERSION_ >= 1.7) {
					$this->context->controller->registerJavascript ( 'modules-plugindetect', 'modules/' . $this->name . '/js/plugindetect.js', array (
							'position' => 'bottom',
							'priority' => 150 
					) );
				} else {
