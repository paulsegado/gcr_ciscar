		...
		if (Tools::isSubmit ( 'submitBlocktopmenu' )) {
			...
			if (Tools::isSubmit ( 'submitBlocktopmenuLinks' )) {
				...
				foreach ( $shops as $shop_id ) {
					...
					Configuration::updateValue ( 'MOD_BLOCKTOPMENU_ITEMS', str_replace ( array (
							'LNK' . $id_linksmenutop . ',',
							'LNK' . $id_linksmenutop 
...
//<!-- CISCAR, Récupération du code ISO de la langue par le passer en paramètre à la PAPETERIE -->		
		$languages = $this->context->language->language_code;
		
		//$languages = Context::getLanguages ();
		
//<!-- CISCAR, On recherche si l'utilisateur apartient au groupe BELGE -->
		$carterie = "N";
		foreach ($this->context->customer->getGroupsStatic($this->context->customer->id) as $value) {
			if ($value >= 5 && $value < 15) $carterie = "O";
		}

//<!-- CISCAR, On recherche les groupes de l'utilisateur -->
		$customer_groups = $this->context->customer->getGroupsStatic($this->context->customer->id);
		...
		foreach ( $menu_items as $item ) {
			...
			switch (substr ( $item, 0, strlen ( $value [1] ) )) {
				...
				case 'CMS' :
					...
					$cms = CMS::getLinks ( ( int ) $id_lang, array (
							$id 
					) );
//<!-- CISCAR, on recherche les groupes de rattachement AU CMS		
					$cms_groups = CMS::getGroupsStatic ( $id );
					//on compare les groupes du l'utiliateur à ceux du cms
					$groupes_communs = array();
					$groupes_communs = array_intersect($customer_groups,$cms_groups);	
//<!-- CISCAR, le menu est  visible s'il y a un groupe en commun -->
					if (count ( $groupes_communs ) && $this->context->customer->isLogged () ) {
				...
				case 'CMS_CAT' :
					$category = new CMSCategory ( ( int ) $id, ( int ) $id_lang );
//<!-- CISCAR, on recherche les groupes de rattachement des CMS	rattachés à la catégorie
					$cms_groups_categorie =	array();
					$cms_groups_categorie = CMS::getGroupsCategory ( $id );
					//on compare les groupes du l'utiliateur à ceux du cms
					$groupes_communs = array();
					$groupes_communs = array_intersect($customer_groups,$cms_groups_categorie);	
					//<!-- CISCAR, le menu est  visible que pour le groupe BELGIQUE -->
					if (count ( $groupes_communs ) && $this->context->customer->isLogged () ) {
				...
				case 'LNK' :
					$link = MenuTopLinks::get ( ( int ) $id, ( int ) $id_lang, ( int ) $id_shop );
//<!-- CISCAR, le menu est  visible que pour le groupe BELGIQUE -->
					if (count ( $link ) && $this->context->customer->isLogged () && $carterie == "O") {
						if (! isset ( $link [0] ['label'] ) || ($link [0] ['label'] == '')) {
							$default_language = Configuration::get ( 'PS_LANG_DEFAULT' );
							$link = MenuTopLinks::get ( $link [0] ['id_linksmenutop'], $default_language, ( int ) Shop::getContextShopID () );
						}
						$link [0] ['link'] = $link [0] ['link'].$this->context->customer->id.'&idLang='.$languages ;
...
//<!-- CISCAR, on recherche les groupes de rattachement AU CMS		
				$cms_groups = CMS::getGroupsStatic ( $page ['id_cms'] );
				//on compare les groupes du l'utiliateur à ceux du cms
				$groupes_communs = array();
				$groupes_communs = array_intersect($customer_groups,$cms_groups);	
				if(count($groupes_communs))
				{
					$links = $cms->getLinks ( ( int ) $id_lang, array (
							( int ) $cms->id 
					) );
	...
	protected function getCacheId($name = null) {
		$page_name = in_array ( $this->page_name, array (
				'category',
				'supplier',
				'manufacturer',
				'cms',
...
//<!-- CISCAR on n'utilise pas le menu qui est en cache -->	
...
//<!-- CISCAR on n'utilise pas le menu qui est en cache -->
		...
		$html = $this->display ( __FILE__, 'blocktopmenu.tpl');
		return $html;
	}
