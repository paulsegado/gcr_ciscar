...
class Blockreinsurance extends Module {
	...
	public function install() {
		return parent::install () && $this->installDB () && Configuration::updateValue ( 'BLOCKREINSURANCE_NBBLOCKS', 5 ) && $this->registerHook ( 'footer' ) && $this->installFixtures () && 
		...
		if (isset ( $_POST ['nbblocks'] )) {
			for($i = 1; $i <= ( int ) $_POST ['nbblocks']; $i ++) {
				...
				Db::getInstance ()->execute ( 'INSERT INTO `' . _DB_PREFIX_ . 'reinsurance` (`filename`,`text`)
											VALUES ("' . ((isset ( $filename [0] ) && $filename [0] != '') ? pSQL ( $filename [0] ) : '') . '", "' . ((isset ( $_POST ['info' . $i . '_text'] ) && $_POST ['info' . $i . '_text'] != '') ? pSQL ( $_POST ['info' . $i . '_text'] ) : '') . '")' );
			}
			return true;
		} else
			return false;
	...
	public function installFixtures() {
		...
		$tab_texts = array (
				array (
						'text' => $this->l ( 'Money back guarantee.' ),
						'file_name' => 'reinsurance-1-1.jpg' 
				),
				array (
						'text' => $this->l ( 'In-store exchange.' ),
						'file_name' => 'reinsurance-2-1.jpg' 
				),
				array (
						'text' => $this->l ( 'Payment upon shipment.' ),
						'file_name' => 'reinsurance-3-1.jpg' 
				),
				array (
						'text' => $this->l ( 'Free Shipping.' ),
						'file_name' => 'reinsurance-4-1.jpg' 
				),
				array (
						'text' => $this->l ( '100% secure payment processing.' ),
