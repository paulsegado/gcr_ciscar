...
class DashGoals extends Module {
	...
	protected static $real_color = array (
			'#9E5BA1',
			'#00A89C',
			'#3AC4ED',
			'#F99031' 
	);
	protected static $more_color = array (
			'#803E84',
			'#008E7E',
			'#20B2E7',
			'#F66E1B' 
	);
	protected static $less_color = array (
			'#BC77BE',
			'#00C2BB',
			'#51D6F2',
			'#FBB244' 
	...
	public function setMonths($year) {
		...
		for($i = '01'; $i <= 12; $i = sprintf ( '%02d', $i + 1 ))
			$months [$i . '_' . $year] = array (
					'label' => Dashgoals::$month_labels [$i],
					'values' => array () 
			);
	...
	public function hookDashboardData($params) {
		...
		return array (
				'data_chart' => array (
						'dash_goals_chart1' => $this->getChartData ( $year ) 
				) 
		);
	}
	...
	public function getChartData($year) {
		...
		$stream_types = array (
				array (
						...
						'unit_text' => $this->l ( 'visits' ) 
				),
				array (
						'type' => 'conversion',
						'title' => $this->l ( 'Conversion' ),
						'unit_text' => '' 
				),
				array (
						'type' => 'avg_cart_value',
						'title' => $this->l ( 'Average cart value' ),
						'unit_text' => '' 
				),
				array (
						'type' => 'sales',
		...
		$stream_zones = array (
				...
				array (
						'zone' => 'more',
						'text' => $this->l ( 'Goal exceeded' ) 
				),
				array (
						'zone' => 'less',
						'text' => $this->l ( 'Goal not reached' ) 
				) 
		);
	...
	protected function getValuesFromGoals($average_goal, $month_goal, $value, $label) {
		...
		elseif ($fullfilment < 1) {
			$stream_values ['real'] = array (
					'x' => $label,
					'y' => round ( $fullfilment * $base_rate, 2 ) 
			);
			$stream_values ['less'] = array (
					'x' => $label,
					'y' => round ( $base_rate - ($fullfilment * $base_rate), 2 ) 
			);
		...
		elseif ($fullfilment > 1) {
			...
			$stream_values ['more'] = array (
					'x' => $label,
					'y' => round ( ($fullfilment * $base_rate) - $base_rate, 2 ) 
			);
		}
