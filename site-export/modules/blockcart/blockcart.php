...
class BlockCart extends Module {
				...
				foreach ( $products as $key => &$product ) {
					if ($product ['id_product'] == $cart_rule ['gift_product'] && $product ['id_product_attribute'] == $cart_rule ['gift_product_attribute']) {
						...
						if ($product ['cart_quantity'] > 1) {
							array_splice ( $products, $key, 0, array (
									$product 
							) );
	...
	public function install() {
		if (parent::install () == false || $this->registerHook ( 'top' ) == false || $this->registerHook ( 'header' ) == false || $this->registerHook ( 'actionCartListOverride' ) == false || Configuration::updateValue ( 'PS_BLOCK_CART_AJAX', 1 ) == false || Configuration::updateValue ( 'PS_BLOCK_CART_XSELL_LIMIT', 12 ) == false || Configuration::updateValue ( 'PS_BLOCK_CART_SHOW_CROSSSELLING', 1 ) == false)
			return false;
		return true;
	}
