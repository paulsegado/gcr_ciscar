...
class CronJobs extends Module {
	...
	public function install() {
		...
		if (parent::install ()) {
			...
			return $this->installDb () && $this->installTab () && $this->registerHook ( 'actionModuleRegisterHookAfter' ) && $this->registerHook ( 'actionModuleUnRegisterHookAfter' ) && $this->registerHook ( 'backOfficeHeader' );
		}
	...
	public function installDb() {
		return Db::getInstance ()->execute ( 'CREATE TABLE IF NOT EXISTS ' . _DB_PREFIX_ . bqSQL ( $this->name ) . ' (
	...
	public static function addOneShotTask($task, $description, $execution = array()) {
		...
		if (count ( $execution ) == 0) {
			$query = 'INSERT INTO ' . _DB_PREFIX_ . 'cronjobs
                ...
                VALUES (\'' . Db::getInstance ()->escape ( $description ) . '\', \'' . urlencode ( $task ) . '\', \'0\', \'' . CronJobs::EACH . '\', \'' . CronJobs::EACH . '\', \'' . CronJobs::EACH . '\',
			...
			if ($is_frequency_valid == true) {
				$query = 'INSERT INTO ' . _DB_PREFIX_ . 'cronjobs
                    (`description`, `task`, `hour`, `day`, `month`, `day_of_week`, `updated_at`, `one_shot`, `active`, `id_shop`, `id_shop_group`)
                    VALUES (\'' . Db::getInstance ()->escape ( $description ) . '\', \'' . urlencode ( $task ) . '\', \'' . $hour . '\', \'' . $day . '\', \'' . $month . '\', \'' . $day_of_week . '\',
                        NULL, TRUE, TRUE, ' . $id_shop . ', ' . $id_shop_group . ')';
	...
	protected function isLocalEnvironment() {
		...
		return in_array ( Tools::getRemoteAddr (), array (
				'127.0.0.1',
				'::1' 
		) ) || preg_match ( '/^172\.16\.|^192\.168\.|^10\.|^127\.|^localhost|\.local$/', Configuration::get ( 'PS_SHOP_DOMAIN' ) );
	}
	...
	protected function isNewJobValid() {
		if ((Tools::isSubmit ( 'description' ) == true) && (Tools::isSubmit ( 'task' ) == true) && (Tools::isSubmit ( 'hour' ) == true) && (Tools::isSubmit ( 'day' ) == true) && (Tools::isSubmit ( 'month' ) == true) && (Tools::isSubmit ( 'day_of_week' ) == true)) {
