...
class BlockCms extends Module {
	...
	protected function changePosition() {
		if (! Validate::isInt ( Tools::getValue ( 'position' ) ) || (Tools::getValue ( 'location' ) != BlockCMSModel::LEFT_COLUMN && Tools::getValue ( 'location' ) != BlockCMSModel::RIGHT_COLUMN) || (Tools::getValue ( 'way' ) != 0 && Tools::getValue ( 'way' ) != 1))
			Tools::displayError ();
	...
	protected function _postValidation() {
		...
		if (Tools::isSubmit ( 'submitBlockCMS' )) {
			...
			$footer_text = array (
					( int ) Configuration::get ( 'PS_LANG_DEFAULT' ) => Tools::getValue ( 'footer_text_' . ( int ) Configuration::get ( 'PS_LANG_DEFAULT' ) ) 
			);
