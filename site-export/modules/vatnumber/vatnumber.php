...
class VatNumber extends TaxManagerModule {
	...
	public function install() {
		return parent::install () && Configuration::updateValue ( 'VATNUMBER_MANAGEMENT', 1 ) && $this->registerHook ( 'actionValidateCustomerAddressForm' );
	}
	...
	public static function WebServiceCheck($vat_number) {
		...
		if (array_search ( $prefix, self::getPrefixIntracomVAT () ) === false)
			return array (
					Tools::displayError ( 'Invalid VAT number' ) 
			);
		...
		for($i = 0; $i < 3; $i ++) {
			if ($page_res = Tools::file_get_contents ( $url )) {
				if (preg_match ( '/invalid VAT number/i', $page_res )) {
					...
					return array (
							Tools::displayError ( 'VAT number not found' ) 
					);
		...
		return array (
				Tools::displayError ( 'VAT number validation service unavailable' ) 
		);
	}
