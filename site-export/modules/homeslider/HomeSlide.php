...
class HomeSlide extends ObjectModel {
	...
	//<!-- CISCAR affectations groupes à un SLIDE -->
	protected static $_slide_groups = array ();
	public $groupBox;
	...
	public function cleanGroups() {
		return Db::getInstance ()->delete ( 'homeslider_slides_group', 'id_homeslider_slides = ' . ( int ) $this->id );
	}
	
	public function addGroups($groups) {
		foreach ( $groups as $group ) {
			$row = array (
					'id_homeslider_slides' => ( int ) $this->id,
					'id_group' => ( int ) $group
			);
			Db::getInstance ()->insert ( 'homeslider_slides_group', $row, false, true, Db::INSERT_IGNORE );
		}
	}
	
	public static function getGroupsStatic($id_slide) {
	
		self::$_slide_groups [$id_slide] = array ();
		$result = Db::getInstance ()->executeS ( '
			SELECT hsg.`id_group`
			FROM ' . _DB_PREFIX_ . 'homeslider_slides_group hsg
			WHERE hsg.`id_homeslider_slides` = ' . ( int ) $id_slide );
		foreach ( $result as $group ) {
			self::$_slide_groups [$id_slide] [] = ( int ) $group ['id_group'];
		}
	
		return self::$_slide_groups [$id_slide];
	}
	public function getGroups() {
		return HomeSlide::getGroupsStatic ( ( int ) $this->id );
	}
