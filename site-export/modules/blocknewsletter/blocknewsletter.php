...
class Blocknewsletter extends Module {
	...
	public function install() {
		if (! parent::install () || ! Configuration::updateValue ( 'PS_NEWSLETTER_RAND', rand () . rand () ) || ! $this->registerHook ( array (
				'header',
				'footer',
	...
	public function renderList() {
		...
		if (version_compare ( _PS_VERSION_, '1.6.0.7', '<' )) {
			...
			$helper_list->actions = array_merge ( $helper_list->actions, array (
					'unsubscribe' 
			) );
		}
	 ...
	 * It will create an entry in the newsletter table
	...
	protected function sendVoucher($email, $code) {
		return Mail::Send ( $this->context->language->id, 'newsletter_voucher', Mail::l ( 'Newsletter voucher', $this->context->language->id ), array (
				'{discount}' => $code 
	...
	protected function sendVerificationEmail($email, $token) {
		...
		return Mail::Send ( $this->context->language->id, 'newsletter_verif', Mail::l ( 'Email verification', $this->context->language->id ), array (
				'{verif_url}' => $verif_url 
		), $email, null, null, null, null, null, dirname ( __FILE__ ) . '/mails/', false, $this->context->shop->id );
	}
