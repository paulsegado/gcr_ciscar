...
class CrossSelling extends Module {
	...
	public function install() {
		if (! parent::install () || ! $this->registerHook ( 'productFooter' ) || ! $this->registerHook ( 'header' ) || ! $this->registerHook ( 'shoppingCart' ) || ! $this->registerHook ( 'actionOrderStatusPostUpdate' ) || ! Configuration::updateValue ( 'CROSSSELLING_DISPLAY_PRICE', 0 ) || ! Configuration::updateValue ( 'CROSSSELLING_NBR', 10 )) {
			return false;
		}
	...
	public function uninstall() {
		$this->_clearCache ( 'crossselling.tpl' );
		if (! parent::uninstall () || ! Configuration::deleteByName ( 'CROSSSELLING_DISPLAY_PRICE' ) || ! Configuration::deleteByName ( 'CROSSSELLING_NBR' )) {
			return false;
		}
	...
	public function hookHeader() {
		...
		if (in_array ( $this->context->controller->php_self, array (
				'order' 
		) ) && Tools::getValue ( 'step' )) {
			return;
		}
	...
	public function hookProductFooter($params) {
		...
		if (! $this->isCached ( 'crossselling.tpl', $this->getCacheId ( $cache_id ) )) {
			$final_products_list = $this->getOrderProducts ( array (
					$params ['product']->id 
			) );
