			...
			if ($products_count < 5000) // Lock indexation if too many products
{
	...
	public function hookAfterSaveAttributeGroup($params) {
		...
		Db::getInstance ()->execute ( 'DELETE FROM ' . _DB_PREFIX_ . 'layered_indexable_attribute_group
			WHERE `id_attribute_group` = ' . ( int ) $params ['id_attribute_group'] );
		Db::getInstance ()->execute ( 'DELETE FROM ' . _DB_PREFIX_ . 'layered_indexable_attribute_group_lang_value
			WHERE `id_attribute_group` = ' . ( int ) $params ['id_attribute_group'] );
		
		Db::getInstance ()->execute ( 'INSERT INTO ' . _DB_PREFIX_ . 'layered_indexable_attribute_group (`id_attribute_group`, `indexable`)
	...
	public function hookAttributeGroupForm($params) {
		$values = array ();
		$is_indexable = Db::getInstance ( _PS_USE_SQL_SLAVE_ )->getValue ( 'SELECT `indexable`
		...
		if ($result = Db::getInstance ( _PS_USE_SQL_SLAVE_ )->executeS ( 'SELECT `url_name`, `meta_title`, `id_lang` FROM ' . _DB_PREFIX_ . 'layered_indexable_attribute_group_lang_value
			WHERE `id_attribute_group` = ' . ( int ) $params ['id_attribute_group'] ))
			foreach ( $result as $data )
				$values [$data ['id_lang']] = array (
	...
	public function hookAfterSaveAttribute($params) {
		...
		Db::getInstance ()->execute ( 'DELETE FROM ' . _DB_PREFIX_ . 'layered_indexable_attribute_lang_value
	...
	public function hookAttributeForm($params) {
		...
		if ($result = Db::getInstance ( _PS_USE_SQL_SLAVE_ )->executeS ( 'SELECT `url_name`, `meta_title`, `id_lang`
	...
	public function hookAfterSaveFeature($params) {
		...
		Db::getInstance ()->execute ( 'DELETE FROM ' . _DB_PREFIX_ . 'layered_indexable_feature
			WHERE `id_feature` = ' . ( int ) $params ['id_feature'] );
		Db::getInstance ()->execute ( 'DELETE FROM ' . _DB_PREFIX_ . 'layered_indexable_feature_lang_value
			WHERE `id_feature` = ' . ( int ) $params ['id_feature'] );
		
		Db::getInstance ()->execute ( 'INSERT INTO ' . _DB_PREFIX_ . 'layered_indexable_feature
	...
	public function hookFeatureForm($params) {
		...
		if ($result = Db::getInstance ( _PS_USE_SQL_SLAVE_ )->executeS ( 'SELECT `url_name`, `meta_title`, `id_lang` FROM ' . _DB_PREFIX_ . 'layered_indexable_feature_lang_value
			WHERE `id_feature` = ' . ( int ) $params ['id_feature'] ))
			foreach ( $result as $data )
				$values [$data ['id_lang']] = array (
						'url_name' => $data ['url_name'],
	...
	public function hookAfterSaveFeatureValue($params) {
		...
		Db::getInstance ()->execute ( 'DELETE FROM ' . _DB_PREFIX_ . 'layered_indexable_feature_value_lang_value
	...
	public function hookProductListAssign($params) {
		if ((isset ( $this->context->controller->display_column_left ) && ! $this->context->controller->display_column_left) && (isset ( $this->context->controller->display_column_right ) && ! $this->context->controller->display_column_right))
			return false;
			...
			switch ($type) {
				case 'id_attribute_group' :
					foreach ( $val as $attr ) {
						...
						if (in_array ( $attr_id, $attributes ) || in_array ( array (
								'id_attribute_group' => $attr_id 
				...
				case 'id_feature' :
					foreach ( $val as $feat ) {
						...
						if (in_array ( $feat_id, $features ) || in_array ( array (
								'id_feature' => $feat_id 
						), $feature_list )) {
							$smarty->assign ( 'nobots', true );
							$smarty->assign ( 'nofollow', true );
	...
	public function hookCategoryAddition($params) {
		$this->rebuildLayeredCache ( array (), array (
				( int ) $params ['category']->id 
		) );
	}
	...
	public function hookCategoryDeletion($params) {
		...
		foreach ( $layered_filter_list as $layered_filter ) {
			...
			if (in_array ( ( int ) $params ['category']->id, $data ['categories'] )) {
				...
				Db::getInstance ()->execute ( 'UPDATE `' . _DB_PREFIX_ . 'layered_filter`
			...
			switch ($filter ['type']) {
				...
				case 'manufacturer' :
					...
					foreach ( $manufacturers as $manufacturer ) {
						...
						$attribute_values_by_lang [$manufacturer ['id_lang']] ['manufacturer'] [] = array (
								...
								'id_value' => $manufacturer ['id_manufacturer'],
								'category_name' => $filter ['link_rewrite'],
								'type' => $filter ['type'] 
						);
					}
				...
				case 'quantity' :
					...
					foreach ( $avaibility_list as $key => $quantity )
						$attribute_values_by_lang [$filter ['id_lang']] ['quantity'] [] = array (
				...
				case 'condition' :
					...
					foreach ( $condition_list as $key => $condition )
						$attribute_values_by_lang [$filter ['id_lang']] ['condition'] [] = array (
			...
			foreach ( $attribute_values as $attribute )
				foreach ( $attribute as $param ) {
					...
					if ($id_layered_friendly_url == false) {
						Db::getInstance ()->insert ( 'layered_friendly_url', array (
								'url_key' => $url_key,
								'data' => serialize ( $selected_filters ),
								'id_lang' => ( int ) $id_lang 
						) );
			...
			elseif (! Tools::getValue ( 'categoryBox' ))
				...
				if (isset ( $_POST ['checkBoxShopAsso_layered_filter'] )) {
					foreach ( $_POST ['checkBoxShopAsso_layered_filter'] as $id_shop => $row ) {
						$assos [] = array (
								'id_object' => ( int ) $id_layered_filter,
								'id_shop' => ( int ) $id_shop 
						);
		...
		if (strpos ( $_SERVER ['SCRIPT_FILENAME'], 'blocklayered-ajax.php' ) === false || Tools::getValue ( 'selected_filters' ) !== false) {
			...
			if (! empty ( $url_attributes )) {
				foreach ( $url_attributes as $url_attribute ) {
					...
					else if (in_array ( $attribute_name, array (
							'price',
							'weight' 
					) ))
						$selected_filters [$attribute_name] = array (
								$this->filterVar ( $url_parameters [0] ),
								$this->filterVar ( $url_parameters [1] ) 
						);
					else {
		...
		foreach ( $_GET as $key => $value )
			if (substr ( $key, 0, 8 ) == 'layered_') {
				...
				if (isset ( $res [1] )) {
					...
					if (isset ( $tmp_tab [1] ))
						$id_key = $tmp_tab [1];
					if ($res [1] == 'condition' && in_array ( $value, array (
							'new',
							'used',
							'refurbished' 
					) ))
	...
	public function getProductByFilters($selected_filters = array()) {
		...
		if ($this->nbr_products == 0)
			...
			if (version_compare ( _PS_VERSION_, '1.6.1', '>=' ) === true) {
				...
				ORDER BY ' . Tools::getProductsOrder ( 'by', Tools::getValue ( 'orderby' ), true ) . ' ' . Tools::getProductsOrder ( 'way', Tools::getValue ( 'orderway' ) ) . ' , cp.id_product' . ' LIMIT ' . ((( int ) $this->page - 1) * $n . ',' . $n), true, false );
			} else {
				$this->products = Db::getInstance ()->executeS ( '
				SELECT
					p.*,
	...
	public function getFilterBlock($selected_filters = array()) {
		...
		foreach ( $filters as $filter ) {
			...
			foreach ( $filters as $filter_tmp ) {
				...
				if (method_exists ( 'BlockLayered', $method_name ) && ($filter ['type'] != 'price' && $filter ['type'] != 'weight' && $filter ['type'] != $filter_tmp ['type'] || $filter ['type'] == $filter_tmp ['type'])) {
		...
		foreach ( $filter_blocks as $type_filter ) {
			...
			if (($lower_filter == 'price' || $lower_filter == 'weight') && ( float ) $type_filter ['values'] [0] > ( float ) $type_filter ['min'] && ( float ) $type_filter ['values'] [1] > ( float ) $type_filter ['max']) {
	...
	private static function filterProductsByPrice($filter_value, $product_collection) {
		...
		foreach ( $product_collection as $key => $product ) {
			if (isset ( $filter_value ) && $filter_value && isset ( $product ['price_min'] ) && isset ( $product ['id_product'] ) && (($product ['price_min'] < ( int ) $filter_value [0] && $product ['price_max'] > ( int ) $filter_value [0]) || ($product ['price_max'] > ( int ) $filter_value [1] && $product ['price_min'] < ( int ) $filter_value [1]))) {
				$price = Product::getPriceStatic ( $product ['id_product'], $ps_layered_filter_price_usetax );
				if ($ps_layered_filter_price_rounding) {
					$price = ( int ) $price;
				}
	...
	private static function getWeightFilterSubQuery($filter_value, $ignore_join = false) {
		if (isset ( $filter_value ) && $filter_value) {
			if ($filter_value [0] != 0 || $filter_value [1] != 0) {
				return array (
						'where' => ' AND p.`weight` BETWEEN ' . ( float ) ($filter_value [0] - 0.001) . ' AND ' . ( float ) ($filter_value [1] + 0.001) . ' ' 
				);
			}
		}
	...
	public function ajaxCall() {
		...
		if (version_compare ( _PS_VERSION_, '1.6.0', '>=' ) === true)
			$vars = array_merge ( $vars, array (
					'pagination_bottom' => $smarty->assign ( 'paginationId', 'bottom' )->fetch ( _PS_THEME_DIR_ . 'pagination.tpl' ) 
			) );
	...
	public function rebuildLayeredCache($products_ids = array(), $categories_ids = array()) {
		...
		WHERE c.active = 1' . (count ( $categories_ids ) ? ' AND cp.id_category IN (' . implode ( ',', $categories_ids ) . ')' : '') . '
		AND ' . $alias . '.active = 1 AND ' . $alias . '.`visibility` IN ("both", "catalog")
		' . (count ( $products_ids ) ? 'AND p.id_product IN (' . implode ( ',', $products_ids ) . ')' : '') . ' AND (fv.custom IS NULL OR fv.custom = 0)
		GROUP BY p.id_product' );
		...
		while ( $product = $db->nextRow ( $result ) ) {
			...
			foreach ( $c as $id_category => $category ) {
				...
				if (! isset ( $done_categories [( int ) $id_category] ['cat'] )) {
					$filter_data ['layered_selection_subcategories'] = array (
							'filter_type' => 0,
							'filter_show_limit' => 0 
					);
				...
				if (is_array ( $attribute_groups_by_id ) && count ( $attribute_groups_by_id ) > 0)
					foreach ( $a as $k_attribute => $attribute )
						if (! isset ( $done_categories [( int ) $id_category] ['a' . ( int ) $attribute_groups_by_id [( int ) $k_attribute]] )) {
							$filter_data ['layered_selection_ag_' . ( int ) $attribute_groups_by_id [( int ) $k_attribute]] = array (
									'filter_type' => 0,
									'filter_show_limit' => 0 
							);
				...
				if (is_array ( $attribute_groups_by_id ) && count ( $attribute_groups_by_id ) > 0)
					foreach ( $f as $k_feature => $feature )
						if (! isset ( $done_categories [( int ) $id_category] ['f' . ( int ) $features_by_id [( int ) $k_feature]] )) {
							$filter_data ['layered_selection_feat_' . ( int ) $features_by_id [( int ) $k_feature]] = array (
									'filter_type' => 0,
									'filter_show_limit' => 0 
							);
				...
				if (! isset ( $done_categories [( int ) $id_category] ['q'] )) {
					$filter_data ['layered_selection_stock'] = array (
							'filter_type' => 0,
							'filter_show_limit' => 0 
					);
				...
				if (! isset ( $done_categories [( int ) $id_category] ['m'] )) {
					$filter_data ['layered_selection_manufacturer'] = array (
							'filter_type' => 0,
							'filter_show_limit' => 0 
					);
				...
				if (! isset ( $done_categories [( int ) $id_category] ['c'] )) {
					$filter_data ['layered_selection_condition'] = array (
							'filter_type' => 0,
							'filter_show_limit' => 0 
					);
				...
				if (! isset ( $done_categories [( int ) $id_category] ['w'] )) {
					$filter_data ['layered_selection_weight_slider'] = array (
							'filter_type' => 0,
							'filter_show_limit' => 0 
					);
				...
				if (! isset ( $done_categories [( int ) $id_category] ['p'] )) {
					$filter_data ['layered_selection_price_slider'] = array (
							'filter_type' => 0,
							'filter_show_limit' => 0 
					);
