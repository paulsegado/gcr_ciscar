...
class Ganalytics extends Module {
	...
	protected function _getGoogleAnalyticsTag($back_office = false) {
		$user_id = null;
		if (Configuration::get ( 'GA_USERID_ENABLED' ) && $this->context->customer && $this->context->customer->isLogged ()) {
		...
		return '
			<script type="text/javascript">
				...
				ga(\'require\', \'ec\');' . (($user_id && ! $back_office) ? 'ga(\'set\', \'&uid\', \'' . $user_id . '\');' : '') . ($back_office ? 'ga(\'set\', \'nonInteraction\', true);' : '') . '</script>';
	}
	...
	public function hookProductFooter($params) {
		...
		if ($controller_name == 'product') {
			...
			if (isset ( $_SERVER ['HTTP_REFERER'] ) && strpos ( $_SERVER ['HTTP_REFERER'], $_SERVER ['HTTP_HOST'] ) > 0)
				$js .= $this->addProductClickByHttpReferal ( array (
						$ga_product 
				) );
	...
	public function hookActionProductCancel($params) {
		...
		foreach ( $qty_refunded as $orderdetail_id => $qty ) {
			...
			$ga_scripts .= 'MBG.add(' . Tools::jsonEncode ( array (
		...
		$this->context->cookie->ga_admin_refund = $ga_scripts . 'MBG.refundByProduct(' . Tools::jsonEncode ( array (
				'id' => $params ['order']->id 
		) ) . ');';
	}
