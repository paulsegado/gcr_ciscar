...
class AdminGamificationController extends ModuleAdminController {
	...
	public function setMedia() {
		...
		$this->addCSS ( array (
				_MODULE_DIR_ . $this->module->name . '/views/css/bubble-popup.css',
				_MODULE_DIR_ . $this->module->name . '/views/css/isotope.css' 
		) );
	...
	public function renderView() {
		...
		$badges_type = array (
				'badges_feature' => array (
						'name' => $this->l ( 'Features' ),
						'badges' => $badges_feature 
				),
				'badges_achievement' => array (
						'name' => $this->l ( 'Achievements' ),
						'badges' => $badges_achievement 
				),
				'badges_international' => array (
						'name' => $this->l ( 'International' ),
						'badges' => $badges_international 
				) 
		);
	...
	public function ajaxProcessGamificationTasks() {
		...
		if (count ( $return ['advices_to_display'] ['advices'] ) > 1) {
			...
			$return ['advices_to_display'] ['advices'] = array (
					$return ['advices_to_display'] ['advices'] [$rand] 
			);
		}
		...
		if (Tab::getIdFromClassName ( 'AdminDashboard' ) == Tools::getValue ( 'id_tab' )) {
			...
			if (count ( $return ['advices_premium_to_display'] ['advices'] ) >= 2) {
				...
				$return ['advices_premium_to_display'] ['advices'] = array (
						$weighted_advices_array [$rand],
						$weighted_advices_array [$rand2] 
				);
