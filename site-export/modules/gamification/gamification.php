...
class gamification extends Module {
	...
	public function installTab() {
		...
		if (version_compare ( _PS_VERSION_, '1.7.0.0', '>=' )) {
			// AdminPreferences
			$tab->id_parent = ( int ) Db::getInstance ( _PS_USE_SQL_SLAVE_ )->getValue ( 'SELECT MIN(id_tab)
	...
	public function __call($name, $arguments) {
		if (! empty ( self::$_batch_mode )) {
			self::$_defered_func_call [get_class () . '::__call_' . $name] = array (
					array (
							$this,
							'__call' 
					),
		...
		if (! $this->isFresh ( $cache_file, 86400 )) {
			if ($this->getData ( $iso_lang )) {
				...
				if (function_exists ( 'openssl_verify' ) && self::TEST_MODE === false) {
					if (! openssl_verify ( Tools::jsonencode ( array (
							$data->conditions,
							$data->advices_lang 
					) ), base64_decode ( $data->signature ), file_get_contents ( dirname ( __FILE__ ) . '/prestashop.pub' ) )) {
						return false;
					}
				}
				...
				if (function_exists ( 'openssl_verify' ) && self::TEST_MODE === false) {
					if (! openssl_verify ( Tools::jsonencode ( array (
							$data->advices_lang_16 
					) ), base64_decode ( $data->signature_16 ), file_get_contents ( dirname ( __FILE__ ) . '/prestashop.pub' ) )) {
						return false;
					}
				}
	...
	public function processImportBadges($badges, $badges_lang, $id_lang) {
		...
		foreach ( $badges_lang as $lang ) {
			$formated_badges_lang [$lang->id_ps_badge] = array (
					...
					'description' => array (
							$id_lang => $lang->description 
					),
					'group_name' => array (
							$id_lang => $lang->group_name 
					) 
			);
		}
			...
			try {
				...
				if (in_array ( ( int ) $badge->id_ps_badge, $current_badges )) {
					...
					foreach ( $badge->conditions as $cond ) {
						Db::getInstance ()->insert ( 'condition_badge', array (
								'id_condition' => $cond_ids [$cond],
								'id_badge' => $bdg->id 
						) );
					}
				}
	...
	public function processImportAdvices($advices, $advices_lang, $id_lang) {
		...
		foreach ( $advices_lang as $lang ) {
			$formated_advices_lang [$lang->id_ps_advice] = array (
					'html' => array (
							$id_lang => $lang->html 
					) 
			);
		...
		if (is_array ( $display_conditions )) {
			foreach ( $display_conditions as $cond ) {
				Db::getInstance ()->insert ( 'condition_advice', array (
						'id_condition' => ( int ) $cond_ids [$cond],
