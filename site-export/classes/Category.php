...
class CategoryCore extends ObjectModel {
	...
	public function add($autodate = true, $null_values = false) {
		...
		Hook::exec ( 'actionCategoryAdd', array (
				'category' => $this 
		) );
		return $ret;
	}
	 ...
	 * @param array $excluded_ids_array
	...
	public static function recurseCategory($categories, $current, $id_category = null, $id_selected = 1) {
		...
		echo '<option value="' . $id_category . '"' . (($id_selected == $id_category) ? ' selected="selected"' : '') . '>' . str_repeat ( '&nbsp;', $current ['infos'] ['level_depth'] * 5 ) . stripslashes ( $current ['infos'] ['name'] ) . '</option>';
	 ...
	 *
	 * @param
	 *        	array &$to_delete Array reference where categories ID will be saved
	...
	public function delete() {
		...
		foreach ( $all_cat as $cat ) {
			...
			if (! $this->hasMultishopEntries ()) {
				...
				CartRule::cleanProductRuleIntegrity ( 'categories', array (
						$cat->id 
				) );
		...
		Hook::exec ( 'actionCategoryDelete', array (
				'category' => $this,
	...
	public static function getAllCategoriesName($root_category = null, $id_lang = false, $active = true, $groups = null, $use_shop_restriction = true, $sql_filter = '', $sql_sort = '', $sql_limit = '') {
		...
		$cache_id = 'Category::getAllCategoriesName_' . md5 ( ( int ) $root_category . ( int ) $id_lang . ( int ) $active . ( int ) $use_shop_restriction . (isset ( $groups ) && Group::isFeatureActive () ? implode ( '', $groups ) : '') );
	...
	public static function getNestedCategories($root_category = null, $id_lang = false, $active = true, $groups = null, $use_shop_restriction = true, $sql_filter = '', $sql_sort = '', $sql_limit = '') {
		...
		$cache_id = 'Category::getNestedCategories_' . md5 ( ( int ) $root_category . ( int ) $id_lang . ( int ) $active . ( int ) $use_shop_restriction . (isset ( $groups ) && Group::isFeatureActive () ? implode ( '', $groups ) : '') );
	 ...
	 *        	If set to true, returns the total number of results only
	 * @param bool $active
	 *        	If set to true, finds only active products
	 * @param bool $random
	 *        	If true, sets a random filter for returned products
	 * @param int $random_number_products
	 *        	Number of products to return if random is activated
	 * @param bool $check_access
	 *        	If set tot rue, check if the current customer
...
//<!-- CISCAR, On recherche les groupes de l'utilisateur -->
		//$customer_groups = customer::getGroupsStatic($context->customer->id);
		$groups = FrontController::getCurrentCustomerGroups ();
		$customer_groups = implode(",", $groups);

//<!-- CISCAR, on recupère les catégories rattachées au produit -->
		if($customer_groups != '')
		{
			$catProd =',(select group_concat(scg.id_group) from ' . _DB_PREFIX_ . 'category_product scp, ' . _DB_PREFIX_ . 'category_group scg where scp.id_category = scg.id_category and scp.id_product = p.id_product and scp.id_category <> 2 and scg.id_group in('.$customer_groups.') order by scg.id_group) as groups_product';
			$catProd_Having = ' HAVING groups_product <> \'\'';
		}
		else 
		{
			$catProd ='';
...
//<!-- FIN CISCAR -->		
	...
	public function updatePosition($way, $position) {
		...
		$result = (Db::getInstance ()->execute ( '
            ...
            AND c.`id_parent`=' . ( int ) $moved_category ['id_parent'] ) && Db::getInstance ()->execute ( '
            UPDATE `' . _DB_PREFIX_ . 'category` c ' . Shop::addSqlAssociation ( 'category', 'c' ) . '
		...
		Hook::exec ( 'actionCategoryUpdate', array (
				'category' => new Category ( $moved_category ['id_category'] ) 
		) );
		return $result;
	}
	 ...
	 * but remove duplicate position.
	 * Should not be used if positions
	...
	public static function getUrlRewriteInformations($id_category) {
		return Db::getInstance ()->executeS ( 'SELECT l.`id_lang`, c.`link_rewrite`
