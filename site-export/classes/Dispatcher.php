...
class DispatcherCore {
	...
	protected function loadRoutes($id_shop = null) {
		...
		$modules_routes = Hook::exec ( 'moduleRoutes', array (
				'id_shop' => $id_shop 
		), null, true, false );
	 ...
	 * @param int $id_lang        	
	 * @param array $params        	
	 * @param bool $use_routes
	 *        	If false, don't use to create this url
	 * @param string $anchor
	...
	public static function getModuleControllers($type = 'all', $module = null) {
		...
		if (is_null ( $module )) {
			...
			$modules = array (
					Module::getInstanceByName ( $module ) 
			);
		} else {
			$modules = array ();
			...
			if ($controller_filename [0] != '.') {
				if (! strpos ( $controller_filename, '.php' ) && is_dir ( $dir . $controller_filename )) {
					...
					$key = str_replace ( array (
							'controller.php',
							'.php' 
					), '', strtolower ( $controller_filename ) );
