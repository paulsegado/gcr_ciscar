...
class AttributeGroupCore extends ObjectModel {
	...
	public function add($autodate = true, $nullValues = false) {
		...
		Hook::exec ( 'actionAttributeGroupSave', array (
				'id_attribute_group' => $this->id 
		) );
		return $return;
	}
	...
	public function delete() {
		...
		if ($return) {
			Hook::exec ( 'actionAttributeGroupDelete', array (
					'id_attribute_group' => $this->id 
			) );
		}
		return $return;
	...
	public function updatePosition($way, $position) {
		...
		return (Db::getInstance ()->execute ( '
			...
			WHERE `position`
			' . ($way ? '> ' . ( int ) $moved_group_attribute ['position'] . ' AND `position` <= ' . ( int ) $position : '< ' . ( int ) $moved_group_attribute ['position'] . ' AND `position` >= ' . ( int ) $position) ) && Db::getInstance ()->execute ( '
			UPDATE `' . _DB_PREFIX_ . 'attribute_group`
