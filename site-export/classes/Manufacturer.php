...
class ManufacturerCore extends ObjectModel {
	 ...
	 *        	[optional] return products numbers for each
	 * @param int $id_lang        	
	 * @param bool $active        	
	 * @param int $p        	
	 * @param int $n        	
	...
	public static function getManufacturers($get_nb_products = false, $id_lang = 0, $active = true, $p = false, $n = false, $all_group = false, $group_by = false) {
		...
		' . ($active ? 'WHERE m.`active` = 1' : '') . ($group_by ? ' GROUP BY m.`id_manufacturer`' : '') . '
		ORDER BY m.`name` ASC
	...
	public function getProductsLite($id_lang) {
		...
		WHERE p.`id_manufacturer` = ' . ( int ) $this->id . ($front ? ' AND product_shop.`visibility` IN ("both", "catalog")' : '') );
	}
