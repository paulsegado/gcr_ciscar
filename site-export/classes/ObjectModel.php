...
abstract class ObjectModelCore implements Core_Foundation_Database_EntityInterface {
	...
	public function getFieldsLang() {
		...
		if ($this->id_lang === null) {
			...
			$fields = array (
					$this->id_lang => $this->formatFields ( self::FORMAT_LANG, $this->id_lang ) 
			);
	...
	public function add($auto_date = true, $null_values = false) {
		...
		Hook::exec ( 'actionObjectAddBefore', array (
				'object' => $this 
		) );
		...
		Hook::exec ( 'actionObjectAddAfter', array (
				'object' => $this 
		) );
		Hook::exec ( 'actionObject' . get_class ( $this ) . 'AddAfter', array (
				'object' => $this 
		) );
	...
	public function update($null_values = false) {
		...
		Hook::exec ( 'actionObjectUpdateBefore', array (
				'object' => $this 
		) );
		Hook::exec ( 'actionObject' . get_class ( $this ) . 'UpdateBefore', array (
				'object' => $this 
		) );
		...
		Hook::exec ( 'actionObjectUpdateAfter', array (
				'object' => $this 
		) );
		Hook::exec ( 'actionObject' . get_class ( $this ) . 'UpdateAfter', array (
				'object' => $this 
		) );
	...
	public function delete() {
		...
		Hook::exec ( 'actionObjectDeleteBefore', array (
				'object' => $this 
		) );
		Hook::exec ( 'actionObject' . get_class ( $this ) . 'DeleteBefore', array (
				'object' => $this 
		) );
		...
		Hook::exec ( 'actionObjectDeleteAfter', array (
				'object' => $this 
		) );
		Hook::exec ( 'actionObject' . get_class ( $this ) . 'DeleteAfter', array (
				'object' => $this 
		) );
	 ...
	 *        	If true, uses more descriptive, translatable error strings.
	 *        	
	 ...
	 *        	Field name
	 * @param string $class
	 *        	ObjectModel class name
	 * @param bool $htmlentities
	 *        	If true, applies htmlentities() to result string
	 ...
	 *
	 * @param bool $htmlentities
	 *        	If true, uses htmlentities() for field name translations in errors.
	 *        	
	 * @return array
	 */
	public function validateController($htmlentities = true) {
		...
		foreach ( $this->def ['fields'] as $field => $data ) {
			...
			if (isset ( $data ['size'] ) && ! empty ( $value ) && Tools::strlen ( $value ) > $data ['size']) {
				$errors [$field] = sprintf ( Tools::displayError ( '%1$s is too long. Maximum length: %2$d' ), self::displayFieldName ( $field, get_class ( $this ), $htmlentities ), $data ['size'] );
			}
	...
	public function getWebserviceParameters($ws_params_attribute_name = null) {
		...
		foreach ( $this->def ['fields'] as $field_name => $details ) {
			...
			if (isset ( $details ['validate'] )) {
				$current_field ['validateMethod'] = (array_key_exists ( 'validateMethod', $resource_parameters ['fields'] [$field_name] ) ? array_merge ( $resource_parameters ['fields'] [$field_name] ['validateMethod'], array (
						$details ['validate'] 
				) ) : array (
						$details ['validate'] 
				));
	 ...
	 *
	 * @param bool $all
	 *        	If true, returns required fields of all object classes.
	 *        	
	 * @return array|null
	 * @throws PrestaShopDatabaseException
	 */
	 ...
	 *
	 * @param bool $all
	 *        	If true, caches required fields of all object classes.
	 */
	...
	public function addFieldsRequiredDatabase($fields) {
		...
		foreach ( $fields as $field ) {
			if (! Db::getInstance ()->insert ( 'required_field', array (
					'object_name' => get_class ( $this ),
					'field_name' => pSQL ( $field ) 
			) )) {
				return false;
			}
		}
	 ...
	 *
	 * @param bool $all
	 *        	If true, clears cache for all objects
	 */
	...
	public function deleteImage($force_delete = false) {
		...
		if ($force_delete || ! $this->hasMultishopEntries ()) {
			...
			if ($this->image_dir) {
				if (file_exists ( $this->image_dir . $this->id . '.' . $this->image_format ) && ! unlink ( $this->image_dir . $this->id . '.' . $this->image_format )) {
					return false;
				}
			}
			if (file_exists ( _PS_TMP_IMG_DIR_ . $this->def ['table'] . '_' . $this->id . '.' . $this->image_format ) && ! unlink ( _PS_TMP_IMG_DIR_ . $this->def ['table'] . '_' . $this->id . '.' . $this->image_format )) {
				return false;
			}
			if (file_exists ( _PS_TMP_IMG_DIR_ . $this->def ['table'] . '_mini_' . $this->id . '.' . $this->image_format ) && ! unlink ( _PS_TMP_IMG_DIR_ . $this->def ['table'] . '_mini_' . $this->id . '.' . $this->image_format )) {
				return false;
			}
			
			$types = ImageType::getImagesTypes ();
			foreach ( $types as $image_type ) {
				if (file_exists ( $this->image_dir . $this->id . '-' . stripslashes ( $image_type ['name'] ) . '.' . $this->image_format ) && ! unlink ( $this->image_dir . $this->id . '-' . stripslashes ( $image_type ['name'] ) . '.' . $this->image_format )) {
					return false;
				}
			}
		}
	...
	/**
	 * Fill an object with given data.
	 * Data must be an array with this syntax:
