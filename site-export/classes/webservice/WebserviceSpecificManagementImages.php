...
class WebserviceSpecificManagementImagesCore implements WebserviceSpecificManagementInterface {
	 ...
	 */
	protected $acceptedImgMimeTypes = array (
			'image/gif',
			'image/jpg',
			'image/jpeg',
			'image/pjpeg',
	...
	public function getContent() {
		...
		elseif ($this->imgToDisplay) {
			...
			if (! $imageResource) {
				throw new WebserviceException ( sprintf ( 'Unable to load the image "%s"', str_replace ( _PS_ROOT_DIR_, '[SHOP_ROOT_DIR]', $this->imgToDisplay ) ), array (
						47,
						500 
				) );
			} else {
	...
	protected function manageImages() {
		...
		switch ($this->wsObject->urlSegment [1]) {
			...
			case '' :
				...
				break;
			default :
				$exception = new WebserviceException ( sprintf ( 'Image of type "%s" does not exist', $this->wsObject->urlSegment [1] ), array (
						48,
						400 
				) );
	...
	protected function manageGeneralImages() {
		...
		switch ($this->wsObject->urlSegment [2]) {
			...
			case 'header' :
				if (in_array ( $this->wsObject->method, array (
						'GET',
						'HEAD',
						'PUT' 
				) )) {
					...
					throw new WebserviceException ( 'This method is not allowed with general image resources.', array (
							49,
							405 
					) );
				}
			...
			default :
				$exception = new WebserviceException ( sprintf ( 'General image of type "%s" does not exist', $this->wsObject->urlSegment [2] ), array (
						53,
						400 
				) );
		...
		switch ($this->wsObject->method) {
			...
			case 'PUT' :
				...
				if ($this->writePostedImageOnDisk ( $path, null, null )) {
					...
					throw new WebserviceException ( 'Error while copying image to the directory', array (
							54,
							400 
					) );
				}
	...
	protected function manageListDeclinatedImages($directory, $normal_image_sizes) {
		...
		if ($this->wsObject->method != 'GET') {
			throw new WebserviceException ( 'This method is not allowed for listing category images.', array (
					55,
					405 
			) );
		}
		...
		foreach ( $normal_image_sizes as $image_size ) {
			$this->output .= $this->objOutput->getObjectRender ()->renderNodeHeader ( 'image_type', array (), array (
					'id' => $image_size ['id_image_type'],
					'name' => $image_size ['name'],
					'xlink_resource' => $this->wsObject->wsUrl . 'image_types/' . $image_size ['id_image_type'] 
			), false );
		}
		$this->output .= $this->objOutput->getObjectRender ()->renderNodeFooter ( 'image_types', array () );
		...
		if ($this->imageType == 'products') {
			...
			asort ( $ids );
			foreach ( $ids as $id ) {
				$this->output .= $this->objOutput->getObjectRender ()->renderNodeHeader ( 'image', array (), array (
						'id' => $id,
						'xlink_resource' => $this->wsObject->wsUrl . 'images/' . $this->imageType . '/' . $id 
				), false );
			}
		} else {
	...
	protected function manageEntityDeclinatedImages($directory, $normal_image_sizes) {
		...
		if (! Validate::isUnsignedId ( $object_id )) {
			throw new WebserviceException ( 'The image id is invalid. Please set a valid id or the "default" value', array (
					60,
					400 
			) );
		}
		...
		if ($this->imageType == 'products') {
			...
			if ($this->wsObject->urlSegment [3] != '') {
				...
				if (! Validate::isUnsignedId ( $object_id ) || ! in_array ( $this->wsObject->urlSegment [3], $available_image_ids )) {
					throw new WebserviceException ( 'This image id does not exist', array (
							57,
							400 
					) );
				} else {
			...
			elseif ($this->wsObject->method == 'GET' || $this->wsObject->method == 'HEAD') {
				if ($available_image_ids) {
					...
					foreach ( $available_image_ids as $available_image_id ) {
						$this->output .= $this->objOutput->getObjectRender ()->renderNodeHeader ( 'declination', array (), array (
								'id' => $available_image_id,
								'xlink_resource' => $this->wsObject->wsUrl . 'images/' . $this->imageType . '/' . $object_id . '/' . $available_image_id 
						), false );
					}
		...
		elseif (isset ( $image_size ) && $image_size != '') {
			...
			if ($this->imageType == 'products' && $image_size == 'bin') {
				...
				$exception = new WebserviceException ( 'This image size does not exist', array (
						58,
						400 
				) );
			...
			if (! file_exists ( $filename )) {
				throw new WebserviceException ( 'This image does not exist on disk', array (
						59,
						500 
				) );
			}
	...
	protected function manageCustomizationImages() {
		...
		if (empty ( $this->wsObject->urlSegment [2] )) {
			...
			foreach ( $ids as $id ) {
				$this->output .= $this->objOutput->getObjectRender ()->renderNodeHeader ( 'cart', array (), array (
						'id' => $id,
						'xlink_resource' => $this->wsObject->wsUrl . 'images/' . $this->imageType . '/' . $id 
				), false );
			}
			...
			foreach ( $customizations as $id ) {
				$this->output .= $this->objOutput->getObjectRender ()->renderNodeHeader ( 'customization', array (), array (
						'id' => $id,
						'xlink_resource' => $this->wsObject->wsUrl . 'images/' . $this->imageType . '/' . $id 
				), false );
			}
			...
			if ($this->wsObject->method == 'GET') {
				...
				foreach ( $results as $result ) {
					$this->output .= $this->objOutput->getObjectRender ()->renderNodeHeader ( 'image', array (), array (
							'id' => $result ['index'],
							'xlink_resource' => $this->wsObject->wsUrl . 'images/' . $this->imageType . '/' . $result ['index'] 
					), false );
				}
				$this->output .= $this->objOutput->getObjectRender ()->renderNodeFooter ( 'images', array () );
				return true;
			...
			if ($this->wsObject->method == 'POST') {
				...
				if (! in_array ( ( int ) $this->wsObject->urlSegment [3], $customizations )) {
					throw new WebserviceException ( 'Customization does not exist', array (
							61,
							500 
					) );
				}
				$results = Db::getInstance ()->executeS ( 'SELECT id_customization_field
					...
					AND type = 0' );
				if (empty ( $results )) {
					throw new WebserviceException ( 'Customization field does not exist.', array (
							61,
							500 
					) );
				}
				...
				if (! empty ( $results )) { // customization field exists and has no value
					throw new WebserviceException ( 'Customization field already have a value, please use PUT method.', array (
							61,
							500 
					) );
				}
		...
		switch ($this->wsObject->method) {
			...
			case 'PUT' :
				if ($filename_exists) {
					if ($this->writePostedImageOnDisk ( $filename, null, null, $image_sizes, $directory )) {
						...
						throw new WebserviceException ( 'Unable to save this image.', array (
								62,
								500 
						) );
					}
			...
			case 'DELETE' :
				...
				if ($this->imageType == 'products') {
					...
					if (in_array ( $this->imageType, array (
							'categories',
							'manufacturers',
							'suppliers',
							'stores' 
			...
			case 'POST' :
				...
				if ($filename_exists) {
					throw new WebserviceException ( 'This image already exists. To modify it, please use the PUT method', array (
							65,
							400 
					) );
				} else {
				...
				break;
			default :
				throw new WebserviceException ( 'This method is not allowed', array (
						67,
						405 
				) );
		}
	...
	protected function writeImageOnDisk($base_path, $new_path, $dest_width = null, $dest_height = null, $image_types = null, $parent_path = null) {
		...
		if (! $source_width) {
			throw new WebserviceException ( 'Image width was null', array (
					68,
					400 
			) );
		}
		...
		if (! ($dest_image = imagecreatetruecolor ( $dest_width, $dest_height )) || ! ($white = imagecolorallocate ( $dest_image, 255, 255, 255 )) || ! imagefill ( $dest_image, 0, 0, $white ) || ! imagecopyresampled ( $dest_image, $source_image, $border_width, $border_height, 0, 0, $next_width, $next_height, $source_width, $source_height ) || ! imagecolortransparent ( $dest_image, $white )) {
			throw new WebserviceException ( sprintf ( 'Unable to build the image "%s".', str_replace ( _PS_ROOT_DIR_, '[SHOP_ROOT_DIR]', $new_path ) ), array (
					69,
					500 
			) );
		}
		...
		if (! $imaged) {
			throw new WebserviceException ( sprintf ( 'Unable to write the image "%s".', str_replace ( _PS_ROOT_DIR_, '[SHOP_ROOT_DIR]', $new_path ) ), array (
					70,
					500 
			) );
		}
		...
		if ($image_types) {
			foreach ( $image_types as $image_type ) {
				...
				if (! $this->writeImageOnDisk ( $base_path, $declination_path, $image_type ['width'], $image_type ['height'] )) {
					throw new WebserviceException ( sprintf ( 'Unable to save the declination "%s" of this image.', $image_type ['name'] ), array (
							71,
							500 
					) );
				}
		...
		Hook::exec ( 'actionWatermark', array (
				'id_image' => $this->wsObject->urlSegment [3],
				'id_product' => $this->wsObject->urlSegment [2] 
		) );
		return $new_path;
	}
		...
		if ($this->wsObject->method == 'PUT') {
			if (isset ( $_FILES ['image'] ['tmp_name'] ) && $_FILES ['image'] ['tmp_name']) {
				...
				if ($file ['size'] > $this->imgMaxUploadSize) {
					throw new WebserviceException ( sprintf ( 'The image size is too large (maximum allowed is %d KB)', ($this->imgMaxUploadSize / 1000) ), array (
							72,
							400 
					) );
				}
				...
				if (! $mime_type || ! in_array ( $mime_type, $this->acceptedImgMimeTypes )) {
					throw new WebserviceException ( 'This type of image format is not recognized, allowed formats are: ' . implode ( '", "', $this->acceptedImgMimeTypes ), array (
							73,
							400 
					) );
				}				// Check error while uploading
				elseif ($file ['error']) {
					throw new WebserviceException ( 'Error while uploading image. Please change your server\'s settings', array (
							74,
							400 
					) );
				}
				...
				if (! ($tmp_name = tempnam ( _PS_TMP_IMG_DIR_, 'PS' )) || ! move_uploaded_file ( $_FILES ['image'] ['tmp_name'], $tmp_name )) {
					throw new WebserviceException ( 'Error while copying image to the temporary directory', array (
							75,
							400 
					) );
				...
				throw new WebserviceException ( 'Please set an "image" parameter with image data for value', array (
						76,
						400 
				) );
			}
			...
			if (isset ( $_FILES ['image'] ['tmp_name'] ) && $_FILES ['image'] ['tmp_name']) {
				...
				if ($error = ImageManager::validateUpload ( $file )) {
					throw new WebserviceException ( 'Image upload error : ' . $error, array (
							76,
							400 
					) );
				}
				...
				if (isset ( $file ['tmp_name'] ) && $file ['tmp_name'] != null) {
					if ($this->imageType == 'products') {
						...
						if (! Validate::isLoadedObject ( $product )) {
							throw new WebserviceException ( 'Product ' . ( int ) $this->wsObject->urlSegment [2] . ' does not exist', array (
									76,
									400 
							) );
						}
						...
						if (! $image->add ()) {
							throw new WebserviceException ( 'Error while creating image', array (
									76,
									400 
							) );
						}
					...
					if ($error = ImageManager::validateUpload ( $file, $this->imgMaxUploadSize )) {
						throw new WebserviceException ( 'Bad image : ' . $error, array (
								76,
								400 
						) );
					}
					...
					if ($this->imageType == 'products') {
						...
						if (! ($tmp_name = tempnam ( _PS_TMP_IMG_DIR_, 'PS' )) || ! move_uploaded_file ( $file ['tmp_name'], $tmp_name )) {
							throw new WebserviceException ( 'An error occurred during the image upload', array (
									76,
									400 
							) );
						} elseif (! ImageManager::resize ( $tmp_name, _PS_PROD_IMG_DIR_ . $image->getExistingImgPath () . '.' . $image->image_format )) {
							throw new WebserviceException ( 'An error occurred while copying image', array (
									76,
									400 
							) );
						} else {
						...
						$image_content = array (
								'sqlId' => 'content',
								'value' => base64_encode ( file_get_contents ( $this->imgToDisplay ) ),
								'encode' => 'base64' 
						);
						$this->output .= $this->objOutput->objectRender->renderField ( $image_content );
					} elseif (in_array ( $this->imageType, array (
							'categories',
							'manufacturers',
							'suppliers',
							'stores' 
						...
						if (! ImageManager::resize ( $this->imgToDisplay, $this->imgToDisplay . '_small', $product_picture_width, $product_picture_height )) {
							throw new WebserviceException ( 'An error occurred while resizing image', array (
									76,
									400 
							) );
						}
			...
			throw new WebserviceException ( 'Method ' . $this->wsObject->method . ' is not allowed for an image resource', array (
					77,
					405 
			) );
		}
