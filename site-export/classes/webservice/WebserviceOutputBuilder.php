...
class WebserviceOutputBuilderCore {
	...
	public function setObjectRender(WebserviceOutputInterface $obj_render) {
		if (! $obj_render instanceof WebserviceOutputInterface) {
			throw new WebserviceException ( 'Obj_render param must be an WebserviceOutputInterface object type', array (
					83,
					500 
			) );
		}
	...
	public function setHeaderParams($key, $value) {
		if (! Validate::isCleanHtml ( $key ) || ! Validate::isCleanHtml ( $value )) {
			throw new WebserviceException ( 'the key or your value is corrupted.', array (
					94,
					500 
			) );
		}
	...
	public function getHeaderParams($key = null) {
		...
		if (! is_null ( $key )) {
			if (! Validate::isCleanHtml ( $key )) {
				throw new WebserviceException ( 'the key you write is a corrupted text.', array (
						95,
						500 
				) );
			}
			if (! array_key_exists ( $key, $this->headerParams )) {
				throw new WebserviceException ( sprintf ( 'The key %s does\'nt exist', $key ), array (
						96,
						500 
				) );
			}
	...
	public function getResourcesList($key_permissions) {
		if (is_null ( $this->wsResource )) {
			throw new WebserviceException ( 'You must set web service resource for get the resources list.', array (
					82,
					500 
			) );
		}
		$output = '';
		$more_attr = array (
				'shopName' => htmlspecialchars ( Configuration::get ( 'PS_SHOP_NAME' ) ) 
		);
	...
	public function setSpecificField($object, $method, $field_name, $entity_name) {
		...
		$this->specificFields [$field_name] = array (
				'entity' => $entity_name,
				'object' => $object,
				'method' => $method,
				'type' => gettype ( $object ) 
	...
	protected function validateObjectAndMethod($object, $method) {
		if (is_string ( $object ) && ! class_exists ( $object )) {
			throw new WebserviceException ( 'The object you want to set in ' . __METHOD__ . ' is not allowed.', array (
					98,
					500 
			) );
		}
		if (! method_exists ( $object, $method )) {
			throw new WebserviceException ( 'The method you want to set in ' . __METHOD__ . ' is not allowed.', array (
					99,
					500 
			) );
		}
	...
	public function setVirtualField($object, $method, $entity_name, $parameters) {
		...
		$this->virtualFields [$entity_name] [] = array (
			...
			foreach ( $virtual_fields [$entity_name] as $function_infos ) {
				...
				foreach ( $return_fields as $field_name => $value ) {
					if (Validate::isConfigName ( $field_name )) {
						...
						throw new WebserviceException ( 'Name for the virtual field is not allow', array (
								128,
								400 
						) );
					}
