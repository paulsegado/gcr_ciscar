...
class WebserviceRequestCore {
	 ...
	 * These fields will be displayed when retrieving objects
	 * 
	 * @var string
	 */
	...
	public static function getResources() {
		$resources = array (
				...
				'manufacturers' => array (
						'description' => 'The product manufacturers',
						'class' => 'Manufacturer' 
				),
				...
				'order_discounts' => array (
						'description' => 'Discounts of an order',
						'class' => 'OrderDiscount' 
				),
				...
				'translated_configurations' => array (
						'description' => 'Shop configuration',
				...
				'search' => array (
						'description' => 'Search',
						'specific_management' => true,
						'forbidden_method' => array (
								'PUT',
								'POST',
								'DELETE' 
						) 
				),
				'content_management_system' => array (
				...
				'warehouse_product_locations' => array (
				...
				'supply_order_receipt_histories' => array (
	...
	public function getPriceForProduct($field, $entity_object, $ws_params) {
		if (is_int ( $entity_object->id )) {
			$arr_return = $this->specificPriceForProduct ( $entity_object, array (
					'default_price' => '' 
			) );
	...
	public function specificPriceCalculation($parameters) {
		...
		foreach ( $parameters as $name => $value ) {
			...
			$arr_return [$name] = array (
					'sqlId' => strtolower ( $name ),
					'value' => sprintf ( '%f', $return_value ) 
			);
		}
		return $arr_return;
	}
	...
	protected function manageFilters() {
		...
		foreach ( $this->resourceConfiguration ['fields'] as $fieldName => $field ) {
			if ((! isset ( $this->resourceConfiguration ['hidden_fields'] ) || (isset ( $this->resourceConfiguration ['hidden_fields'] ) && ! in_array ( $fieldName, $this->resourceConfiguration ['hidden_fields'] )))) {
		...
		if (! empty ( $this->urlFragments ['date'] ) && $this->urlFragments ['date']) {
			...
			if (! array_key_exists ( 'date_add', $this->resourceConfiguration ['fields'] )) {
				$this->resourceConfiguration ['fields'] ['date_add'] = array (
						'sqlId' => 'date_add' 
				);
			}
			if (! array_key_exists ( 'date_upd', $this->resourceConfiguration ['fields'] )) {
				$this->resourceConfiguration ['fields'] ['date_upd'] = array (
						'sqlId' => 'date_upd' 
				);
			}
		} else {
		...
		if (isset ( $this->urlFragments ['sort'] )) {
			...
			foreach ( $sorts as $sort ) {
				...
				if ($delimiterPosition === false || ! in_array ( $direction, array (
						'ASC',
						'DESC' 
				) )) {
	...
	public function getFilteredObjectList() {
		...
		if (isset ( $this->urlFragments ['schema'] ) && in_array ( $this->urlFragments ['schema'], array (
				'blank',
				'synopsis' 
		) )) {
			$filters = array (
		...
		$sqlObjects = call_user_func_array ( array (
				$tmp,
				$this->resourceConfiguration ['retrieveData'] ['retrieveMethod'] 
		), $this->resourceConfiguration ['retrieveData'] ['params'] );
		if ($sqlObjects) {
	...
	public function filterLanguage() {
		...
		elseif (strpos ( $this->urlFragments ['language'], '[' ) === 0 && strpos ( $this->urlFragments ['language'], ']' ) === $length_values - 1) {
			if (strpos ( $this->urlFragments ['language'], '|' ) !== false xor strpos ( $this->urlFragments ['language'], ',' ) !== false) {
				$params_values = str_replace ( array (
						']',
						'[' 
				), '', $this->urlFragments ['language'] );
				// it's a list
