...
abstract class PaymentModuleCore extends Module {
	...
	public function addCheckboxCountryRestrictionsForModule(array $shops = array()) {
		...
		return Country::addModuleRestrictions ( $shops, $countries, array (
				array (
...
//<!-- CISCAR on stock tous les transporteurs dans un tableau						
			$carrier_list = array ();
...
//<!-- CISCAR on determine le nom du transporteur
			if (count($carrier_list)>1)	
				$carrier_name =  Mail::l ( 'To be compute' );
			else 
				$carrier_name = Mail::l ( $carrier->name );
						
			
//!-- CISCAR gestion de la rupture sur numero de reference pour n'envoyer qu'un seul mail			
			$reference_prec = '';
...
//<!-- CISCAR liste des produits par rapport au panier et non pas par rapport à la commande	
...
//<!--CISCAR on récupère les totaux par rapport au panier et non pas par rapport à la commande						
					$order->total_products = ( float ) $this->context->cart->getOrderTotal ( false, Cart::ONLY_PRODUCTS, $order->product_list,null );
					$order->total_products_wt = ( float ) $this->context->cart->getOrderTotal ( true, Cart::ONLY_PRODUCTS, $order->product_list,null);
					$order->total_discounts_tax_excl = ( float ) abs ( $this->context->cart->getOrderTotal ( false, Cart::ONLY_DISCOUNTS, $order->product_list,null ) );
					$order->total_discounts_tax_incl = ( float ) abs ( $this->context->cart->getOrderTotal ( true, Cart::ONLY_DISCOUNTS, $order->product_list,null ) );
					$order->total_discounts = $order->total_discounts_tax_incl;
					
					$order->total_shipping_tax_excl = ( float ) $this->context->cart->getPackageShippingCost ( null, false, null, $order->product_list );
					$order->total_shipping_tax_incl = ( float ) $this->context->cart->getPackageShippingCost ( null, true, null, $order->product_list );
					$order->total_shipping = $order->total_shipping_tax_incl;
					...
					$order->total_wrapping_tax_excl = ( float ) abs ( $this->context->cart->getOrderTotal ( false, Cart::ONLY_WRAPPING, $order->product_list,null ) );
					$order->total_wrapping_tax_incl = ( float ) abs ( $this->context->cart->getOrderTotal ( true, Cart::ONLY_WRAPPING, $order->product_list,null ) );
					$order->total_wrapping = $order->total_wrapping_tax_incl;
					
					$order->total_paid_tax_excl = ( float ) Tools::ps_round ( ( float ) $this->context->cart->getOrderTotal ( false, Cart::BOTH, $order->product_list, null ), _PS_PRICE_COMPUTE_PRECISION_ );
					$order->total_paid_tax_incl = ( float ) Tools::ps_round ( ( float ) $this->context->cart->getOrderTotal ( true, Cart::BOTH, $order->product_list, null ), _PS_PRICE_COMPUTE_PRECISION_ );
					$order->total_paid = $order->total_paid_tax_incl;
//<!-- FIN CISCAR	

//!-- CISCAR gestion de la rupture sur numero de reference pour n'envoyer qu'un seul mail
//           Si numéro de référence identique on n'envoi pas le mail
						if ($reference_prec !== $order->reference)
						{	
							$reference_prec = $order->reference;
							$data = array (
									'{firstname}' => $this->context->customer->firstname,
									'{lastname}' => $this->context->customer->lastname,
...
//<!-- CISCAR on renseigne la traduction du transporteur -->									
...
//									'{carrier}' => ($virtual_product || ! isset ( $carrier->name )) ? Tools::displayError ( 'No carrier' ) : $carrier->name,
									...
									//CISCAR, on formate le numero de commande
									//'{id_order_num}' => 'EW-'.str_pad($order->id,6,'0',STR_PAD_LEFT)
									'{id_order_num}' => $order->reference
									//FIN CISCAR
									);
						...
						} //Fin CISCAR $reference_prec != $order->reference
					}
	...
	protected function _getFormatedAddress(Address $the_address, $line_sep, $fields_style = array()) {
		return AddressFormat::generateAddress ( $the_address, array (
				'avoid' => array () 
		), $line_sep, ' ', $fields_style );
	}
	
	/**
	 *
	 * @param int $id_currency
	 *        	: this parameter is optionnal but on 1.5 version of Prestashop, it will be REQUIRED
	 * @return Currency
	 */
	...
	public static function getInstalledPaymentModules() {
		...
		LEFT JOIN `' . _DB_PREFIX_ . 'hook_module` hm ON hm.`id_module` = m.`id_module`' . Shop::addSqlRestriction ( false, 'hm' ) . '
