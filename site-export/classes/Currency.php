...
class CurrencyCore extends ObjectModel {
	...
	public function getSign($side = null) {
		...
		$formats = array (
				1 => array (
						'left' => &$formated_strings ['left'],
						'right' => '' 
				),
				2 => array (
						'left' => '',
						'right' => &$formated_strings ['right'] 
				),
				3 => array (
						'left' => &$formated_strings ['left'],
						'right' => '' 
	...
	public static function getCurrencies($object = false, $active = true, $group_by = false) {
		...
		' . Shop::addSqlAssociation ( 'currency', 'c' ) . ' WHERE `deleted` = 0' . ($active ? ' AND c.`active` = 1' : '') . ($group_by ? ' GROUP BY c.`id_currency`' : '') . ' ORDER BY `name` ASC' );
		if ($object) {
