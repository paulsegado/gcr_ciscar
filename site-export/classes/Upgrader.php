...
class UpgraderCore {
	 ...
	 *        	new filename
	 * @return bool @TODO ftp if copy is not possible (safe_mode for example)
	 */
	...
	/**
	 * checkPSVersion ask to prestashop.com if there is a new version.
	 * return an array if yes, false otherwise
	 *
	 * @return mixed
	 */
	 ...
	 * 
	 * @param string $path
	 *        	filepath to add, relative to _PS_ROOT_DIR_
	 */
	protected function addChangedFile($path) {
		...
		} elseif (strpos ( $path, '/en.php' ) !== false || strpos ( $path, '/fr.php' ) !== false || strpos ( $path, '/es.php' ) !== false || strpos ( $path, '/it.php' ) !== false || strpos ( $path, '/de.php' ) !== false || strpos ( $path, 'translations/' ) !== false) {
