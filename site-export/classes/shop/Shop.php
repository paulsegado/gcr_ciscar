...
class ShopCore extends ObjectModel {
	...
	protected static function init() {
		Shop::$id_shop_default_tables = array (
				'product',
				'category' 
		);
	 ...
	 * Get shop URL
	 *
	 * @param string $auto_secure_mode
	 *        	if set to true, secure mode will be checked
	 * @param string $add_base_uri
	 *        	if set to true, shop base uri will be added
	...
	/**
	 * Get a list of ID concerned by the shop context (E.g.
	 * if context is shop group, get list of children shop ID)
	 *
	 * @param string $share
	...
	public static function getContextListShopID($share = false) {
		if (Shop::getContext () == Shop::CONTEXT_SHOP) {
			$list = ($share) ? Shop::getSharedShops ( Shop::getContextShopID (), $share ) : array (
	...
	public static function isFeatureActive() {
		...
		if ($feature_active === null) {
			$feature_active = ( bool ) Db::getInstance ()->getValue ( 'SELECT value FROM `' . _DB_PREFIX_ . 'configuration` WHERE `name` = "PS_MULTISHOP_FEATURE_ACTIVE"' ) && (Db::getInstance ()->getValue ( 'SELECT COUNT(*) FROM ' . _DB_PREFIX_ . 'shop' ) > 1);
		}
