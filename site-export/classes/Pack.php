...
class PackCore extends Product {
	 ...
	 * @param
	 *        	$id_product
	 * @param $id_product_attribute Optional
	...
	public static function deleteItems($id_product) {
		return Db::getInstance ()->update ( 'product', array (
				'cache_is_pack' => 0 
		), 'id_product = ' . ( int ) $id_product ) && Db::getInstance ()->execute ( 'DELETE FROM `' . _DB_PREFIX_ . 'pack` WHERE `id_product_pack` = ' . ( int ) $id_product ) && Configuration::updateGlobalValue ( 'PS_PACK_FEATURE_ACTIVE', Pack::isCurrentlyUsed () );
	}
	...
	public static function addItem($id_product, $id_item, $qty, $id_attribute_item = 0) {
		...
		return Db::getInstance ()->update ( 'product', array (
				'cache_is_pack' => 1 
		), 'id_product = ' . ( int ) $id_product ) && Db::getInstance ()->insert ( 'pack', array (
	 ...
	 *        	Product item id that could be contained in a|many pack(s)
	 * @param integer $id_attribute_item
	 *        	The declinaison of the product
