...
//<!-- CISCAR en autologin, on force le reconnexion et on initialise le cookie qui permet de stocker le site appelant
		if (isset($_GET['autologin']))
		{
			//$this->context->cookie->from_url = '';
			setcookie("from_url", '',0);
			if (isset($_GET['autolang']))
			{
				//$this->context->cookie->from_url = 'NL';
				setcookie("from_url", 'NL',0);
			}
			if (isset($_GET['autolangBE']))
			{
				//$this->context->cookie->from_url = 'BE';
				setcookie("from_url", 'BE',0);
			}
		...
		$this->context->smarty->assign ( array (
				...
				'come_from' => Tools::getHttpHost ( true, true ) . Tools::htmlentitiesUTF8 ( str_replace ( array (
						'\'',
						'\\' 
	 ...
	 *
	 * @param bool $display
	 *        	If true, renders visual page header section
	 * @deprecated 1.5.0.1
	 */
	 ...
	 *
	 * @param string|array $media_uri
	 *        	Path to file, or an array of paths like: array(array(uri => media_type), ...)
	 * @param string|null $css_media_type
	 *        	CSS media type
	 * @param int|null $offset        	
	 * @param bool $remove
	 *        	If True, removes media files
	 * @param bool $check_path
	 *        	If true, checks if files exists
	 * @return true|void
	 */
	 ...
	 *
	 * @param string|array $media_uri
	 *        	Path to file, or an array paths of like: array(array(uri => media_type), ...)
	 * @param string|null $css_media_type
	 *        	CSS media type
	 * @param bool $check_path
	 *        	If true, checks if files exists
	 ...
	 *
	 * @param array|string $css_uri
	 *        	$media_uri Path to file, or an array of paths like: array(array(uri => media_type), ...)
	 ...
	 *
	 * @param array|string $js_uri
	 *        	Path to file, or an array of paths
	 * @param bool $check_path
	 *        	If true, checks if files exists
	 * @return true|void
	 */
	...
	public function initLogoAndFavicon() {
		...
		//<!-- CISCAR, modifie l'URL du logo
		foreach ( $this->getCurrentCustomerGroups() as $row ) {
			if( $row >= 5 && $row < 15)
			{
				$ps_logo_group = 'PS_LOGO_GROUP_'.$row ;
				$logo = $this->context->link->getMediaLink ( _PS_IMG_ . Configuration::get ( $ps_logo_group ) );
			}
			else
			{
			    if( $row < 15)
				    $logo = $this->context->link->getMediaLink ( _PS_IMG_ . Configuration::get ( 'PS_LOGO_GROUP_1' ) );
			}
		}
