...
abstract class ModuleCore {
	...
	/**
	 * @var string Module web path (eg.
	 * '/shop/modules/modulename/')
	 */
	...
	public static $hosted_modules_blacklist = array (
			'autoupgrade' 
	);
	...
	public function install() {
		Hook::exec ( 'actionModuleInstallBefore', array (
				'object' => $this 
		) );
		...
		Hook::exec ( 'actionModuleInstallAfter', array (
				'object' => $this 
		) );
	...
	public function updateModuleTranslations() {
		return Language::updateModulesTranslations ( array (
				$this->name 
		) );
	}
...
;
	...
	public static function getUpgradeStatus($module_name) {
		return (isset ( self::$modules_cache [$module_name] ) && self::$modules_cache [$module_name] ['upgrade'] ['success']);
	}
	...
	/**
	 * This function enable module $name.
	 * If an $name is an array,
	...
	public function enableDevice($device) {
		Db::getInstance ()->execute ( '
			...
			WHERE (enable_device &~ ' . ( int ) $device . ' OR enable_device = 0) AND id_module=' . ( int ) $this->id . Shop::addSqlRestriction () );
	...
	public function disableDevice($device) {
		Db::getInstance ()->execute ( '
			UPDATE ' . _DB_PREFIX_ . 'module_shop
			SET enable_device = enable_device - ' . ( int ) $device . '
			WHERE enable_device & ' . ( int ) $device . ' AND id_module=' . ( int ) $this->id . Shop::addSqlRestriction () );
		
		return true;
	}
	
	/**
	 * This function disable module $name.
	 * If an $name is an array,
	...
	public function registerHook($hook_name, $shop_list = null) {
		...
		foreach ( $hook_names as $hook_name ) {
			...
			Hook::exec ( 'actionModuleRegisterHookBefore', array (
					'object' => $this,
					'hook_name' => $hook_name 
			) );
			// Get hook id
			...
			Hook::exec ( 'actionModuleRegisterHookAfter', array (
					'object' => $this,
					'hook_name' => $hook_name 
			) );
		}
	 ...
	 *
	 * @param mixed $id_hook
	 *        	Hook id (can be a hook name since 1.5.0)
	 * @param array $shop_list
	 *        	List of shop
	 * @return bool result
	 */
	public function unregisterHook($hook_id, $shop_list = null) {
		...
		Hook::exec ( 'actionModuleUnRegisterHookBefore', array (
				'object' => $this,
				'hook_name' => $hook_name 
		) );
		...
		$sql = 'DELETE FROM `' . _DB_PREFIX_ . 'hook_module`
			WHERE `id_module` = ' . ( int ) $this->id . ' AND `id_hook` = ' . ( int ) $hook_id . (($shop_list) ? ' AND `id_shop` IN(' . implode ( ', ', array_map ( 'intval', $shop_list ) ) . ')' : '');
		$result = Db::getInstance ()->execute ( $sql );
		
		// Clean modules position
		$this->cleanPositions ( $hook_id, $shop_list );
		
		Hook::exec ( 'actionModuleUnRegisterHookAfter', array (
				'object' => $this,
				'hook_name' => $hook_name 
		) );
	...
	public function editExceptions($id_hook, $excepts) {
		...
		foreach ( $excepts as $shop_id => $except ) {
			$shop_list = ($shop_id == 0) ? Shop::getContextListShopID () : array (
					$shop_id 
			);
	...
	public static function getModulesOnDisk($use_config = false, $logged_on_addons = false, $id_employee = false) {
		...
		$files_list = array (
				array (
						'type' => 'addonsNative',
						'file' => _PS_ROOT_DIR_ . self::CACHE_FILE_DEFAULT_COUNTRY_MODULES_LIST,
						'loggedOnAddons' => 0 
				),
				array (
						'type' => 'addonsMustHave',
						'file' => _PS_ROOT_DIR_ . self::CACHE_FILE_MUST_HAVE_MODULES_LIST,
						'loggedOnAddons' => 0 
				),
				array (
						'type' => 'addonsBought',
						'file' => _PS_ROOT_DIR_ . self::CACHE_FILE_CUSTOMER_MODULES_LIST,
						'loggedOnAddons' => 1 
				) 
		);
	...
	public static function getNonNativeModuleList() {
		...
		if (is_array ( $native_modules )) {
			foreach ( $native_modules as $native_modules_type ) {
				if (in_array ( $native_modules_type ['type'], array (
						'native',
						'partner' 
				) )) {
					$arr_native_modules [] = '""';
	 ...
	 *
	 * @param string $name
	 *        	The module name (the folder name)
	 * @param string $key
	 *        	The key provided by addons
	 * @return int
	 */
	...
	public function updatePosition($id_hook, $way, $position = null) {
		foreach ( Shop::getContextListShopID () as $shop_id ) {
			...
			$sql = 'UPDATE `' . _DB_PREFIX_ . 'hook_module`
				...
				WHERE position between ' . ( int ) (min ( array (
					$from ['position'],
					$to ['position'] 
			) )) . ' AND ' . max ( array (
					$from ['position'],
	...
	protected function _clearCache($template, $cache_id = null, $compile_id = null) {
		...
		if (self::$_batch_mode) {
			...
			if (! isset ( self::$_defered_clearCache [$key] )) {
				self::$_defered_clearCache [$key] = array (
...
<module>
	...
	<author><![CDATA[' . str_replace ( '&amp;', '&', Tools::htmlentitiesUTF8 ( $this->author ) ) . ']]></author>' . $author_uri . '
	...
	public function isHookableOn($hook_name) {
		...
		return (is_callable ( array (
				$this,
				'hook' . ucfirst ( $hook_name ) 
		) ) || is_callable ( array (
				$this,
				'hook' . ucfirst ( $retro_hook_name ) 
		) ));
	}
	...
	public function addOverride($classname) {
		...
		if ($file = PrestaShopAutoload::getInstance ()->getClassPath ( $classname )) {
			...
			eval ( preg_replace ( array (
					...
					' ',
					'class ' . $classname . 'OverrideOriginal' . $uniq 
	...
	public function removeOverride($classname) {
		...
		if ($orig_path) {
			...
			eval ( preg_replace ( array (
					...
					' ',
					'class ' . $classname . 'OverrideOriginal_remove' . $uniq 
			), implode ( '', $override_file ) ) );
			...
			eval ( preg_replace ( array (
					...
					' ',
					'class ' . $classname . 'Override_remove' . $uniq 
			), implode ( '', $module_file ) ) );
