...
class TaxRulesGroupCore extends ObjectModel {
	...
	public function historize(TaxRulesGroup $tax_rules_group) {
		...
		WHERE id_tax_rules_group=' . ( int ) $this->id ) && Db::getInstance ()->execute ( '
	...
	public static function getTaxRulesGroups($only_active = true) {
		return Db::getInstance ()->executeS ( '
			...
			FROM `' . _DB_PREFIX_ . 'tax_rules_group` g' . Shop::addSqlAssociation ( 'tax_rules_group', 'g' ) . ' WHERE deleted = 0' . ($only_active ? ' AND g.`active` = 1' : '') . '
			ORDER BY name ASC' );
	}
	...
	public static function getTaxRulesGroupsForOptions() {
		$tax_rules [] = array (
				...
				'name' => Tools::displayError ( 'No tax' ) 
		);
