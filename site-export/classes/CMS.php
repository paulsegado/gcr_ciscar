...
//<!-- CISCAR affectations groupes a CMS -->	
		$success = parent::add ( $autodate, true );
...
//<!-- CISCAR affectations groupes a CMS -->
	...
	public function update($null_values = false) {
		if (parent::update ( $null_values )) {
			//<!-- CISCAR affectations groupes a CMS -->			
			$this->updateGroup ( $this->groupBox );	
//<!-- CISCAR FIN affectations groupes a CMS -->			
	...
	public function delete() {
		if (parent::delete ()) {
//<!-- CISCAR affectations groupes a CMS -->			
			Db::getInstance ()->execute ( 'DELETE FROM `' . _DB_PREFIX_ . 'cms_group` WHERE `id_cms` = ' . ( int ) $this->id );
//<!-- CISCAR FIN affectations groupes a CMS -->			
			return $this->cleanPositions ( $this->id_cms_category );
		}
		return false;
	...
	public static function getLinks($id_lang, $selection = null, $active = true, Link $link = null) {
		...
		WHERE 1
		' . (($selection !== null) ? ' AND c.id_cms IN (' . implode ( ',', array_map ( 'intval', $selection ) ) . ')' : '') . ($active ? ' AND c.`active` = 1 ' : '') . 'GROUP BY c.id_cms
		ORDER BY c.`position`' );
	...
	public function updatePosition($way, $position) {
		...
		return (Db::getInstance ()->execute ( '
			...
			WHERE `position`
			' . ($way ? '> ' . ( int ) $moved_cms ['position'] . ' AND `position` <= ' . ( int ) $position : '< ' . ( int ) $moved_cms ['position'] . ' AND `position` >= ' . ( int ) $position) . '
			AND `id_cms_category`=' . ( int ) $moved_cms ['id_cms_category'] ) && Db::getInstance ()->execute ( '
			UPDATE `' . _DB_PREFIX_ . 'cms`
...
//<!-- CISCAR on recupère que les CMS liés aux  groupes de l'utilisateur -->
		$sql->where (' c.id_cms in (SELECT id_cms from ps_cms_group where id_group in ('.implode(',',$customer_groups). ') and id_cms = c.id_cms)');
...
//<!-- CISCAR affectations groupes a CMS -->
	protected static $_cms_groups = array ();
	public $groupBox;
	...
	public function cleanGroups() {
		return Db::getInstance ()->delete ( 'cms_group', 'id_cms = ' . ( int ) $this->id );
	}
	...
	public function addGroups($groups) {
		foreach ( $groups as $group ) {
			...
			Db::getInstance ()->insert ( 'cms_group', $row, false, true, Db::INSERT_IGNORE );
		}
	}
	
	public static function getGroupsStatic($id_cms) {
		
			self::$_cms_groups [$id_cms] = array ();
			$result = Db::getInstance ()->executeS ( '
			SELECT cg.`id_group`
			FROM ' . _DB_PREFIX_ . 'cms_group cg
			WHERE cg.`id_cms` = ' . ( int ) $id_cms );
			foreach ( $result as $group ) {
				self::$_cms_groups [$id_cms] [] = ( int ) $group ['id_group'];
			}

		return self::$_cms_groups [$id_cms];
	}
	
	public static function getGroupsCategory($id_category) {
		
			self::$_cms_groups [$id_category] = array ();
			$result = Db::getInstance ()->executeS ( '
			SELECT DISTINCT cg.`id_group`
			FROM ' . _DB_PREFIX_ . 'cms_group cg, '. _DB_PREFIX_ .'cms c
			WHERE cg.`id_cms` = c.`id_cms` and c.`id_cms_category` = ' . ( int ) $id_category );
			foreach ( $result as $group ) {
				self::$_cms_groups [$id_category] [] = ( int ) $group ['id_group'];
			}

		return self::$_cms_groups [$id_category];
	}
	
	public function getGroups() {
		return Cms::getGroupsStatic ( ( int ) $this->id );
	}
//<!-- CISCAR FIN affectations groupes a CMS -->	
