...
class AttributeCore extends ObjectModel {
	...
	public function delete() {
		...
		if ($return) {
			Hook::exec ( 'actionAttributeDelete', array (
	...
	public function update($null_values = false) {
		...
		if ($return) {
			Hook::exec ( 'actionAttributeSave', array (
					'id_attribute' => $this->id 
			) );
		}
	 ...
	 *        	Language id
	 * @param bool $notNull
	 *        	Get only not null fields if true
	...
	public function updatePosition($way, $position) {
		...
		$res1 = Db::getInstance ()->execute ( '
			...
			WHERE `position`
			' . ($way ? '> ' . ( int ) $moved_attribute ['position'] . ' AND `position` <= ' . ( int ) $position : '< ' . ( int ) $moved_attribute ['position'] . ' AND `position` >= ' . ( int ) $position) . '
