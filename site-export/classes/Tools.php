...
class ToolsCore {
	 ...
	 * @param string $flag
	 *        	Output type (NUMERIC, ALPHANUMERIC, NO_NUMERIC, RANDOM)
	...
	public static function getShopProtocol() {
		$protocol = (Configuration::get ( 'PS_SSL_ENABLED' ) || (! empty ( $_SERVER ['HTTPS'] ) && Tools::strtolower ( $_SERVER ['HTTPS'] ) != 'off')) ? 'https://' : 'http://';
		return $protocol;
	}
	 ...
	 * @param bool $entities
	 *        	if true, convert special chars to HTML entities
	...
	public static function usingSecureMode() {
		if (isset ( $_SERVER ['HTTPS'] )) {
			return in_array ( Tools::strtolower ( $_SERVER ['HTTPS'] ), array (
					1,
					'on' 
			) );
		}
		...
		if (isset ( $_SERVER ['SSL'] )) {
			return in_array ( Tools::strtolower ( $_SERVER ['SSL'] ), array (
					1,
					'on' 
			) );
		}
		...
		if (isset ( $_SERVER ['REDIRECT_HTTPS'] )) {
			return in_array ( Tools::strtolower ( $_SERVER ['REDIRECT_HTTPS'] ), array (
					1,
					'on' 
			) );
		}
		if (isset ( $_SERVER ['HTTP_SSL'] )) {
			return in_array ( Tools::strtolower ( $_SERVER ['HTTP_SSL'] ), array (
					1,
					'on' 
			) );
		}
	...
	public static function displayNumber($number, $currency) {
		...
		return number_format ( $number, 0, '.', in_array ( $format, array (
				1,
				4 
		) ) ? ',' : ' ' );
	}
	 ...
	 *        	String to sanitize
	 * @param bool $full
	 *        	String contains HTML or not (optional)
	...
	public static function debug_backtrace($start = 0, $limit = null) {
		...
		foreach ( $backtrace as $id => $trace ) {
			...
			$relative_file = (isset ( $trace ['file'] )) ? 'in /' . ltrim ( str_replace ( array (
					_PS_ROOT_DIR_,
					'\\' 
			), array (
					'',
	 ...
	 * @param bool $entites
	 *        	Set to true to use htmlentities function on URL param
	 */
	 ...
	 *        	Image name
	 * @param bool $entites
	 *        	Set to true to use htmlentities function on image param
	 */
	 ...
	 * @param string $path
	 *        	Path end
	 * @param bool $linkOntheLastItem
	 *        	Put or not a link on the current category
	 * @param
	 *        	string [optionnal] $categoryType defined what type of categories is used (products or cms)
	 */
	 ...
	 *
	 * @param
	 *        	string [optionnal] $type_cat defined what type of categories is used (products or cms)
	 */
...
';
	...
	public static function getSafeModeStatus() {
		...
		return in_array ( Tools::strtolower ( $safe_mode ), array (
				1,
				'on' 
		) );
	}
	 ...
	 *
	 * @param
	 *        	s string $path Path to scan
	 * @param
	 *        	s string $ext Extention to filter files
	 * @param
	 *        	s string $dir Add this to prefix output for example /path/dir/*
	 *        	
	...
	public static function addonsRequest($request, $params = array()) {
		...
		switch ($request) {
			...
			case 'must-have-themes' :
				...
				break;
			case 'customer' :
				$post_data .= '&method=listing&action=customer&username=' . urlencode ( trim ( Context::getContext ()->cookie->username_addons ) ) . '&password=' . urlencode ( trim ( Context::getContext ()->cookie->password_addons ) );
				break;
			case 'customer_themes' :
				$post_data .= '&method=listing&action=customer-themes&username=' . urlencode ( trim ( Context::getContext ()->cookie->username_addons ) ) . '&password=' . urlencode ( trim ( Context::getContext ()->cookie->password_addons ) );
				break;
			case 'check_customer' :
			...
			case 'module' :
				...
				break;
			case 'hosted_module' :
				$post_data .= '&method=module&id_module=' . urlencode ( ( int ) $params ['id_module'] ) . '&username=' . urlencode ( $params ['hosted_email'] ) . '&password=' . urlencode ( $params ['password_addons'] ) . '&shop_url=' . urlencode ( isset ( $params ['shop_url'] ) ? $params ['shop_url'] : Tools::getShopDomain () ) . '&mail=' . urlencode ( isset ( $params ['email'] ) ? $params ['email'] : Configuration::get ( 'PS_SHOP_EMAIL' ) );
				$protocols [] = 'https';
				break;
	 ...
	 *        	If true, returns uploaded file contents
	 *        	
	 * @return array|null
	 */
	 ...
	 *
	 * @param $amount float
	 *        	The amount to spread across the rows
	 * @param $precision int
	 *        	Rounding precision
	 ...
	 * @param
	 *        	&$rows array An array, associative or not, containing arrays that have at least $column and $sort_column fields
	 * @param $column string
	 *        	The column on which to perform adjustments
	 */
