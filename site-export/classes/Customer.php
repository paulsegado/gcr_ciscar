...
//<!-- CISCAR pour conserver le mote de passe en clair -->
		/**
	 * @var string Pwdclear
	 */
	public $pwdclear;
	...
	/**
	 * @var string Object Login CISCAR
	 */
	public $login_ciscar;
	public $years;
	public $days;
...
//<!-- CISCAR pour conserver le mote de passe en clair -->
					...
					'siret' => array (
							'type' => self::TYPE_STRING,
//<!-- CISCAR on ne controle plus le siret  qui est remplacé par la tva intra communautaire-->							
//							'validate' => 'isSiret' 
							'validate' => 'isGenericName',
					),
					'ape' => array (
							'type' => self::TYPE_STRING,
//<!-- CISCAR on ne controle plus le code APE   qui est remplacé par la Fonction-->							
//							'validate' => 'isApe' 
							'validate' => 'isGenericName',
					),
			...
			// 'login_ciscar' => array('type' => self::TYPE_STRING, 'validate' => 'isLogin_Ciscar', 'copy_post' => false),
	 ...
	 *
	 * @param null|bool $only_active
	 *        	Returns only active customers when true
	 * @return array Customers
	 */
	public static function getCustomers($only_active = null) {
		$sql = 'SELECT `id_customer`, `email`, `firstname`, `lastname`
				FROM `' . _DB_PREFIX_ . 'customer`
				WHERE 1 ' . Shop::addSqlRestriction ( Shop::SHARE_CUSTOMER ) . ($only_active ? ' AND `active` = 1' : '') . '
	 ...
	 * @param $ignore_guest boolean,
	 *        	to exclude guest customer
	...
	public function logout() {
		Hook::exec ( 'actionCustomerLogoutBefore', array (
				'customer' => $this 
		) );
		...
		Hook::exec ( 'actionCustomerLogoutAfter', array (
				'customer' => $this 
		) );
	}
