...
class StockAvailableCore extends ObjectModel {
			...
			if (StockAvailable::dependsOnStock ( $id_product, $id_shop )) {
				...
				if (empty ( $ids_product_attribute )) {
					...
					foreach ( $ids_product_attribute as $id_product_attribute ) {
						...
						if (($order_id_shop != null && ! count ( array_intersect ( $allowed_warehouse_for_combination_clean, $order_warehouses ) )) || ! count ( $allowed_warehouse_for_combination_clean )) {
							continue;
						}
						...
						if (( int ) Db::getInstance ( _PS_USE_SQL_SLAVE_ )->getValue ( $query )) {
							$query = array (
									...
									'where' => 'id_product = ' . ( int ) $id_product . ' AND id_product_attribute = ' . ( int ) $id_product_attribute . StockAvailable::addSqlShopRestriction ( null, $id_shop ) 
							);
				...
				if (($order_id_shop != null && array_intersect ( $warehouses, $order_warehouses )) || $order_id_shop == null) {
					$query = array (
							...
							'where' => 'id_product = ' . ( int ) $id_product . ' AND id_product_attribute = 0' . StockAvailable::addSqlShopRestriction ( null, $id_shop ) 
					);
					Db::getInstance ()->update ( $query ['table'], $query ['data'], $query ['where'] );
				}
			}
	...
	public static function setProductOutOfStock($id_product, $out_of_stock = false, $id_shop = null, $id_product_attribute = 0) {
		...
		if ($existing_id > 0) {
			...
			), 'id_product = ' . ( int ) $id_product . (($id_product_attribute) ? ' AND id_product_attribute = ' . ( int ) $id_product_attribute : '') . StockAvailable::addSqlShopRestriction ( null, $id_shop ) );
		} else {
			$params = array (
					'out_of_stock' => ( int ) $out_of_stock,
					'id_product' => ( int ) $id_product,
	 ...
	 *        	Shop id or shop object Optional
	 *        	
	 * @return bool
	 */
	...
	public static function resetProductFromStockAvailableByShopGroup(ShopGroup $shop_group) {
		...
		if (count ( $shop_list ) > 0) {
			...
			return Db::getInstance ()->update ( 'stock_available', array (
					'quantity' => 0 
