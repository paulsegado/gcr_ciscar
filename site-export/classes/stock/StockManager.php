...
class StockManagerCore implements StockManagerInterface {
	 ...
	 */
	public function addProduct($id_product, $id_product_attribute = 0, Warehouse $warehouse, $quantity, $id_stock_mvt_reason, $price_te, $is_usable = true, $id_supply_order = null, $employee = null) {
	 ...
	 * @throws PrestaShopException
	 */
	public function removeProduct($id_product, $id_product_attribute = null, Warehouse $warehouse, $quantity, $id_stock_mvt_reason, $is_usable = true, $id_order = null, $ignore_pack = 0, $employee = null, Stock $stock = null) {
		...
		if ($this->shouldHandleStockOperationForProductsPack ( $id_product, $ignore_pack )) {
			if (Validate::isLoadedObject ( $product = new Product ( ( int ) $id_product ) )) {
							...
							foreach ( $product_warehouses as $product_warehouse ) {
								if (! $warehouse_stock_found) {
									if (Warehouse::exists ( $product_warehouse ['id_warehouse'] )) {
										...
										$removedProducts [] = $this->removeProduct ( $product_pack->id, $product_pack->id_pack_product_attribute, $current_warehouse, $product_pack->pack_quantity * $quantity, $id_stock_mvt_reason, $is_usable, $id_order );
				...
				if ($product->pack_stock_type == 0 || $product->pack_stock_type == 2 || ($product->pack_stock_type == 3 && (Configuration::get ( 'PS_PACK_STOCK_TYPE' ) == 0 || Configuration::get ( 'PS_PACK_STOCK_TYPE' ) == 2))) {
					$removedProducts = array_merge ( $removedProducts, $this->removeProduct ( $id_product, $id_product_attribute, $warehouse, $quantity, $id_stock_mvt_reason, $is_usable, $id_order, 1 ) );
				}
			} else {
				return false;
			}
		} else {
			$quantity_in_stock = $this->computeProductQuantityInStock ( $warehouse, $id_product, $id_product_attribute, $is_usable, $stock );
			
			if ($this->ensureProductQuantityRequestedForRemovalIsValid ( $quantity, $quantity_in_stock )) {
				return $removedProducts;
			}
			
			$stock_collection = $this->getProductStockLinesInWarehouse ( $id_product, $id_product_attribute, $warehouse, $stock );
			...
			switch ($warehouse->management_type) {
				...
				case 'WA' :
					...
					$this->removeProductQuantityApplyingCump ( $quantity, $id_stock_mvt_reason, $is_usable, $id_order, $employee, $stock );
			...
			if (Pack::isPacked ( $id_product, $id_product_attribute )) {
				...
				foreach ( $packs as $pack ) {
					...
					$max_pack_quantity = max ( array (
							0,
							floor ( $stock_available_quantity / $quantity_by_pack ) 
					) );
		...
		$this->hookCoverageOnProductRemoval ( $warehouse, $id_product, $id_product_attribute, $is_usable );
	...
	public function getPhysicalProductQuantities($productStockCriteria) {
		...
		return ( int ) $this->getProductPhysicalQuantities ( $productStockCriteria ['product_id'], $productStockCriteria ['product_attribute_id'], $productStockCriteria ['warehouse_id'] );
	}
	...
	public function getUsableProductQuantities($productStockCriteria) {
		...
		return ( int ) $this->getProductPhysicalQuantities ( $productStockCriteria ['product_id'], $productStockCriteria ['product_attribute_id'], $productStockCriteria ['warehouse_id'], $usable = true );
	}
	...
	public function transferBetweenWarehouses($id_product, $id_product_attribute, $quantity, $id_warehouse_from, $id_warehouse_to, $usable_from = true, $usable_to = true) {
		...
		if ($this->getProductPhysicalQuantities ( $id_product, $id_product_attribute, array (
				$id_warehouse_from 
		), $usable_from ) < $quantity) {
			return false;
		}
	...
	/**
	 *
	 * @see StockManagerInterface::getProductCoverage() Here, $coverage is a number of days
	...
	public function getProductCoverage($id_product, $id_product_attribute, $coverage, $id_warehouse = null) {
		...
		$physical_quantity = $this->getProductPhysicalQuantities ( $id_product, $id_product_attribute, ($id_warehouse ? array (
				$id_warehouse 
		) : null), true );
	 ...
	 */
	protected function getStockCollection($id_product, $id_product_attribute, $id_warehouse = null, $price_te = null, Stock $stock = null) {
	 ...
	 *        	$isUsable
	 */
	protected function hookCoverageOnProductRemoval(Warehouse $warehouse, $productId, $productAttributeId, $isUsable) {
		if ($isUsable) {
	 ...
	 * @return int
	 */
	protected function computeProductQuantityInStock(Warehouse $warehouse, $productId, $productAttributeId, $shouldHandleUsableQuantity, Stock $stock = null) {
	 ...
	 */
	protected function getProductStockLinesInWarehouse($id_product, $id_product_attribute, Warehouse $warehouse, Stock $stock = null) {
	 ...
	 *        	$stock
	 */
	public function removeProductQuantityApplyingCump($quantity, $id_stock_mvt_reason, $is_usable, $id_order, $employee, $stock) {
		$employeeAttributes = $this->getAttributesOfEmployeeRequestingStockMovement ( $employee );
