...
class FileUploaderCore {
	...
	public function handleUpload() {
		if (! $this->file) {
			return array (
					'error' => Tools::displayError ( 'No files were uploaded.' ) 
			);
		}
		...
		if ($size == 0) {
			return array (
					'error' => Tools::displayError ( 'File is empty' ) 
			);
		}
		if ($size > $this->sizeLimit) {
			return array (
					'error' => Tools::displayError ( 'File is too large' ) 
			);
		}
		...
		if (! isset ( $pathinfo ['extension'] )) {
			return array (
					'error' => sprintf ( Tools::displayError ( 'File has an invalid extension, it should be one of these: %s.' ), $these ) 
			);
		}
...
class QqUploadedFileForm {
	...
	public function save() {
		...
		if (! Validate::isLoadedObject ( $product )) {
			return array (
					'error' => Tools::displayError ( 'Cannot add image because product creation failed.' ) 
			);
		} else {
			$image = new Image ();
			...
			if (is_array ( $legends )) {
				foreach ( $legends as $key => $legend ) {
					if (Validate::isGenericName ( $legend )) {
						...
						return array (
								'error' => sprintf ( Tools::displayError ( 'Error on image caption: "%1s" is not a valid caption.' ), Tools::safeOutput ( $legend ) ) 
						);
					}
				}
			}
			...
			if (($validate = $image->validateFieldsLang ( false, true )) !== true) {
				return array (
						'error' => Tools::displayError ( $validate ) 
				);
			}
			if (! $image->add ()) {
				return array (
	...
	public function copyImage($id_product, $id_image, $method = 'auto') {
		...
		if (! $new_path = $image->getPathForCreation ()) {
			return array (
					'error' => Tools::displayError ( 'An error occurred during new folder creation' ) 
			);
		}
