...
class CMSCategoryCore extends ObjectModel {
	...
	public static function recurseCMSCategory($categories, $current, $id_cms_category = 1, $id_selected = 1, $is_html = 0) {
		$html = '<option value="' . $id_cms_category . '"' . (($id_selected == $id_cms_category) ? ' selected="selected"' : '') . '>' . str_repeat ( '&nbsp;', $current ['infos'] ['level_depth'] * 5 ) . CMSCategory::hideCMSCategoryPosition ( stripslashes ( $current ['infos'] ['name'] ) ) . '</option>';
		if ($is_html == 0) {
			echo $html;
		}
	 ...
	 *
	 * @param
	 *        	array &$toDelete Array reference where categories ID will be saved
	 * @param array|int $id_cms_category
	 ...
	 * @param int $id_lang
	 *        	Language ID
	 * @param string $CMSCategory_name
	 *        	Searched CMSCategory name
	 * @param int $id_parent_CMSCategory
	 *        	parent CMSCategory ID
	...
	public function updatePosition($way, $position) {
		...
		return (Db::getInstance ()->execute ( '
			...
			AND `id_parent`=' . ( int ) $moved_category ['id_parent'] ) && Db::getInstance ()->execute ( '
			UPDATE `' . _DB_PREFIX_ . 'cms_category`
