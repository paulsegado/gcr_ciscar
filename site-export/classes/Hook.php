...
class HookCore extends ObjectModel {
	...
	public static function getModulesFromHook($id_hook, $id_module = null) {
		...
		if ($id_module) {
			return (isset ( $module_list [$id_module] )) ? array (
	 ...
	 *        	If specified, module output will be set by name in an array
	 * @param bool $check_exceptions
	 *        	Check permission exceptions
	 * @param bool $use_push
	 *        	Force change to be refreshed on Dashboard widgets
	 * @param int $id_shop
	 *        	If specified, hook will be execute the shop with this ID
	 *        	
		...
		return '<script type="text/javascript"> modules_list.push(\'' . Tools::safeOutput ( $moduleInstance->name ) . '\');</script>
				...
				class="dndModule" style="border: 1px dotted red;' . (! strlen ( $display ) ? 'height:50px;' : '') . '">
					<span style="font-family: Georgia;font-size:13px;font-style:italic;">
						<img style="padding-right:5px;" src="' . _MODULE_DIR_ . Tools::safeOutput ( $moduleInstance->name ) . '/logo.gif">' . Tools::safeOutput ( $moduleInstance->displayName ) . '<span style="float:right">
	...
	public static function updateOrderStatus($new_order_status_id, $id_order) {
		...
		$return = (( int ) $new_os->id == Configuration::get ( 'PS_OS_PAYMENT' )) ? Hook::exec ( 'paymentConfirm', array (
				'id_order' => ( int ) ($order->id) 
		) ) : true;
		$return = Hook::exec ( 'updateOrderStatus', array (
	...
	public static function postUpdateOrderStatus($new_order_status_id, $id_order) {
		...
		$order = new Order ( ( int ) $id_order );
		$new_os = new OrderState ( ( int ) $new_order_status_id, $order->id_lang );
		$return = Hook::exec ( 'postUpdateOrderStatus', array (
				'newOrderStatus' => $new_os,
				'id_order' => ( int ) ($order->id) 
		) );
		return $return;
	...
	public static function PDFInvoice($pdf, $id_order) {
		...
		return Hook::exec ( 'PDFInvoice', array (
				'pdf' => $pdf,
				'id_order' => $id_order 
		) );
	}
	...
	public static function backBeforePayment($module) {
		Tools::displayAsDeprecated ();
		if ($module) {
			return Hook::exec ( 'backBeforePayment', array (
	...
	public static function updateCarrier($id_carrier, $carrier) {
		...
		return Hook::exec ( 'updateCarrier', array (
	...
	public static function updateQuantity($product, $order = null) {
		Tools::displayAsDeprecated ();
		return Hook::exec ( 'updateQuantity', array (
				'product' => $product,
				'order' => $order 
		) );
	}
	...
	public static function productFooter($product, $category) {
		Tools::displayAsDeprecated ();
		return Hook::exec ( 'productFooter', array (
				'product' => $product,
				'category' => $category 
		) );
	}
	...
	public static function productOutOfStock($product) {
		Tools::displayAsDeprecated ();
		return Hook::exec ( 'productOutOfStock', array (
				'product' => $product 
		) );
	}
	...
	public static function addProduct($product) {
		Tools::displayAsDeprecated ();
		return Hook::exec ( 'addProduct', array (
				'product' => $product 
		) );
	}
	...
	public static function updateProduct($product) {
		Tools::displayAsDeprecated ();
		return Hook::exec ( 'updateProduct', array (
				'product' => $product 
		) );
	}
	...
	public static function deleteProduct($product) {
		Tools::displayAsDeprecated ();
		return Hook::exec ( 'deleteProduct', array (
				'product' => $product 
		) );
	}
	...
	public static function updateProductAttribute($id_product_attribute) {
		Tools::displayAsDeprecated ();
		return Hook::exec ( 'updateProductAttribute', array (
