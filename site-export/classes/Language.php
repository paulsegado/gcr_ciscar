...
?>' );
	 ...
	 *
	 * @param bool $active
	 *        	Select only active languages
	 * @param int|bool $id_shop
	 *        	Shop ID
	 * @param bool $ids_only
	 *        	If true, returns an array of language IDs
	 *        	
	...
	public static function getLanguageByIETFCode($code) {
		...
		// or a close match.
		$id_lang = Db::getInstance ()->getValue ( 'SELECT `id_lang`, IF(language_code = \'' . pSQL ( $code ) . '\', 0, LENGTH(language_code)) as found
	...
	public static function checkAndAddLanguage($iso_code, $lang_pack = false, $only_add = false, $params_lang = null) {
		...
		if (isset ( $params_lang ['allow_accented_chars_url'] ) && in_array ( $params_lang ['allow_accented_chars_url'], array (
				'1',
				'true' 
		) )) {
	...
	public static function downloadAndInstallLanguagePack($iso, $version = null, $params = null, $install = true) {
		...
		if (! file_exists ( $file )) {
			...
			if (! $gz->extractList ( AdminTranslationsController::filesListToPaths ( $files_list ), _PS_TRANSLATIONS_DIR_ . '../' ) || ! $gz->extractList ( $translations_extract, _PS_TRANSLATIONS_DIR_ . '../' )) {
