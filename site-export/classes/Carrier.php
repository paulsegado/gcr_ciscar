...
class CarrierCore extends ObjectModel {
	...
	public function getDeliveryPriceByWeight($total_weight, $id_zone) {
		...
		$price_by_weight = Hook::exec ( 'actionDeliveryPriceByWeight', array (
	...
	public function getDeliveryPriceByPrice($order_total, $id_zone, $id_currency = null) {
		...
		$price_by_price = Hook::exec ( 'actionDeliveryPriceByPrice', array (
				'id_carrier' => $id_carrier,
	 ...
	 * @param int $id_lang
	 *        	Language id
	 * @param $modules_filters, possible
	 *        	values:
	 *        	PS_CARRIERS_ONLY
	 *        	CARRIERS_MODULE
	...
	public static function getCarriers($id_lang, $active = false, $delete = false, $id_zone = false, $ids_group = null, $modules_filters = self::PS_CARRIERS_ONLY) {
		...
		LEFT JOIN `' . _DB_PREFIX_ . 'carrier_zone` cz ON (cz.`id_carrier` = c.`id_carrier`)' . ($id_zone ? 'LEFT JOIN `' . _DB_PREFIX_ . 'zone` z ON (z.`id_zone` = ' . ( int ) $id_zone . ')' : '') . '
	 ...
	 * @param array $defaultCarrier
	 *        	the last carrier selected
	...
	public static function getCarriersForOrder($id_zone, $groups = null, $cart = null, &$error = array()) {
		...
		if (is_array ( $groups ) && ! empty ( $groups )) {
			...
			$result = Carrier::getCarriers ( $id_lang, true, false, ( int ) $id_zone, array (
	 ...
	 *
	 * @param string $rangeTable
	 *        	Table name to clean (weight or price according to shipping method)
	 ...
	 *        	Prices list in multiple arrays (changed to array since 1.5.0)
	...
	public function addDeliveryPrice($price_list, $delete = false) {
		...
		foreach ( $price_list as $values ) {
			...
			if ($delete) {
				Db::getInstance ()->execute ( 'DELETE FROM `' . _DB_PREFIX_ . 'delivery`
	 ...
	 *
	 * @param int $oldId
	 *        	Old id carrier (copy from that id)
	 */
	...
	public function updatePosition($way, $position) {
		if (! $res = Db::getInstance ()->executeS ( 'SELECT `id_carrier`, `position`
		...
		return (Db::getInstance ()->execute ( '
			...
			WHERE `position`
			' . ($way ? '> ' . ( int ) $moved_carrier ['position'] . ' AND `position` <= ' . ( int ) $position : '< ' . ( int ) $moved_carrier ['position'] . ' AND `position` >= ' . ( int ) $position . '
			AND `deleted` = 0') ) && Db::getInstance ()->execute ( '
			UPDATE `' . _DB_PREFIX_ . 'carrier`
	...
	/**
	 * Return the carrier name from the shop name (e.g.
	 * if the carrier name is '0').
	 *
