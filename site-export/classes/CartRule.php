...
class CartRuleCore extends ObjectModel {
	...
	public function getContextualValue($use_tax, Context $context = null, $filter = null, $package = null, $use_cache = true) {
		...
		if ($this->free_shipping && in_array ( $filter, array (
				CartRule::FILTER_ACTION_ALL,
		...
		// Free gift
		if (( int ) $this->gift_product && in_array ( $filter, array (
				CartRule::FILTER_ACTION_ALL,
				CartRule::FILTER_ACTION_ALL_NOCAP,
	...
	public function getAssociatedRestrictions($type, $active_only, $i18n, $offset = null, $limit = null, $search_cart_rule_name = '') {
		...
		if (! Validate::isLoadedObject ( $this ) || $this->{$type . '_restriction'} == 0) {
			...
			' . ($type == 'cart_rule' ? 'AND t.id_cart_rule != ' . ( int ) $this->id : '') . $shop_list . (in_array ( $type, array (
					'carrier',
					'shop' 
			) ) ? ' ORDER BY t.name ASC ' : '') . (in_array ( $type, array (
					'country',
					'group',
					'cart_rule' 
			) ) && $i18n ? ' ORDER BY tl.name ASC ' : '') . $sql_limit );
		} else {
			if ($type == 'cart_rule') {
				...
				WHERE 1 ' . ($active_only ? ' AND t.active = 1' : '') . $shop_list . (in_array ( $type, array (
						'carrier',
						'shop' 
				) ) ? ' AND t.deleted = 0' : '') . (in_array ( $type, array (
						'carrier',
						'shop' 
				) ) ? ' ORDER BY t.name ASC ' : '') . (in_array ( $type, array (
						'country',
						'group',
						'cart_rule' 
				) ) && $i18n ? ' ORDER BY tl.name ASC ' : '') . $sql_limit, false );
