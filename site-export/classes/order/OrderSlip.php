...
class OrderSlipCore extends ObjectModel {
	...
	public static $definition = array (
			...
			'fields' => array (
					...
					'total_products_tax_excl' => array (
							'type' => self::TYPE_FLOAT,
							'validate' => 'isFloat',
							'required' => true 
					),
					'total_products_tax_incl' => array (
							'type' => self::TYPE_FLOAT,
							'validate' => 'isFloat',
							'required' => true 
					),
			...
			'associations' => array (
					'order_slip_details' => array (
							...
							'fields' => array (
									...
									'amount_tax_excl' => array (
											'required' => true 
									),
									'amount_tax_incl' => array (
											'required' => true 
									) 
							) 
					) 
	...
	public static function getOrdersSlip($customer_id, $order_id = false) {
		...
		WHERE `id_customer` = ' . ( int ) ($customer_id) . ($order_id ? ' AND `id_order` = ' . ( int ) ($order_id) : '') . '
		ORDER BY `date_add` DESC' );
	}
	public static function getOrdersSlipDetail($id_order_slip = false, $id_order_detail = false) {
		return Db::getInstance ( _PS_USE_SQL_SLAVE_ )->executeS ( ($id_order_detail ? 'SELECT SUM(`product_quantity`) AS `total`' : 'SELECT *') . 'FROM `' . _DB_PREFIX_ . 'order_slip_detail`' . ($id_order_slip ? ' WHERE `id_order_slip` = ' . ( int ) ($id_order_slip) : '') . ($id_order_detail ? ' WHERE `id_order_detail` = ' . ( int ) ($id_order_detail) : '') );
	}
	...
	public static function create(Order $order, $product_list, $shipping_cost = false, $amount = 0, $amount_choosen = false, $add_tax = true) {
		...
		foreach ( $product_list as &$product ) {
			...
			if (in_array ( Configuration::get ( 'PS_ROUND_TYPE' ), array (
					Order::ROUND_ITEM,
					Order::ROUND_LINE 
			) )) {
	...
	public function setWsOrderSlipDetails($values) {
		if (Db::getInstance ()->execute ( 'DELETE from `' . _DB_PREFIX_ . 'order_slip_detail` where id_order_slip = ' . ( int ) $this->id )) {
			...
			foreach ( $values as $value ) {
				$query .= '(' . ( int ) $this->id . ', ' . ( int ) $value ['id_order_detail'] . ', ' . ( int ) $value ['product_quantity'] . ', ' . (isset ( $value ['amount_tax_excl'] ) ? ( float ) $value ['amount_tax_excl'] : 'NULL') . ', ' . (isset ( $value ['amount_tax_incl'] ) ? ( float ) $value ['amount_tax_incl'] : 'NULL') . '),';
			}
			$query = rtrim ( $query, ',' );
