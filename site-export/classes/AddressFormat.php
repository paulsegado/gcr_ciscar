...
class AddressFormatCore extends ObjectModel {
	...
	protected function _checkValidateClassField($className, $fieldName, $isIdField) {
		...
		if (! class_exists ( $className )) {
			...
			foreach ( $publicProperties as $property ) {
				...
				if (($propertyName == $fieldName) && ($isIdField || (! preg_match ( '/\bid\b|id_\w+|\bid[A-Z]\w+/', $propertyName )))) {
					$isValide = true;
				}
			}
	...
	protected function _checkLiableAssociation($patternName, $fieldsValidate) {
		...
		if ($associationName = explode ( ':', $patternName )) {
			...
			if ($totalNameUsed > 2) {
				...
				if (in_array ( $associationName [0], self::$forbiddenPropertyList ) || ! $this->_checkValidateClassField ( 'Address', $associationName [0], false )) {
	 ...
	 *        	A defined rules array to avoid some pattern
	 * @param string $newLine
	 *        	A string containing the newLine format
	 * @param string $separator
	 *        	A string containing the separator format
	 * @param array $style        	
	 * @return string
	 */
	...
	public static function getValidateFields($className) {
		...
		if (class_exists ( $className )) {
			...
			foreach ( $publicProperties as $property ) {
				$propertyName = $property->getName ();
				if ((! in_array ( $propertyName, AddressFormat::$forbiddenPropertyList )) && (! preg_match ( '#id|id_\w#', $propertyName ))) {
		...
		if (class_exists ( $className )) {
			...
			foreach ( $publicProperties as $property ) {
				...
				if (preg_match ( '#id_\w#', $propertyName ) && strlen ( $propertyName ) > 3) {
					...
					if (! in_array ( $nameObject, self::$forbiddenClassList ) && class_exists ( $nameObject )) {
	 ...
	 *        	If null using PS_COUNTRY_DEFAULT
