...
class MailCore extends ObjectModel {
	 ...
	 *
	 * @param int $id_lang
	 *        	Language ID of the email (to translate the template)
	 ...
	 * @param bool $mode_smtp
	 *        	SMTP mode (deprecated)
...
//<!-- CISCAR Ajout destinataire CISCAR en copie cache des mails -->		
		$message->addBcc ('info@ciscar.net');
	 ...
	 * @param int $smtp_port
	 *        	SMTP Port
	 * @param bool|string $smtp_encryption
	 *        	Encryption type. "off" or false disable encryption.
	...
	public static function sendMailTest($smtp_checked, $smtp_server, $content, $subject, $type, $to, $from, $smtp_login, $smtp_password, $smtp_port = 25, $smtp_encryption) {
		...
		try {
			if ($smtp_checked) {
				...
				$smtp = Swift_SmtpTransport::newInstance ( $smtp_server, $smtp_port, $smtp_encryption )->setUsername ( $smtp_login )->setPassword ( $smtp_password );
