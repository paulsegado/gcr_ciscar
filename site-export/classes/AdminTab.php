...
abstract class AdminTabCore {
	...
	public function __construct() {
		...
		$this->_conf = array (
				...
				2 => $this->l ( 'Selection successfully deleted' ),
				3 => $this->l ( 'Creation successful' ),
				4 => $this->l ( 'Update successful' ),
				5 => $this->l ( 'Status update successful' ),
				6 => $this->l ( 'Settings update successful' ),
				7 => $this->l ( 'Image successfully deleted' ),
				8 => $this->l ( 'Module downloaded successfully' ),
				9 => $this->l ( 'Thumbnails successfully regenerated' ),
				10 => $this->l ( 'Message sent to the customer' ),
				11 => $this->l ( 'Comment added' ),
				12 => $this->l ( 'Module installed successfully' ),
				13 => $this->l ( 'Module uninstalled successfully' ),
				14 => $this->l ( 'Language successfully copied' ),
				15 => $this->l ( 'Translations successfully added' ),
				16 => $this->l ( 'Module transplanted successfully to hook' ),
				17 => $this->l ( 'Module removed successfully from hook' ),
				18 => $this->l ( 'Upload successful' ),
				19 => $this->l ( 'Duplication completed successfully' ),
				20 => $this->l ( 'Translation added successfully but the language has not been created' ),
	...
	public function display() {
		...
		if ($this->includeSubTab ( 'display', array (
				'submitAdd2',
				'add',
				'update',
				'view' 
	...
	public function displayRequiredFields() {
		...
		$rules = call_user_func_array ( array (
				$this->className,
				'getValidationRules' 
		), array (
				$this->className 
		) );
		$required_class_fields = array (
				$this->identifier 
		);
	...
	public function validateRules($className = false) {
		...
		foreach ( $rules ['required'] as $field ) {
			if (($value = Tools::getValue ( $field )) == false && ( string ) $value != '0') {
				if (! Tools::getValue ( $this->identifier ) || ($field != 'passwd' && $field != 'no-picture')) {
					$this->_errors [] = sprintf ( Tools::displayError ( 'The field %s is required.' ), call_user_func ( array (
							$className,
							'displayFieldName' 
					), $field, $className ) );
				}
		...
		foreach ( $rules ['requiredLang'] as $fieldLang ) {
			if (($empty = Tools::getValue ( $fieldLang . '_' . $defaultLanguage->id )) === false || $empty !== '0' && empty ( $empty )) {
				$this->_errors [] = sprintf ( Tools::displayError ( 'The field %1$s is required at least in %2$s.' ), call_user_func ( array (
						$className,
						'displayFieldName' 
				), $fieldLang, $className ), $defaultLanguage->name );
			}
		}
		...
		foreach ( $rules ['size'] as $field => $maxLength ) {
			if (Tools::getValue ( $field ) !== false && Tools::strlen ( Tools::getValue ( $field ) ) > $maxLength) {
				$this->_errors [] = sprintf ( Tools::displayError ( 'field %1$s is too long. (%2$d chars max)' ), call_user_func ( array (
						$className,
						'displayFieldName' 
		...
		foreach ( $rules ['sizeLang'] as $fieldLang => $maxLength ) {
			foreach ( $languages as $language ) {
				if (Tools::getValue ( $fieldLang . '_' . $language ['id_lang'] ) !== false && Tools::strlen ( Tools::getValue ( $fieldLang . '_' . $language ['id_lang'] ) ) > $maxLength) {
					$this->_errors [] = sprintf ( Tools::displayError ( 'field %1$s is too long. (%2$d chars max, html chars including)' ), call_user_func ( array (
							$className,
							'displayFieldName' 
		...
		foreach ( $rules ['validate'] as $field => $function ) {
			if (($value = Tools::getValue ( $field )) !== false && ! empty ( $value ) && ($field != 'passwd')) {
				if (! Validate::$function ( $value )) {
					$this->_errors [] = sprintf ( Tools::displayError ( 'The field %1$s (%2$s) is invalid.' ), call_user_func ( array (
							$className,
							'displayFieldName' 
					), $field, $className ) );
				}
	...
	public function postProcess() {
		...
		$this->includeSubTab ( 'postProcess', array (
				'status',
				'submitAdd1',
				'submitDel',
				'delete',
			...
			foreach ( $this->optionsList as $category => $categoryData ) {
				...
				if (! count ( $this->_errors )) {
					foreach ( $fields as $key => $options ) {
						...
						} elseif (isset ( $options ['type'] ) && in_array ( $options ['type'], array (
								'textLang',
								'textareaLang' 
						) )) {
							$list = array ();
	...
	public function displayListContent($token = null) {
		...
		if ($this->_list) {
			...
			$keyToGet = 'id_' . ($isCms ? 'cms_' : '') . 'category' . (in_array ( $this->identifier, array (
					'id_category',
					'id_cms_category' 
			) ) ? '_parent' : '');
			foreach ( $this->_list as $tr ) {
				...
				foreach ( $this->fieldsDisplay as $key => $params ) {
					...
					if (isset ( $params ['active'] ) && isset ( $tr [$key] )) {
						...
						echo isset ( $params ['callback'] ) ? call_user_func_array ( array (
								(isset ( $params ['callback_object'] )) ? $params ['callback_object'] : $this->className,
								$params ['callback'] 
						), array (
								$echo,
								$tr 
	...
	protected function _displayEnableLink($token, $id, $value, $active, $id_category = null, $id_product = null) {
		$href = Tools::safeOutput ( self::$currentIndex . '&' . $this->identifier . '=' . ( int ) $id . '&' . $active . $this->table . (( int ) $id_category && ( int ) $id_product ? '&id_category=' . ( int ) $id_category : '') . '&token=' . ($token != null ? $token : $this->token) );
	...
	protected function _displayDeleteLink($token = null, $id) {
		...
		echo '<a href="' . $href . '" onclick="return confirm(\'' . $_cacheLang ['DeleteItem'] . ( int ) $id . ' ?' . (! is_null ( $this->specificConfirmDelete ) ? '\r' . $this->specificConfirmDelete : '') . '\');">
	 ...
	 * @param bool $use_vars_instead_of_ids
	...
	protected function warnDomainName() {
		if ($_SERVER ['HTTP_HOST'] != Configuration::get ( 'PS_SHOP_DOMAIN' ) && $_SERVER ['HTTP_HOST'] != Configuration::get ( 'PS_SHOP_DOMAIN_SSL' ) && ! defined ( '_PS_HOST_MODE_' )) {
			$this->displayWarning ( $this->l ( 'You are currently connected with the following domain name:' ) . ' <span style="color: #CC0000;">' . $_SERVER ['HTTP_HOST'] . '</span><br />' . $this->l ( 'This one is different from the main shop\'s domain name set in "Preferences > SEO & URLs":' ) . ' <span style="color: #CC0000;">' . Configuration::get ( 'PS_SHOP_DOMAIN' ) . '</span><br />
			<a href="index.php?tab=AdminMeta&token=' . Tools::getAdminTokenLite ( 'AdminMeta' ) . '#SEO%20%26%20URLs">' . $this->l ( 'Click here if you want to modify the main shop\'s domain name' ) . '</a>' );
		}
	}
