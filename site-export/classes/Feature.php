...
class FeatureCore extends ObjectModel {
	...
	public function add($autodate = true, $nullValues = false) {
		...
		Hook::exec ( 'actionFeatureSave', array (
				'id_feature' => $this->id 
		) );
		return $return;
	}
	public function delete() {
		...
		if ($return) {
			Hook::exec ( 'actionFeatureDelete', array (
					'id_feature' => $this->id 
			) );
		}
	...
	public function updatePosition($way, $position, $id_feature = null) {
		...
		return (Db::getInstance ()->execute ( '
			...
			WHERE `position`
			' . ($way ? '> ' . ( int ) $moved_feature ['position'] . ' AND `position` <= ' . ( int ) $position : '< ' . ( int ) $moved_feature ['position'] . ' AND `position` >= ' . ( int ) $position) ) && Db::getInstance ()->execute ( '
			UPDATE `' . _DB_PREFIX_ . 'feature`
