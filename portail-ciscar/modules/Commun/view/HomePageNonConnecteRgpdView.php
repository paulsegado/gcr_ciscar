 ...
 * @version 1.0.4
 */
class HomePageNonConnecteRgpdView {
	...
	public function renderHTML($msg = Null, $urlReturn = Null) {
		...
		$aff .= '#inputMdp0mail, #inputmdp0pwd1,#inputmdp0pwd2 {
			background-color: transparent;
		...
		<script type="text/javascript">
		$(function(){
			// MDP0 => observe link with id "mdp0_link"
			$("#mdp0_link").fancybox({
				overlayOpacity:0.8,
				overlayColor:\'#000000\'
			});
		...
		$aff .= '<input id="inputfield" name="username" placeholder="Nom d\'utilisateur" required type="text" ';
		...
		$aff .= '<!-- LINK THAT OPEN FANCYBOX -->
				<a href="#login_lost" id="login_lost_link" style="font-family:Fjalla One,helvetica,arial;font-size:12px;color:white;">Mot de passe oublié ?</a>';		
		$aff .= '</div>';			
		$aff .= '</form>';
		...
		$aff .= '<div style="display:none;"><div id="mdp0" style="text-align:left;font-family:arial;font-size:12px;">';
		$aff .= '             <h1 style="color:#ee8b1e;">Changement d\'identifiants...</h1>';
		$aff .= '              <p><font style="color:#b91509;font-weight:bold;">A compter du xx/xx/xxxx vos identifiants ne seront plus actifs.</font><br/>Le Règlement général sur la protection des données (RGPD) nous impose de modifier</br> notre système de déclaration et d\'enregistrement des mots de passe.</br>
		...
		$aff .= '              <div>
								...
								<td >
								<input id="inputmdp0mail" style="margin-bottom:5px;" type="text" value="'.$mdpmail.'" class="input_login_lost" name="mdp0mail" >
								</td>
								</tr>
								<tr>
								<td >								
								...
								<td>
								<input type="hidden" name="username" value="'.$mdpuser.'">
								<input type="hidden" name="password" value="'.$mdppwd.'">
		...
		$aff .= '              <div id="login_rgpd_error" class="display_none">Adresse mail déjà existante...</div>';
		$aff .= '             <div id="login_rgpd_success" class="display_none">Nous venons de vous envoyer un e-mail avec votre mot de passe.<br/>Merci de vérifier votre boite de réception (ou courrier indésirable).</div>';		
		$aff .= '</div></div>';
		...
		// Validation mpot de passe RGPD
		$aff .= '$(function() {
			$( "#button-mdpValidate" ).on( "click", function() {
			if ($(".password-strength").width() / $(".password-strength").parent().width() * 100 < 54) 
			{
		    $.confirm({
		        title: \'<label style="font-family:arial;color:#444;">Erreur sur Mot de passe!</label>\',
		        content: \'<label style="font-family:arial;text-align:left;font-weight: normal;color:#444">8 caractères minimum. Mélanges de majuscules, de chiffres ou de caractères spéciaux.</label>\',
		        type: \'red\',
		        typeAnimated: true,
				boxWidth: \'20%\',
				useBootstrap: false,
			...
			$.ajax({
				url: "/index.php?action=ajaxmotdepasseRgpd",
				type: "POST",
				...
				success: function(code_html, statut){
					if(code_html!="ko"){
						if(code_html=="false" || code_html=="falsefalse" || code_html=="falsefalseok"){
							...
							$("#login_rgpg_error").hide();
							$("#login_rgpd_success").show();
						}
					}else{
						$("#login_rgpd_error").show();
						$("#login_rgpd_success").hide();
					}
				}
			});
			return false;
